...
// 
//---------------------------------------------------------------------------------------
inline int CServerConnection::MakeSureCompletedPacket(CNtlNetBuffer* pBuffer, WORD* pwPacketLength, CNtlPacketEncoder * pPacketEncoder)
{
	...
	// 헤더가 완성되지 않은 경우
	if ( pBuffer->GetWorkRemainSize() < PACKET_HEADSIZE )
	{
	...
	// 헤더가 버퍼에서 잘린 경우
	if ( nLinearSize < PACKET_HEADSIZE )
	{
		::CopyMemory( pBuffer->InGetQueueExtraPtr(), pBuffer->InGetQueueBufferPtr(), PACKET_HEADSIZE - nLinearSize );
	}


	PACKETHEADER* pHeader = (PACKETHEADER*) pBuffer->GetQueueWorkPtr();

	if( pPacketEncoder )
	{
		int rc = pPacketEncoder->RxDecrypt( pHeader );
		if( NTL_SUCCESS != rc )
		{
			return rc;
		}
	...
	int nPacketSize = PACKET_HEADSIZE + pHeader->wPacketLen;
	...
	if ( nLinearSize < nPacketSize )
	{
		if ( nLinearSize < PACKET_HEADSIZE )
		{
			::CopyMemory(	pBuffer->GetQueueWorkPtr() + PACKET_HEADSIZE,
				pBuffer->InGetQueueBufferPtr() + PACKET_HEADSIZE - nLinearSize,
				pHeader->wPacketLen );
		}
		else
		{
...
//-----------------------------------------------------------------------------------
#if __SGPRO_BOTSYSTEM_SENDANDRECVTESTCODE__
bool CNtlClientNet::Connect( __int64 nBotID, char* szServerAddress, WORD wPort, HSERVER * pServerHandle, bool bAsync /* = false */, eNETCLIENT_ENCODE_TYPE encode /* = NETCLIENT_ENCODE_RANDKEY */, int nMaxPacketCount /* = 0 */, int nMaxPacketSize /* = 0 */ )
#else
bool CNtlClientNet::Connect(char* szServerAddress, WORD wPort, HSERVER * pServerHandle, bool bAsync /* = false */, eNETCLIENT_ENCODE_TYPE encode /* = NETCLIENT_ENCODE_RANDKEY */, int nMaxPacketCount /* = 0 */, int nMaxPacketSize /* = 0 */)
#endif
{
	_ASSERT( NULL != m_pImp );
...
//-----------------------------------------------------------------------------------
#if __SGPRO_BOTSYSTEM_SENDANDRECVTESTCODE__
bool CNtlClientNet::Connect( __int64 nBotID, WCHAR* pwszServerAddress, WORD wPort, HSERVER * pServerHandle, bool bAsync /* = false */, eNETCLIENT_ENCODE_TYPE encode /* = NETCLIENT_ENCODE_RANDKEY */, int nMaxPacketCount /* = 0 */, int nMaxPacketSize /* = 0 */)
#else
bool CNtlClientNet::Connect(WCHAR* pwszServerAddress, WORD wPort, HSERVER * pServerHandle, bool bAsync /* = false */, eNETCLIENT_ENCODE_TYPE encode /* = NETCLIENT_ENCODE_RANDKEY */, int nMaxPacketCount /* = 0 */, int nMaxPacketSize /* = 0 */)
#endif
{
...
int	CServerConnection::RecvPendingData()
{
	...
	do 
	{
		...
		PACKETHEADER* pHeader = (PACKETHEADER*)packet.GetPacketHeader();
		...
		if( NTL_SUCCESS != rc )
		{
			...
			CONNECTION_LOGDL("Packet Header[0x%02X:0x%02X:0x%02X:0x%02X]", pHeader->bEncrypt, pHeader->wPacketLen, pHeader->bySequence, pHeader->byChecksum);
		/*	int i = 0;
				char str[1024];
				for (i = 0; i < packet.GetPacketDataSize(); i++)
				{
					if (i > 0) strcat(str, ":");
					strcat(str, (const char*)packet.GetPacketData()[i]);
				}*/
				
				//CONNECTION_LOGDL("Packet Header[0x%02X:0x%02X:0x%02X:0x%02X] [Data %s]", pHeader->bEncrypt, pHeader->wPacketLen, pHeader->bySequence, pHeader->byChecksum, str);
			CONNECTION_LOGDL( "MakeSureCompletedPacket Fail : (%d)%s Size:%d ", rc, NtlGetErrorMessage(rc), wPacketLength);
			return -1;
		}
		...
		/*	if (pPacket->GetPacketHeader()->bySequence != (m_dwRecvPacketCount & PACKET_MAX_SEQUENCE))
			{
			int i = 0;
			char str[1024];
			for (i = 0; i < pPacket->GetPacketDataSize(); i++)
			{
			if (i > 0) strcat(str, ":");
			strcat(str, (const char*)pPacket->GetPacketData()[i]);
			}

			CONNECTION_LOGDL("Packet Header[0x%02X:0x%02X:0x%02X:0x%02X] [Data %s]", pHeader->bEncrypt, pHeader->wPacketLen, pHeader->bySequence, pHeader->byChecksum, str);

			CONNECTION_LOGDL("Packet Sequence Error Packet Sequence[%u] != Cur Sequence[%u]", pPacket->GetPacketHeader()->bySequence, m_dwRecvPacketCount & PACKET_MAX_SEQUENCE);
			SAFE_DELETE(pPacket);
			return -1;
			}*/
...
int CServerConnection::SendPendingData()
{
	...
	while( pPacket )	
	{
		...
		pPacket->GetPacketHeader()->bySequence = (BYTE) ( m_dwSendPacketCount & PACKET_MAX_SEQUENCE );
...
#endif
{
	...
	else
	{
		...
		{
			NET_LOGDL( "Socket Connect Error : (%d) %s Address:%s:%d", rc, NtlGetErrorMessage( rc ), remoteAddr.GetDottedAddr(), remoteAddr.GetPort() );
			RELEASE_CONNECTION( pConnection );
			return false;
		}
	}
...
void CEventBaseNetworkImp::OnThreadProc()
{
	...
	while( m_bProgress )
	{
		...
		//  [5/23/2007 zeroera] : EventTotal 0 들어오면 윈도우2000에서 나는 에러 방지
		if( 0 == dwEventTotal )
		{
			Sleep( 1000 );
			continue;
