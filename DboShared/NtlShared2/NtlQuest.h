...
{
	sQUEST_INVENTORY_FULLDATA()
	{
		memset(aQuestItemData, -1, sizeof(aQuestItemData));

		for (uint8_t i = 0; i < NTL_QUEST_INVENTORY_SLOT_COUNT; ++i)
		{
			aQuestItemData[i].byPos = i;
		}
	}

	sQUESTITEM_DATA* GetQuestItem(TBLIDX id)
	{
		for (uint8_t i = 0; i < NTL_QUEST_INVENTORY_SLOT_COUNT; ++i)
		{
			if (aQuestItemData[i].tblidx == id)
				return &aQuestItemData[i];
		}
		return NULL;
	}

	sQUESTITEM_DATA* CreateQuestItem(TBLIDX id) {
		for (uint8_t i = 0; i < NTL_QUEST_INVENTORY_SLOT_COUNT; ++i) {
			if (aQuestItemData[i].tblidx == 0 || aQuestItemData[i].tblidx == INVALID_TBLIDX) {
				aQuestItemData[i].tblidx = id;
				aQuestItemData[i].byCurStackCount = 0;
				return &aQuestItemData[i];
			}
		}
		return NULL;
	...
	void DeleteQuestItem(TBLIDX id) {
		for (uint8_t i = 0; i < NTL_QUEST_INVENTORY_SLOT_COUNT; ++i) {
			if (aQuestItemData[i].tblidx == id) {
				aQuestItemData[i].tblidx = INVALID_TBLIDX;
				aQuestItemData[i].byCurStackCount = 0;
			}
		}
	}
	...
	sQUESTITEM_DATA* GetQuestItem(uint8_t pos) {
		return &aQuestItemData[pos];
	}

	BYTE				byQuestItemCount;	// 아이템 개수
	sQUESTITEM_DATA		aQuestItemData[NTL_QUEST_INVENTORY_SLOT_COUNT];	// 아이템
};
