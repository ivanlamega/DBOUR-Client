...
eDRAGON_BALL_TYPE CDragonBallTable::GetDropItemType( UINT dropTblidx ) 
{
    ...
    for(BYTE j = SCHEDULE_EVENT_TYPE_FIRST; j <= SCHEDULE_EVENT_TYPE_LAST; ++j)
    {
		...
		for (int i = 0; i < 7; i++) 
		{
			if (pTblData && pTblData->aBallTblidx[i] == dropTblidx)
...
bool CDragonBallTable::AddTable(void * pvTable, bool bReload)
{
...
bool CDragonBallTable::LoadFromBinary(CNtlSerializer& serializer, bool bReload)
{
	if( false == bReload )
	{
		Reset();
