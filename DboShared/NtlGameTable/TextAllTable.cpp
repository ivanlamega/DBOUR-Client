...
bool CTextTable::AddTable( VOID* pvTable, bool bReload )
{
...
bool CTextTable::LoadFromBinary(CNtlSerializer& serializer, bool bReload)
{
	if( false == bReload )
	{
		Reset();
...
bool CTextTable::InitializeFromXmlDoc(CNtlXMLDoc* pXmlDoc, WCHAR* pwszFileName, bool bReload)
{
	if( NULL == pXmlDoc )
	{
		return false;
...
BOOL CTextTable::GetText( TBLIDX tblidx, std::wstring* pwstr )
{
	...
	if( !pData )
	{
		  swprintf_s( buf, 256, L"%d Tbl %u Index is Invalid\n\r", m_nType, tblidx );
		  std::wstring mywstr(buf);
		  std::string mycstr(mywstr.begin(), mywstr.end());
		  OutputDebugString(mycstr.c_str());
		//  *pwstr = buf;
		*pwstr = L" ";
...
bool CTextAllTable::LoadFromBinary(CNtlSerializer& serializer)
{
	Destroy();
