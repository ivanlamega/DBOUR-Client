...
VOID CPathHandler::OnSave()
{
	...
	string  FullPath		= FileDlg.GetFolderPath() + "\\";
	string  FileLocation	= "";
	string  FileExt			= ".pth4tool";

	FileLocation = FullPath + FileTitle + FileExt;

	if(::fopen_s(&pFile, FileLocation.c_str(), "wb") != 0)
	{
		DBO_TRACE(FALSE, "file open failed. (" << FileLocation.c_str() << ")");
		::SetCurrentDirectory(dirBackup);
		return;
	}
...
VOID CPathHandler::OnLoad()
{
	...
	if(::fopen_s(&pFile, FileLocation.c_str(), "rb") != 0)
	{
		DBO_TRACE(FALSE, "file open failed. (" << FileLocation.c_str() << ")");
		::SetCurrentDirectory(dirBackup);
		return;
