...
bool CNtlWEApplication::CreateWorldFromFolder(string& FullPathFileName)
{
	...
	RwChar cTemp[512];
	sprintf_s(cTemp, 512, "DBOUR Saber(%s-%s)-%s : Power comes in response to need, not desire. - San Gawku.", dNTL_WORLD_VERSION, dNTL_WORLD_TOOL_MODE, dGET_WORLD_PARAM()->WorldProjectFolderName);
...
#ifdef dNTL_WORLD_TOOL_MODE
	dGET_WORLD_PARAM()->WorldFarPlane = 1512.0f;
#else
...
void CNtlWEApplication::SetErrorReport()
{
	...
	BT_SetSupportServer("127.0.0.1", 9999);
...
RwBool CNtlWEApplication::CreateSubSystem(RwUInt32 iWidth, RwUInt32 iHeight, RwBool zBuffer)
{
	...
	//LinkTextureLoad(NULL, TextureLoadLevelPath);
	...
	FILE* pFile = NULL;
	if(fopen_s(&pFile, "./tool/saber/script/datamode.xml", "r"))
	{	
	...
	else
	{
		...
		if(!API_SLInit(TRUE, NULL, (RwUInt8)CTable::LOADING_METHOD_BINARY)) //Change this for raw/edf
			NTL_RETURN(FALSE);		
	}
...
bool CNtlWEApplication::CreateWorldFromFolderEx(string& FullPathFileName)
{
	...
	RwChar cTemp[512];
	sprintf_s(cTemp, 512, "DBONU Saber(%s-%s)-%s : Power comes in response to need, not desire - San Gawku.", dNTL_WORLD_VERSION, dNTL_WORLD_TOOL_MODE, dGET_WORLD_PARAM()->WorldProjectFolderName);
	dGETFRM()->SetWindowText(cTemp);
