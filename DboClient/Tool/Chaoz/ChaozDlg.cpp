...
void CChaozDlg::OnBnClickedButton_CheatPitchRate()
{
	...
	fNumber = (float)atof( (const char*)strText.GetString() );
	if( fNumber == 0 )
	{
...
void CChaozDlg::OnBnClickedButton_OpenSound()
{
	...
	if( LoadDlg.DoModal() == IDOK )
	{
		...
		if( pFindText )
		{
			m_Edit_FileName.SetWindowText( (LPCTSTR)pFindText + strlen("DBO_Resource\\sound\\") );
		}
		else
		{
...
void CChaozDlg::OnBnClickedButton_PlaySound()
{
	...
	{
		fTemp = (float)atof((const char*)strPosX.GetString());
		if( fTemp == 0 )
		{
			DisplayMessage( _T("Error! Input position x") );
			return;
	...
	{
		fTemp = (float)atof((const char*)strPosY.GetString());
		if( fTemp == 0 )
		{
			DisplayMessage( _T("Error! Input position y") );
			return;
	...
	{
		fTemp = (float)atof((const char*)strPosZ.GetString());
		if( fTemp == 0 )
		{
			DisplayMessage( _T("Error! Input position z") );
			return;
	...
	{
		fTemp = (float)atof((const char*)strMin.GetString());
		if( fTemp == 0 )
		{
			DisplayMessage( _T("Error! Input position z") );
			return;
	...
	{
		fTemp = (float)atof((const char*)strMax.GetString());
		if( fTemp == 0 )
		{
			DisplayMessage( _T("Error! Input min distance") );
			return;
	...
	{
		fTemp = (float)atof((const char*)strVolume.GetString());
		if( fTemp == 0 )
		{
			DisplayMessage( _T("Error! Input volume") );
			return;
...
{
	HWND hWnd = ::FindWindow(NULL, _T("DragonBall Online: Universe Revelations") );
	if( hWnd == NULL )
	{
		DisplayMessage( _T("Error! cleint is not operating") );
		return false;
	}
