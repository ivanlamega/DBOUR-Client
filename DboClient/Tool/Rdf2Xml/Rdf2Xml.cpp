...
#include <iomanip>
#include <iostream>
static CTable::eLOADING_METHOD loadingMethod = CTable::LOADING_METHOD_BINARY;
...
int _tmain(int argc, _TCHAR* argv[])
{
	...
	{
		loadingMethod = CTable::LOADING_METHOD_XML;
	}

	if (stricmp(szConvType, "edf2xml") == 0)
	{
		loadingMethod = CTable::LOADING_METHOD_SECURED_BINARY;
	}
	std::cout << "Process started at " << __TIMESTAMP__ << std::endl;
	...
	if (loadingMethod != CTable::LOADING_METHOD_XML)
	{
		//rdf2XmlHelper.SaveToXml_MobTable(szOutPath);
		//rdf2XmlHelper.SaveToXml_NpcTable(szOutPath);
		...
		//rdf2XmlHelper.SaveToXml_HLSItemTable(szOutPath);
		...
		//for (CTable::TABLEIT iter = rdf2XmlHelper.m_pTableContainer->GetWorldTable()->Begin(); rdf2XmlHelper.m_pTableContainer->GetWorldTable()->End() != iter; iter++)
		//{
		//	sWORLD_TBLDAT* pWorldTableData = (sWORLD_TBLDAT*)(iter->second);
//
		//	rdf2XmlHelper.SaveToXml_MobSpawnTable(szOutPath, pWorldTableData->tblidx);
			//rdf2XmlHelper.SaveToXml_NPCSpawnTable(szOutPath, pWorldTableData->tblidx);
	//		rdf2XmlHelper.SaveToXml_ObjectTable(szOutPath, pWorldTableData->tblidx);
		//}
	...
	//	rdf2XmlHelper.SaveToXml_WorldZoneTable(szOutPath); // Errors out sometimes cause of the wszNameText I think it was bigger but not? No way to explain this other than its fixed -- Kalisto
		...
		//rdf2XmlHelper.SaveToXml_ActionTable(szOutPath);
		//rdf2XmlHelper.SaveToXml_DragonBallTable(szOutPath);
	}
	else
	{
		...
		//rdf2XmlHelper.SaveToBinary_SystemEffectTable(szOutPath);
		//rdf2XmlHelper.SaveToBinary_ActionTable(szOutPath);
		//rdf2XmlHelper.SaveToBinary_HLSItemTable(szOutPath);
	}
	t2 = clock();
	float diff((float)t2 - (float)t1);
	float seconds = diff / CLOCKS_PER_SEC;

	std::cout <<"Execuition took " << seconds << " Seconds" << std::endl;
