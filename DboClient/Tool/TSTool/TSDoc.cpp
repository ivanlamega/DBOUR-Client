...
void CTSDoc::OnCreateProject( void )
{
	...
	if ( IsModified() )
	{
		int nRet = AfxMessageBox(_T("A previously opened project has been modified. \nDo you want to save it? "), MB_OKCANCEL );
...
void CTSDoc::OnExportTriggerAll( void )
{
	...
	if ( IsModified() )
	{
		int nRet = AfxMessageBox(_T("There is a trigger that you previously edited. \nDo you want to save it? "), MB_OKCANCEL );
		if ( IDCANCEL == nRet ) return;
...
void CTSDoc::OnEncryptionTrigger( void )
{
	...
	if ( IsModified() )
	{
		int nRet = AfxMessageBox(_T("There is a trigger that was previously being modified. \nDo you want to save it?"), MB_OKCANCEL );
		if ( IDCANCEL == nRet ) return;

		DoFileSave();
	}

	CFileDialog clFile( TRUE, NULL, NULL, OFN_HIDEREADONLY|OFN_OVERWRITEPROMPT, "TS Zip files (*.t)|*.t||", NULL, 0 );
	if ( IDOK == clFile.DoModal() )
	{
		CString strPath = clFile.GetPathName();
