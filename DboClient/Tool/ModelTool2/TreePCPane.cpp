...
RwBool CTreePCPane::SaveTreeXML(const WCHAR* szXmlFileName)
{
    ...
    for(size_t i = 0; i < vTreeItem.size(); ++i)
    {
        ...
        if(sItemName.Find(L"*") == -1)   // '*' 표시가 없는 것들만 저장한다.
        {
            // XML에 추가한다.
            if(nImage == 0)    // 폴더
            {
...
RwBool CTreePCPane::OnSaveScript(HTREEITEM hItem, const WCHAR* szScriptName, BOOL bVisible)
{
    ...
    if(bVisible == FALSE)
    {
		...
		sCurrentScriptFileName.MakeUpper();
    ...
    else
    {
        ...
        if(fileDlg.DoModal() == IDOK)
        {
			...
			sSaveFileName.MakeUpper();
            sSaveFileName.Replace(L".XML", L""); // 프로퍼티의 네임에서는 확장자를 제외한다.
