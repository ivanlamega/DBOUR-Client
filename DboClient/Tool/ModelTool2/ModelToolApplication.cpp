...
RwBool CModelToolApplication::CreateSubSystem(RwUInt32 iWidth, RwUInt32 iHeight, RwBool zBuffer)
{
	...
	//	CNtlPLPropertyContainer::GetInstance()->Load();
	//	CNtlPLPropertyContainer::GetInstance()->LoadSerialize();
//		CNtlPLPropertyContainer::GetInstance()->LoadSerialize(TRUE, ENC_DATA_PASSWORD);
	//	CNtlPLPropertyContainer::GetInstance()->Load();

    // Property Container를 생성하고, 데이터를 Load 한다.
//	if(!m_PropContainer.LoadSerialize(FALSE, ENC_DATA_PASSWORD))
//    if(!m_PropContainer.Load())
	//if (!CNtlPLPropertyContainer::GetInstance()->LoadSerialize(false))
...
//	strcat(dGET_WORLD_PARAM()->WorldProjectFolderName, "\\Tool\\ModelTool\\DBO_Test_Field");
	//strcat(dGET_WORLD_PARAM()->WorldProjectFolderName, "\\world\\dbo_korin_tower");
	strcat(dGET_WORLD_PARAM()->WorldProjectFolderName, "\\world\\dbo_town_002");
	//strcat(dGET_WORLD_PARAM()->WorldProjectFolderName, "\\world\\dbo_main_world");
	...
	if(FILE* pFile = fopen("#######.gwp", "rb"))
	{
		...
		//fread(&dGET_WORLD_PARAM()->WorldVer,					10,					1, pFile);
		//if(strcmp(dNTL_WORLD_VERSION, dGET_WORLD_PARAM()->WorldVer))
		//{
		// RwChar Msg[_MAX_PATH] = {0,};
		// sprintf_s(Msg, _MAX_PATH, "Current world[%s] is not the latest version[%s].", &dGET_WORLD_PARAM()->WorldVer, dNTL_WORLD_VERSION);
		// ::MessageBoxA(NULL, Msg, "DragonBall", MB_OK);
		///* return;*/
		//}

		//fread(&dGET_WORLD_PARAM()->WorldHeightfieldNum,		sizeof(RwInt32),	1, pFile);
		//fread(&dGET_WORLD_PARAM()->WorldSectorTileSize,		sizeof(RwInt32),	1, pFile);
		//fread(&dGET_WORLD_PARAM()->WorldSectorTileNum,		sizeof(RwInt32),	1, pFile);
		//fread(&dGET_WORLD_PARAM()->WorldSectorVertNum,		sizeof(RwInt32),	1, pFile);
		//fread(&dGET_WORLD_PARAM()->WorldSectorSize,			sizeof(RwInt32),	1, pFile);
		//fread(&dGET_WORLD_PARAM()->WorldSectorNum,			sizeof(RwInt32),	1, pFile);
		//fread(&dGET_WORLD_PARAM()->WorldSectorHalfNum,		sizeof(RwInt32),	1, pFile);
		//fread(&dGET_WORLD_PARAM()->WorldSectorPolyNum,		sizeof(RwInt32),	1, pFile);
		//fread(&dGET_WORLD_PARAM()->WorldFieldSectorNum,		sizeof(RwInt32),	1, pFile);
		//fread(&dGET_WORLD_PARAM()->WorldFieldSize,			sizeof(RwInt32),	1, pFile);
		//fread(&dGET_WORLD_PARAM()->WorldFieldNum,			sizeof(RwInt32),	1, pFile);
		//fread(&dGET_WORLD_PARAM()->WorldFieldHalfNum,		sizeof(RwInt32),	1, pFile);
		//fread(&dGET_WORLD_PARAM()->WorldSize,				sizeof(RwInt32),	1, pFile);
		//fread(&dGET_WORLD_PARAM()->WorldSizeHalf,			sizeof(RwInt32),	1, pFile);
		//fread(&dGET_WORLD_PARAM()->WorldValueMax,			sizeof(RwInt32),	1, pFile);
		//fread(&dGET_WORLD_PARAM()->WorldValueMin,			sizeof(RwInt32),	1, pFile);
		//fread(&dGET_WORLD_PARAM()->WorldTexAddr,			sizeof(RwInt32),	1, pFile);
		//fread(&dGET_WORLD_PARAM()->WorldPolyCnt,			sizeof(RwInt32),	1, pFile);
		//fread(&dGET_WORLD_PARAM()->WorldNearPlane,			sizeof(RwReal),		1, pFile);
		//fread(&dGET_WORLD_PARAM()->WorldFarPlane,			sizeof(RwReal),		1, pFile);
		//fread(&dGET_WORLD_PARAM()->WorldCamSpeedPerSec,		sizeof(RwReal),		1, pFile);
		//fread(dGET_WORLD_PARAM()->WorldRAW,					64,					1, pFile);
		//fread(&dGET_WORLD_PARAM()->WorldTriggerSerialID,	sizeof(RwUInt32),	1, pFile);
		//fread(&dGET_WORLD_PARAM()->IdxSkyType,				sizeof(RwInt32),	1, pFile);

		//fread(&dGET_WORLD_PARAM()->WorldType,				sizeof(RwInt32),	1, pFile);
		//fread(&dGET_WORLD_PARAM()->WorldBlockSize,			sizeof(RwInt32),	1, pFile);
		//fread(&dGET_WORLD_PARAM()->WorldBlockNum,			sizeof(RwInt32),	1, pFile);
		//fread(dGET_WORLD_PARAM()->WorldBSP,					64,					1, pFile);
		//	fclose(pFile);
	}
	...
	//dGET_WORLD_PARAM()->FlgObjInfoShow			= FALSE;
    ...
    RwV3d vWorldPos = { 0.0f, 0.0f, 0.0f };
