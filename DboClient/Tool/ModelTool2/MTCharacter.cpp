...
RwBool CMTCharacter::CreateMesh(const char *szDffName)
{
    ...
    if(m_pProperty)
    {
        ...
        CString strWorkPath = A2W(CModelToolApplication::GetInstance()->GetWorkDir());
        CString strFileName = A2W(strMeshFileName.c_str());
        CString strMeshFilePath = A2W(szDffName);
        strWorkPath.MakeUpper();
        strMeshFilePath.MakeUpper();
        strFileName.MakeUpper();

        strMeshFilePath.Replace(strWorkPath, L"");
        strMeshFilePath.Replace(strFileName, L"");

        m_pProperty->SetBaseMeshFilePath(W2A(strMeshFilePath));
...
int CMTCharacter::CallBackBaseAnim(RwUInt32 uiEventData)
{
    ...
    return CNtlPLCharacter::CallBackBaseAnim(uiEventData);
}
