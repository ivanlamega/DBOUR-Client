...
void CConvertingDlg::SetState( EConverState eState ) 
{
	...
	{
	case LOAD_CHARACTER:
		m_msgEdit.SetWindowText(L"Loading Character XML Data...");
		m_progress.SetPos(10);
		break;
	case LOAD_ITEM:
		m_msgEdit.SetWindowText(L"Loading Item XML Data...");
		m_progress.SetPos(20);
		break;
	case LOAD_OBJECT:
		m_msgEdit.SetWindowText(L"Loading Object XML Data...");
		m_progress.SetPos(30);
		break;
	case SAVE_DATA:
		m_msgEdit.SetWindowText(L"XML->Binary Data Converting...");
		m_progress.SetPos(40);
		break;	
	case SAVE_COMPLETE:
		m_msgEdit.SetWindowText(L"XML->Binary Data Convert Complete!!");
		m_progress.SetPos(50);
		break;
	case SAVE_FAIL:
		m_msgEdit.SetWindowText(L"XML->Binary Data Convert Fail!! (It may be Read Only File)");
		m_progress.SetPos(0);
		break;
    case UPDATE_CHAR:
        m_msgEdit.SetWindowText(L"Update Character Data Complete");
        m_progress.SetPos(40);
        break;
    case UPDATE_OBJ:
        m_msgEdit.SetWindowText(L"Update Object Data Complete");
        m_progress.SetPos(45);
        break;
    case UPDATE_ITEM:
        m_msgEdit.SetWindowText(L"Update Item Data Complete");
        m_progress.SetPos(50);
        break;
	}
}
...
RwBool CConvertingDlg::LoadPropertyAll() 
{
    ...
    {
        m_msgEdit.SetWindowText(L"Item XML Data Load Fail!");
        return FALSE;
    }
    ...
    {
        m_msgEdit.SetWindowText(L"Object XML Data Load Fail!");
        return FALSE;
    }
...
DWORD WINAPI CConvertingDlg::PropDataResave( LPVOID pParam ) 
{
    ...
    for(it = GetMapProp(PLENTITY_CHARACTER).begin(); it != GetMapProp(PLENTITY_CHARACTER).end(); ++it)
    {
        ...
        if(bSuccess)
        {
            msg.Format(L"Update : [%S]", it->first.c_str());
            pDlg->m_msgEdit.SetWindowText(msg);            
        }
        else 
        {
            msg.Format(L"FAIL : [%S] Character XML Data Load or Save Fail!", it->first.c_str());
            pDlg->m_msgEdit.SetWindowText(msg);            
        }    
    }
...
RwBool CConvertingDlg::UpdateAnimData(const char* szName, CNtlTypeAnimTable* pAnimTable)
{
    ...
    for(it = pAnimTable->GetTypeAnimMap()->begin(); it != pAnimTable->GetTypeAnimMap()->end(); ++it)
    {
		...
		{
			 WCHAR szMsg[1024] = {0,};
            swprintf_s(szMsg, L"Animation file %S not found\n",szAnimFile);
