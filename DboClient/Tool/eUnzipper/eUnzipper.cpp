...
bool LoadTSCryptoData(std::string strFile, std::string outdir, std::string version)
{
	...
	if(version == "TW")
		strKey = "dnfldbofmftkfkdgowntpdy";
	else
		strKey = "tddkaghghkqlalfqjsghdlqslek";
...
bool LoadTSCryptoData_UnZip(std::string strFile, std::string outfile, std::string version)
{
...
int main(int argc, char* argv[])
{
	...
	if (argc < 3) { // Check the value of argc. If not enough parameters have been passed, inform user and exit.
		std::cout << "Usage is <infile> <outfile> <KR/TW>\n"; // Inform the user of how to use the program
		std::cin.get();
		return 0;
	}
	...
	LoadTSCryptoData_UnZip(iFilename, oFilename, version);
	return 0;
}
