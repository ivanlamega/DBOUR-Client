...
//	Pack 사용
	...
	CNtlPLGuiManager::GetInstance()->GetReourceManager()->AddPage("gui\\Human\\Game.rsr");
	...
	//----------------------------------------------------
	if(!API_SLInit(TRUE, m_pCamera, CTable::LOADING_METHOD_BINARY, TRUE))
	{
		NTL_RETURN(FALSE);
	}
...
void CDirectApplication::OnMobCreate(void)
{
	...
	sMobBrief.wCurLP = pMobTblData->wBasic_LP;
	sMobBrief.wMaxLP = pMobTblData->wBasic_LP;
...
void CDirectApplication::CreteAvatarInfo(RwV3d vPos)
{
	...
	wcscpy_s(pAvatarInfo->sCharPf.awchName, NTL_MAX_SIZE_CHAR_NAME_UNICODE, L"DBOUR");
	...
	pAvatarInfo->sCharPf.wCurLP = 10000;
	pAvatarInfo->sCharPf.avatarAttribute.wBaseMaxLP = 10000;
	pAvatarInfo->sCharPf.avatarAttribute.wLastMaxLP = 10000;
...
//	pAvatarInfo->sItemPf[0].byCurDur		= 1;		// 내구도
	...
	{
		pAvatarInfo->sItemPf[0].aOptionTblidx[i]	= 0xffffffff;
	}
