...
int CKMMemDC::Create( CWnd* pSrcWnd, int width, int height, COLORREF clr )
{
	...
	if( pSrcWnd == NULL )
	{
		AfxMessageBox(L"CKMMemDC::CreateEmpty()\n\n 비정상적인 pSrcWnd" );
		return FALSE;
	}
							   ...
							   BITMAP* pBMInfo )
{
	...
	if( pSrcWnd == NULL )
	{
		AfxMessageBox( L"CKMMemDC::CreateFromID()\n\n 비정상적인 pSrcWnd" );
		return FALSE;
	}
	...
	BITMAP* pBMInfo)
{
	...
	if (pSrcWnd == NULL)
	{
		AfxMessageBox(L"CKMMemDC::CreateFromFile()\n\n 비정상적인 pSrcWnd");
		return FALSE;
	}
	...
	LPWSTR wszFileName =L"";
	MultiByteToWideChar(CP_ACP, 0, szFileName, -1, wszFileName, 4096);

	HBITMAP hBitmap;
	hBitmap = (HBITMAP) ::LoadImage( NULL, wszFileName, IMAGE_BITMAP,
									 0, 0, LR_LOADFROMFILE );
	if( hBitmap == NULL )
		return 0;
							    ...
							    BITMAP* pBMInfo )
{
	...
	if( pSrcWnd == NULL )
	{
		AfxMessageBox( L"CKMMemDC::CreateFromHBM()\n\n 비정상적인 pSrcWnd" );
		return FALSE;
	}
