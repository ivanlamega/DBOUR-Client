...
BOOL	CDataManager::LoadRsrFile( CString& strFullPath, CString& strSubPath, BOOL bTabChange /* = FALSE  */ )
{
	...
	sFilename = CT2A((LPCTSTR)strFullPath);
...
BOOL CDataManager::SaveRsrFile()
{
	...
	errno_t eValue = _wfopen_s( &pFile, (LPCTSTR)strFilename, L"wt" );
	if( eValue )
		return FALSE;
#else
	...
	for( it = m_listResourceData.begin() ; it != m_listResourceData.end() ; ++it )
	{
		_ftprintf( pFile, _T("resource \"%s\"\n{\n"), (LPCTSTR)(*it)->_strName );
		_ftprintf( pFile, _T("\thandle\t= %d;\n"), (*it)->_nHandle );
		_ftprintf( pFile, _T("\tfile\t= \"%s\";\n}\n\n"), (LPCTSTR)(*it)->_strFileName );
	}
...
BOOL	CDataManager::SaveSrfFile()
{
	...
	for( it = m_listSurfaceData.begin() ; it != m_listSurfaceData.end() ; ++it )
	{
		_ftprintf( pFile, _T("surface \"%s\"\n{\n"), (LPCTSTR)(*it)->_strName );
		_ftprintf( pFile, _T("\tresource_file\t= \"%s\";\n"), (LPCTSTR)(*it)->_strResourceFileName );
		_ftprintf( pFile, _T("\tresource_name\t= \"%s\";\n\n"), (LPCTSTR)(*it)->_strResourceName );
		_ftprintf( pFile, _T("\tx\t\t= %ld;\n"), (*it)->_rcCoord.left );
		_ftprintf( pFile, _T("\ty\t\t= %ld;\n"), (*it)->_rcCoord.top );
		_ftprintf( pFile, _T("\twidth\t\t= %ld;\n"), (*it)->_rcCoord.right );
		_ftprintf( pFile, _T("\theight\t\t= %ld;\n\n"), (*it)->_rcCoord.bottom );
		_ftprintf( pFile, _T("\tcolor_red\t= %d;\n"), (*it)->_cRed );
		_ftprintf( pFile, _T("\tcolor_green\t= %d;\n"), (*it)->_cGreen );
		_ftprintf( pFile, _T("\tcolor_blue\t= %d;\n"), (*it)->_cBlue );
		_ftprintf( pFile, _T("\tcolor_alpha\t= %d;\n\n"), (*it)->_cAlpha );
		_ftprintf( pFile, _T("\tuv_left\t\t= %ld;\n"), (*it)->_rcUV.left );
		_ftprintf( pFile, _T("\tuv_top\t\t= %ld;\n"), (*it)->_rcUV.top );
		_ftprintf( pFile, _T("\tuv_right\t= %ld;\n"), (*it)->_rcUV.right );
		_ftprintf( pFile, _T("\tuv_bottom\t= %ld;\n\n"), (*it)->_rcUV.bottom );
		_ftprintf( pFile, _T("\thandle\t\t= %d;\n"), (*it)->_nHandle );
		_ftprintf( pFile, _T("\tblend\t\t= %s;\n}\n\n"), (LPCTSTR)(*it)->_strBlendFunction );
	}
...
{
	std::string sFilename;
	sFilename = CT2A((LPCTSTR)m_strRsrFileName);
...
{
	std::string sFilename;
	sFilename = CT2A((LPCTSTR)m_strSrfFileName);
...
BOOL CDataManager::FileDialogOpen( CString* pFilename, TYPE eFileType, BOOL bLoad )
{
	...
	case RSR:
		lpszDefExt	 = L"rsr";
		lpszFilter	 = L"Resource Script File(*.rsr)|*.rsr|";
		break;
	case SRF:
		lpszDefExt	 = L"srf";
		lpszFilter	 = L"Surface Script File(*.srf)|*.srf|";
		break;
	case FRM:
		lpszDefExt	 = L"frm";
		lpszFilter	 = L"Component Script File(*.frm)|*.frm|";
		break;
	}
	
	CFileDialog openDlg( bOpenDlg, lpszDefExt, L"", dwFlags, lpszFilter );
...
void CDataManager::LoadResourceList()
{
	...
	std::string sFilename = CT2A((LPCTSTR)m_strRsrFileName);
...
void CDataManager::LoadSurfaceList()
{
	...
	std::string sFilename = CT2A((LPCTSTR)m_strSrfFileName);
...
BOOL CDataManager::SaveConfig()
{
	...
	wcscpy_s( buf, 256, (LPCTSTR)m_strWorkDir );
#else
