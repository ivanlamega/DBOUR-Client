...
void CRectInfoDlg::SetMousePoint( CPoint& point )
{
	...
	_sntprintf_s( buf, 256, _T("x : %ld y : %ld"), point.x, point.y );
#else
...
#if defined(_MSC_VER) && (_MSC_VER >= 1400)
	if( bResult )
		_sntprintf_s( buf, 256, _T("r:%X g:%X b:%X a:%X"), r, g, b, a );
	else
		_sntprintf_s( buf, 256, _T("Out of Region") );
#else
	if( bResult )
...
#if defined(_MSC_VER) && (_MSC_VER >= 1400)
	_sntprintf_s( buf, 256, _T("x : %ld y : %ld"), rect.left, rect.top );
#else
...
#if defined(_MSC_VER) && (_MSC_VER >= 1400)
	_sntprintf_s( buf, 256, _T("w : %d h : %d"), rect.Width(), rect.Height() );
#else
