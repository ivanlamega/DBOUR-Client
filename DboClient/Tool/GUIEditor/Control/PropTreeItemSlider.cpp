...
void CPropTreeItemSlider::OnMouseMove(UINT nFlags, CPoint point)
{
	...
	if( nFlags & MK_LBUTTON )
	{
		...
		if( m_FloatingPointMode )
			m_ValueString.Format(L"%4.4f",((FLOAT)GetValue() / 1000.0f) );
		else
			m_ValueString.Format(L"%d", GetValue() );
...
void CPropTreeItemSlider::SetItemValue(LPARAM lParam)
{
	...
	{	
		...
		m_ValueString.Format(L"%4.4f",*(FLOAT*)lParam);
	}
	else
	{
		value = (int)lParam;
		m_ValueString.Format(L"%d",value);
	}
