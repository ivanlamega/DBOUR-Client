...
{    
	...
	else
	{
		...
		CString strmessage;
		strmessage.Format( L"ERROR : Load bitmap resource [%s]", Filename );
...
#if defined(_MSC_VER) && (_MSC_VER >= 1400)
	errno_t nError;
	nError = fopen_s( &file, Filename, "wb" );
	if( nError )
	{
		return DIB_SAVE_CANT_WRITE;
	}
...
CDIB::LoadPPM ( LPSTR Filename )
{
...
#if defined(_MSC_VER) && (_MSC_VER >= 1400)
	errno_t nError;
	nError = fopen_s( &fp, Filename, "rb" );
	if( nError )
	{
		return DIB_LOAD_NOFILE_OR_CORRUPT;
		AfxMessageBox( L"File open Faild !" );
		goto cleanup;
	}
#else
    ...
    if (!strlen(tmp) || 		
#if defined(_MSC_VER) && (_MSC_VER >= 1400)
		(2 != sscanf_s( tmp, "%d %d",&Width,&Height)))   /* Obtain width and height */
#else
		(2 != sscanf (tmp,"%d %d",&Width,&Height)))   /* Obtain width and height */
#endif
    {
        res = DIB_LOAD_NOFILE_OR_CORRUPT;
		AfxMessageBox( L"Obtain width and height !" );
        goto cleanup;
    }
    fgets (tmp,1024,fp); 
    ...
    if (!strlen(tmp) ||
#if defined(_MSC_VER) && (_MSC_VER >= 1400)
		(1 != sscanf_s(tmp,"%d",&MaxColor)))  /* Get max color used */
#else
		(1 != sscanf (tmp,"%d",&MaxColor)))  /* Get max color used */
#endif
    {
        res = DIB_LOAD_NOFILE_OR_CORRUPT;
		AfxMessageBox( L"Max color !" );
        goto cleanup;
    }
    if (!Create (Width, Height))
    {
        res = DIB_LOAD_NO_MEM;
		AfxMessageBox( L"No Memory !" );
        goto cleanup;
    }
    for (i=Height - 1; i >= 0; i--)      /* Read each row (reverse) */
        for (j=0; j < Width; j++)   /* Read each column */
            ...
            {   // PPM 6
                ...
                {   // Can't read pixel
                    res = DIB_LOAD_NOFILE_OR_CORRUPT;
					AfxMessageBox( L"Can't read pixel" );
                    goto cleanup;
                }
                else
                {
...
BOOL CDIB::GaussianBlur( void )
{
	...
	if( !tAlphaImage )	
	{
        AfxMessageBox( L"[ CDIB::GaussianBlur() ] 메모리 할당에 실패 했습니다. [ %ld Bytes]", m_Size.cx * m_Size.cy  * sizeof( COLORREF ) );
		return FALSE;
    }
