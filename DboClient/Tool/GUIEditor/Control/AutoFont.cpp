...
CString CAutoFont::SetFaceName(CString facename)
{
	...
	DeleteObject();
#if defined(_MSC_VER) && (_MSC_VER >= 1400)
	wcscpy_s( lf.lfFaceName, LF_FACESIZE, (LPCTSTR)facename );
#else
...
{
	lf.lfHeight=_wtol((LPCTSTR)GetToken(str, L","));
	lf.lfWidth=_wtol((LPCTSTR)GetToken(str, L","));
	lf.lfEscapement=_wtol((LPCTSTR)GetToken(str, L","));
	lf.lfOrientation=_wtol((LPCTSTR)GetToken(str, L","));
	lf.lfWeight=_wtol((LPCTSTR)GetToken(str, L","));
	lf.lfItalic= _wtoi((LPCTSTR)GetToken(str, L","));
	lf.lfUnderline= _wtoi((LPCTSTR)GetToken(str, L","));
	lf.lfStrikeOut= _wtoi((LPCTSTR)GetToken(str, L","));
	lf.lfCharSet= _wtoi((LPCTSTR)GetToken(str, L","));
	lf.lfOutPrecision= _wtoi((LPCTSTR)GetToken(str, L","));
	lf.lfClipPrecision= _wtoi((LPCTSTR)GetToken(str, L","));
	lf.lfQuality= _wtoi((LPCTSTR)GetToken(str, L","));
	lf.lfPitchAndFamily= _wtoi((LPCTSTR)GetToken(str, L","));
#if defined(_MSC_VER) && (_MSC_VER >= 1400)
	wcscpy_s( lf.lfFaceName, LF_FACESIZE, (LPCTSTR)GetToken(str, L",") );
#else
