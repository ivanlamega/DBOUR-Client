...
IMPLEMENT_DYNAMIC(CPropTree, CWnd)
CPropTree::CPropTree() : 
	m_bShowInfo(TRUE),
	m_nInfoHeight(38),
	m_pVisbleList(NULL),
...
void CPropTree::InitGlobalResources()
{
	...
	wcscpy_s(lf.lfFaceName, LF_FACESIZE, info.lfMenuFont.lfFaceName);
#else
...
#if defined(_MSC_VER) && (_MSC_VER >= 1400)
			wcscpy_s(lf.lfFaceName, LF_FACESIZE, strOfficeFontName);
#else
...
#if defined(_MSC_VER) && (_MSC_VER >= 1400)
			wcscpy_s(lf.lfFaceName, LF_FACESIZE, strDefaultFontName);
#else
...
int	 CPropTree::GetListScrollPos( void )
{
	return m_List.GetScrollPos( SB_VERT );
}

void CPropTree::SetListScrollPos( int scrollpos )
{	
	int minpos, maxpos;
	m_List.GetScrollRange( SB_VERT, &minpos, &maxpos );
	if( minpos > scrollpos ) scrollpos = minpos;
	if( maxpos < scrollpos ) scrollpos = maxpos;	
	m_List.OnVScroll(SB_THUMBTRACK, scrollpos, NULL);
	...
	{
		SetFocus();
		m_List.OnVScroll(SB_THUMBTRACK, 0, NULL);	
	}
	else
	{
		SetFocus();
