...
{
	return  CWnd::Create(m_strMyClass, L"RollupCtrl", dwStyle, rect, pParentWnd, nID);
}
...
int CRollupCtrl::_InsertPage(LPCTSTR caption, CDialog* pwndTemplate, int idx, BOOL bAutoDestroyTpl)
{
	...
	groupbox->Create(L"", WS_CHILD|BS_GROUPBOX, r, this, 0 );
...
void CRollupCtrl::OnContextMenu(CWnd* pWnd, CPoint point) 
{
	...
	for (int i=0; i<m_PageList.GetSize(); i++)
	{
		...
		RECT btnrect;
		pi->pwndButton->GetWindowRect( &btnrect );

		if( point.x >= btnrect.left && point.x <= btnrect.right &&
			point.y >= btnrect.top  && point.y <= btnrect.bottom )
		{
			CMenu menu;
			if (menu.CreatePopupMenu())
			{
				menu.AppendMenu(MF_STRING,		RC_IDM_EXPANDALL,	L"Expand all"	);
				menu.AppendMenu(MF_STRING,		RC_IDM_COLLAPSEALL,	L"Collapse all"	);
