...
RwBool CEntityResource::Create( const CEntityProperty * pProperty )
{
	...
	switch( GetType() )
	{
		...
		/*	else
			{
				CNtlPLWorldProp *		pTerrainProperty = reinterpret_cast<CNtlPLWorldProp*>(CNtlPLPropertyContainer::GetInstance()->GetProperty(PLENTITY_WORLD, pWorldEntityProperty->GetName()->GetBuffer()));
