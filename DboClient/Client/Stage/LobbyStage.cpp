...
{	
	...
	else if( pMsg.Id == g_EventCharMake )
	{
		...
		CreateDummyAvatar( pCharMake->byRace, pCharMake->byClass, pCharMake->byGender, pCharMake->byFace, pCharMake->byHair, pCharMake->bySkinColor, pCharMake->byHairColor, pCharMake->fScale);
		TeleportStage();
	}
	else if( pMsg.Id == g_EventLobbyMessage )
	{
		...
		if(pLobbyMessage->byMessage == LMT_SELECT_CHARACTER)
		{
			...
			if( uiCharIndex == INVALID_INDEX ||
				uiCharIndex >= NTL_MAX_COUNT_USER_CHAR_SLOT ||
		...
		else if(pLobbyMessage->byMessage == LMT_ZOOM_IN)
		{
			...
			// 카메라 위치
			RwV3dSub(&v3Pos, &m_CameraData[m_byRace][m_byCharHeightType][ZL_ZOOM_IN].v3Pos,
				&m_CameraData[m_byRace][m_byCharHeightType][ZL_ZOOM_OUT].v3Pos);			

			RwV3dSub(&v3Dir, &m_v3Dir[m_byRace][m_byCharHeightType][ZL_ZOOM_IN],
...
void CLobbyStage::SetupDefaultCamerabyRace(RwUInt8 byRace)
{
	...
	if( RACE_COUNT >= m_byRace )
	{
		...
		// 카메라가 움직일 최대 거리
		RwV3dSub(&v3Temp, &m_CameraData[m_byRace][m_byCharHeightType][ZL_ZOOM_OUT].v3Pos,
		...
		// 카메라 시점이 움직일 최대 거리
		RwV3dSub(&v3Temp, &m_v3Dir[m_byRace][m_byCharHeightType][ZL_ZOOM_OUT],
...
void CLobbyStage::CreateBackGroundStage( RwUInt32 uiRace, RwBool bForce /* = FALSE */)
{
	...
	{
#ifdef MODSENABLED
		strcpy(dGET_WORLD_PARAM()->CurWorkingFolderName, "."); //Relative paths you fucking koreans
#else
...
void CLobbyStage::CreateDummyAvatar( BYTE byRace, BYTE byClass, BYTE byGender, BYTE byFace, BYTE byHair, BYTE bySkinColor, BYTE byHairColor, FLOAT fScale)
{
	...
	pAvatarInfo->sCharPf.sPcShape.fSize = fScale;
	...
	for(RwInt32 i = 0; i < NTL_MAX_NEWBIE_ITEM; i++)
	{
		...
		pAvatarInfo->sItemPf[pAvatarInfo->byItemCount].aOptionTblidx[0]		= INVALID_TBLIDX;
		pAvatarInfo->sItemPf[pAvatarInfo->byItemCount].aOptionTblidx[1]		= INVALID_TBLIDX;
...
void CLobbyStage::CreateDownLoadAvatar(sPC_SUMMARY *pCharInfo)
{
	...
	pAvatarInfo->sCharPf.sPcShape.fSize = pCharInfo->fScale;
	...
	for(RwInt32 i = 0; i < EQUIP_SLOT_TYPE_COUNT; i++)
	{
		...
		pAvatarInfo->sItemPf[pAvatarInfo->byItemCount].byPlace				= CONTAINER_TYPE_EQUIP;
		pAvatarInfo->sItemPf[pAvatarInfo->byItemCount].byPos				= (BYTE)i;
		pAvatarInfo->sItemPf[pAvatarInfo->byItemCount].byStackcount			= (BYTE)1;
		pAvatarInfo->sItemPf[pAvatarInfo->byItemCount].byRank				= pCharInfo->sItem[i].byRank;
		pAvatarInfo->sItemPf[pAvatarInfo->byItemCount].byCurDur;			// 여기는 채워 넣으시면 됩니다.
		pAvatarInfo->sItemPf[pAvatarInfo->byItemCount].aOptionTblidx[0]		= pCharInfo->sItem[i].aOptionTblidx[0];
		pAvatarInfo->sItemPf[pAvatarInfo->byItemCount].aOptionTblidx[1]		= pCharInfo->sItem[i].aOptionTblidx[1];
		pAvatarInfo->sItemPf[pAvatarInfo->byItemCount].bNeedToIdentify		= FALSE;
