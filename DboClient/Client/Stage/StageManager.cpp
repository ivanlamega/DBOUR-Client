...
CNtlStage* CStageManager::CreateStage(const char *pStageName)
{
	...
	if(strName == SECURITY_STAGE_NAME)
	{
		pStage = NTL_NEW CSecurityStage(pStageName);
	}
	else if(strName == WEB_LOGIN_STAGE_NAME)
	{
		pStage = NTL_NEW CWebLoginStage(pStageName);
	}
	else if(strName == LOGIN_STAGE_NAME)
	{
	...
	else if (strName == DBONU_STAGE_NAME)
	{
		pStage = NTL_NEW CDboNUStage(pStageName);
	}
