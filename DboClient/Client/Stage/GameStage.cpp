...
bool CGameStage::Create(void)
{
	...
	CDboGlobal::GetInstance()->UpdateGameStage(CDboGlobal::GetInstance()->m_pDiscordPresence, pLOBBY_CHARACTER->tSummary.awchName, (ePC_CLASS)pLOBBY_CHARACTER->tSummary.byClass);
...
void CGameStage::Destroy(void)
{
	...
	CNtlCharsetPrint::GetInstance()->Release(m_hCharsetAvatarAspect);
...
{
	// 자동으로 위치 지정이 되지 않을 경우를 대비하여 게임 시작시 한번 초기 위치를 정리한다
...
void CGameStage::UpdateGameEnterLoadingSchedulingWorld(RwReal fElapsed)
{
	...
	// 아직 world와 thread loading이 끝나지 않았다
...
void CGameStage::UpdateTeleportLoadingReadyScene(RwReal fElapsed)
{
	...
	if(m_bWorldChange)
	{
		...
		if(m_pWorldEntity)
		{
			...
			//m_pWorldEntity->GetWorld()->GetAvailablePos(m_AvatarPos)
			m_pWorldEntity->Update(0.01f);
			//LAST THING YOU DID DONT FORGET SANGAWKU
			SetLoadObjectSeamlessScheduling( TRUE );
		}
	}
	else
...
void CGameStage::UpdateDebugDisplay(RwReal fElapsed)
{
	...
	if(GetDboGlobal()->IsDebugDisplay())
	{
        ...
        CNtlSLEventGenerator::SobGetState(m_pAvatar->GetSerialID(), byServerState, byClientState);
		
        const char *pStateString = NtlGetCharStateString(byServerState);
		CNtlFSMLayer* fsmLayer = GetNtlSLGlobal()->GetSobAvatar()->GetFSMLayer();
		const char *pClientStateString = fsmLayer->GetCurrentState()->GetStateName();
        if(pStateString)
        {
            CNtlCharsetPrint::GetInstance()->Format(m_hCharsetAvatarState, "Avatar Server State : (%d, %s), Avatar Client State : (%d, %s)", byServerState, pStateString, byClientState, pClientStateString);  
        }
        else
        {
            CNtlCharsetPrint::GetInstance()->Format(m_hCharsetAvatarState, "Avatar Server State : (%d, NONE)", byServerState);  
        }
		const char *pAspectStateString = NtlGetAspectStateString(GetNtlSLGlobal()->GetSobAvatar()->GetAspectState()->sAspectStateBase.byAspectStateId);
		CNtlCharsetPrint::GetInstance()->Format(m_hCharsetAvatarAspect, "Avatar Aspect State : (%d, %s)", GetNtlSLGlobal()->GetSobAvatar()->GetAspectState()->sAspectStateBase.byAspectStateId, pAspectStateString);
		...
		if(hTargetSerial != INVALID_SERIAL_ID)
		{
			...
			if(pSobObj)
			{
				...
				if(pStateString)
				{
					CNtlCharsetPrint::GetInstance()->Format(m_hCharsetTargetState, "Target Server State : (%d, %s), Target Client State : (%d)", byServerState, pStateString, byClientState);  
				}
				else
				{
...
{
	// gui create
	// 항상 avatar 생성보다 먼저 생성되어야 한다.
	// avatar 생성될 때 이벤트가 호출된다.
	...
	m_hCharsetAvatarAspect = CNtlCharsetPrint::GetInstance()->Acquire();
...
void CGameStage::PostMutiThreadLoading( void )
{
	...
	// thread loading 때문에
	// 로딩이 완료된 시점에서 게임 그래픽 데이터를 렌더링 할 수
	// 있도록 복구한다
	RwFrameListSetAutoUpdate(TRUE);
	RpWorldAddCamera(CNtlPLGlobal::m_pRpWorld, CNtlPLGlobal::m_RwCamera);
...
void CGameStage::AvatarCreate(void)
{
	...
	m_hMouseDown	= CInputHandler::GetInstance()->LinkMouseDown(this, &CGameStage::MouseDownHandler);
	m_hMouseUp		= CInputHandler::GetInstance()->LinkMouseUp(this, &CGameStage::MouseUpHandler);
	m_hMouseMove	= CInputHandler::GetInstance()->LinkMouseMove(this, &CGameStage::MouseMoveHandler);
}
...
int	CGameStage::MouseDownHandler(unsigned int uiMouseData)
{
	NTL_FUNCTION("CGameStage::MouseDownHandler");
...
int CGameStage::MouseUpHandler(unsigned int uiMouseData)
{
	NTL_FUNCTION("CGameStage::MouseUpHandler");
	
	NTL_RETURN(1);
}

int CGameStage::MouseMoveHandler(unsigned int uiMouseData)
{
	NTL_FUNCTION("CGameStage::MouseMoveHandler");
