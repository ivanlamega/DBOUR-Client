...
void CCharStageState::CreateInstance()
{
	...
	CDboGlobal::GetInstance()->UpdateCharStage(CDboGlobal::GetInstance()->m_pDiscordPresence);
}
...
void CCharStageState::FuncUpdateReturnLogin(RwReal fElapsed)
{
	...
	case eDBO_SERVICE_PUBLISHER_DBONU: //COME BACK AND FIX THIS DISCONNECT ISSUE -SANGAWKU Todo
		{
			CDboEventGenerator::StageDelete(CHARACTER_STAGE_NAME);
