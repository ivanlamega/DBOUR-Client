...
RwBool CQuestIndicatorGui::Create( CQuestGui* pQuestGui )
{
	...
	for( RwInt32 i = 0 ; i < QUEST_MAX_INDICATOR_COUNT ; ++i )
	{
		...
		m_IndicatorItem[i].m_pTime->CreateFontStd( "Default", DEFAULT_FONT_SIZE, DEFAULT_FONT_ATTR );
		...
		for( RwInt32 j = 0 ; j < QUEST_MAX_GOAL_COUNT ; ++j )
		{
			...
			m_IndicatorItem[i].m_pGoalInfo[j]->CreateFontStd( "Default", DEFAULT_FONT_SIZE, DEFAULT_FONT_ATTR );
			...
			m_IndicatorItem[i].m_pGoalResult[j]->CreateFontStd( "Default", DEFAULT_FONT_SIZE, DEFAULT_FONT_ATTR );
