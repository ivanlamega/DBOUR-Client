...
RwBool CMainMenuGui::Create(VOID)
{
	...
	m_pbtnWhosOnline = (gui::CButton*)GetComponent("btnWhosOnline");
	...
	m_slotClickedWhosOnline = m_pbtnWhosOnline->SigClicked().Connect(this, &CMainMenuGui::OnClickedWhosOnline); //Edit Main menu for this. 
	...
	m_pbtnStatus->SetText( std::wstring( GetDisplayStringManager()->GetString( DST_MAINMENU_STATUS ) ) );
	m_pBtnInven->SetText( std::wstring( GetDisplayStringManager()->GetString( DST_MAINMENU_INVEN ) ) );
	m_pbtnQuest->SetText( std::wstring( GetDisplayStringManager()->GetString( DST_MAINMENU_QUEST ) ) );
	m_pbtnSystem->SetText( std::wstring( GetDisplayStringManager()->GetString( DST_MAINMENU_OPTION ) ) );
	m_pbtnCommunity->SetText( std::wstring( GetDisplayStringManager()->GetString( DST_MAINMENU_COMMUNITY ) ) );
	m_pbtnWhosOnline->SetText(std::wstring(L"Who's Online")); //Come back to this later - SanGawku
	m_pbtnRankBoard->SetText( std::wstring( GetDisplayStringManager()->GetString( DST_MAINMENU_RANKBOARD) ) );
	m_pbtnFriendList->SetText( std::wstring( GetDisplayStringManager()->GetString( DST_MAINMENU_FRIENDLIST ) ) );
	m_pbtnSkill->SetText( std::wstring( GetDisplayStringManager()->GetString( DST_MAINMENU_SKILL ) ) );
	m_pbtnChannelChange->SetText( std::wstring( GetDisplayStringManager()->GetString( DST_MAINMENU_CHANNEL_CHANGE ) ) );
	m_pbtnLogin->SetText( std::wstring( GetDisplayStringManager()->GetString( DST_MAINMENU_END_SESSION ) ) );
	m_pbtnExit->SetText( std::wstring( GetDisplayStringManager()->GetString( DST_MAINMENU_END_GAME ) ) );
...
VOID CMainMenuGui::OnClickedWhosOnline(gui::CComponent * pComponent)
{
	if (GetDialogManager()->IsOpenDialog(DIALOG_WHOSONLINE))
	{
		if (!Logic_CanMouseInput_in_Tutorial(ETL_MOUSE_INPUT_TYPE_MAINMENU_WHOSONLINE_CLOSE))
			return;
	...
	{
		if (!Logic_CanMouseInput_in_Tutorial(ETL_MOUSE_INPUT_TYPE_MAINMENU_WHOSONLINE_OPEN))
			return;

		GetDialogManager()->OpenDialog(DIALOG_WHOSONLINE);
	}
