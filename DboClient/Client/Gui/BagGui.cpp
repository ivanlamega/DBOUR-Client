...
VOID CBagBaseGui::RightButtonProc( RwInt32 nSlotIdx )
{
	...
	// 주의 : 미확인 아이템은 클라이언트에서 아이템 테이블을 알 수 없다
	sITEM_TBLDAT* pITEM_TBLDAT = pItemAttr->GetItemTbl();

	// 사용기간이 만료된 아이템은 사용기간 연장 메시지박스를 띄운다.
	if( pItemAttr->IsExpired() )
	{
		CTextTable* pItemTextTable = API_GetTableContainer()->GetTextAllTable()->GetItemTbl();

		sMsgBoxData data;
		data.sItemDeleteInfo.hSerial		= hItemHandle;
		data.sItemDeleteInfo.ePlace			= PLACE_BAG;
		data.sItemDeleteInfo.nPlaceIdx		= pItem->GetParentItemSlotIdx();
		data.sItemDeleteInfo.nSlotIdx		= pItem->GetItemSlotIdx();
	...
	else if( GetDialogManager()->IsOpenDialog(DIALOG_GUILD_WAREHOUSEBAR) )
	{
		...
		//&& GetNtlWorldConcept()->IsActivePlayConcept(WORLD_PLAY_PRIVATESHOP) // 아직 상태 변환 안됨.
		//&& GetNtlWorldConcept()->GetWorldConceptController(WORLD_PLAY_PRIVATESHOP)->GetState() == PRIVATESHOP_CLOSE // 세부 스테이트 없음
		)		// 개인상점은 미확인 아이템도 등록 가능하다.
	{		
		CDboEventGenerator::RegPrivateShopItem(PLACE_BAG, m_cBagPos, nSlotIdx);
	}
	// Mail 창이 열려 있으면 메일 창으로 직행
	...
	else if( pITEM_TBLDAT &&
			 ITEM_TYPE_HOIPOIROCK == pITEM_TBLDAT->byItem_Type &&
			 GetDialogManager()->IsOpenDialog( DIALOG_DOJO_UPGRADE ) )
	{
		CDboEventGenerator::DirectMoveIcon( hItemHandle, PLACE_BAG, PLACE_DOJO_UPRAGE, nSlotIdx, pItemAttr->GetStackNum() );
	}
	else if( pITEM_TBLDAT &&
		ITEM_TYPE_DOGI == pITEM_TBLDAT->byItem_Type &&
		GetDialogManager()->IsOpenDialog( DIALOG_DOGI ) )
	{
		CDboEventGenerator::DirectMoveIcon( hItemHandle, PLACE_BAG, PLACE_DOGI, nSlotIdx, pItemAttr->GetStackNum() );
	}
	// 업그레이드창이 열려있으면 상황에 따라서 이동
	...
	else if( GetDialogManager()->IsOpenDialog( DIALOG_VEHICLE ) )
	{
		if( pITEM_TBLDAT )
		{
			...
			{
				GetAlarmManager()->AlarmMessage(GAME_VEHICLE_CANNOT_WHILE_DRIVING, TRUE);
			}
		}
		else
		{
			DBO_FAIL("Not exist item table of handle : " << hItemHandle);
		}
	}
	// 그 외 미확인 아이템은 해당사항 없음.
	...
	// 장착 아이템이면 장착, 가방 아이템은 가방슬롯에, 스카우터에 장착하는 아이템은 스카우터에
	else if( pItemAttr->IsEquipItem() || pItemAttr->IsBagItem() || pItemAttr->IsScouterPartItem() )
	{
		Logic_ItemDirectEquipProc( hItemHandle, PLACE_BAG, (RwUInt8)nSlotIdx );		
	}
	// Charm 아이템은 Charm 슬롯이 열려있다면
	else if( pItemAttr->GetItemTbl()->byItem_Type == ITEM_TYPE_CHARM )
	{
		if( GetSideDialogManager()->IsOpenDialog( SDIALOG_PARTY_CHRAM ) )
			CDboEventGenerator::DirectMoveIcon( hItemHandle, PLACE_BAG, PLACE_PARTYCHARM, nSlotIdx, pItemAttr->GetStackNum() );
	}
	// 사용 가능한 아이템은 사용
	else if( pItem->IsUsePossible() )
	{
		// 사용 가능한 아이템 중에서 ITEM_TYPE_CAPSULE인 경우 Lock을 걸어준다.
		// Lock 이 걸린 아이템 중에서 ItemUseRes에, 실패가 된다면 다시 Lock을 풀어준다.
		// Lock 이 풀리는 경우는 각자의 Contents에서 상황에 맞게 작성한다.
		// 이 Lock은 Client와 Server간의 동기를 하지 않는다.
		//
		// HOIPOIMIX MACHINE = Object가 소멸되었을 경우
		// VEHICLE = Vechcle Gui객체가 종료되었을 경우
		// by Kell(2009.2.9)
...
RwBool CBasicBagGui::Create(VOID)
{
	...
	if( !CNtlPLGui::Create( "", "gui\\Inventory.srf", "gui\\BasicBag.frm" ) )
		NTL_RETURN( FALSE );
	...
	m_pbtnSell = (gui::CButton*)GetComponent( "btnSell" );
	...
	m_slotClickSellBtn = m_pbtnSell->SigClicked().Connect( this, &CBasicBagGui::OnClickSellBtn );
	...
	//m_pbtnSell->SetToolTip( std::wstring( GetDisplayStringManager()->GetString( DST_DISCARD_BTN ) ) );
	m_pbtnSell->SetToolTip( GetDisplayStringManager()->GetString(DST_BAG_REMOTE_SELL) );
...
VOID CBasicBagGui::HandleEvents( RWS::CMsg& msg )
{
	...
	if( msg.Id == g_EventSobInfoUpdate )
	{
		...
		else if( pUpdate->uiUpdateType & EVENT_AIUT_ITEM )
		{
			SetDiscardButton();
		}		
	}
...
{
	m_pstbMoney->SetText( Logic_GetZenny() );
}
...
VOID CBasicBagGui::SetDiscardButton()
{
	if( Logic_ExistScouterPartsType(SCOUTER_PARTS_TRANSMITTER) )
	{
		m_pbtnDiscard->Show(false);
		m_pbtnSell->Show(true);
	}
	else
	{
		m_pbtnDiscard->Show(true);
		m_pbtnSell->Show(false);
	}
}
...
{
	Logic_ItemDropConfirmProc(false);
}

VOID CBasicBagGui::OnClickSellBtn( gui::CComponent* pComponent )
{
	Logic_ItemDropConfirmProc(true);
}
...
RwBool CBag4SpaceGui::Create(VOID)
{
	...
	if( !CNtlPLGui::Create( "", "gui\\Inventory.srf", "gui\\4Bag.frm" ) )
		NTL_RETURN( FALSE );
...
RwBool CBag8SpaceGui::Create(VOID)
{
	...
	if( !CNtlPLGui::Create( "", "gui\\Inventory.srf", "gui\\8Bag.frm" ) )
		NTL_RETURN( FALSE );
...
RwBool CBag12SpaceGui::Create(VOID)
{
	...
	if( !CNtlPLGui::Create( "", "gui\\Inventory.srf", "gui\\12Bag.frm" ) )
		NTL_RETURN( FALSE );
...
RwBool CBag16SpaceGui::Create(VOID)
{
	...
	if( !CNtlPLGui::Create( "", "gui\\Inventory.srf", "gui\\16Bag.frm" ) )
		NTL_RETURN( FALSE );
...
RwBool CNetpyStoreBagGui::Create(VOID)
{
	NTL_FUNCTION( "CNetpyStoreBagGui" );

	if( !CNtlPLGui::Create( "", "gui\\Inventory.srf", "gui\\NetpyBag.frm" ) )
		NTL_RETURN( FALSE );
	...
	m_slotMouseDown = m_pThis->SigMouseDown().Connect( this, &CNetpyStoreBagGui::OnMouseDown );
	m_slotMouseUp	= m_pThis->SigMouseUp().Connect( this, &CNetpyStoreBagGui::OnMouseUp );
	m_slotMouseMove	= m_pThis->SigMouseMove().Connect( this, &CNetpyStoreBagGui::OnMouseMove );
	m_slotMouseOut  = m_pThis->SigMouseLeave().Connect( this, &CNetpyStoreBagGui::OnMouseOut );
	m_slotPaint		= m_pThis->SigPaint().Connect( this, &CNetpyStoreBagGui::OnPaint );
	m_slotMove		= m_pThis->SigMove().Connect( this, &CNetpyStoreBagGui::OnMove );
	m_slotClikedExit= m_pCloseButton->SigClicked().Connect( this, &CNetpyStoreBagGui::OnExitButtonClick );

	m_nNumSlot = 16;
	if( !CBagBaseGui::Create() )
		NTL_RETURN( FALSE );

	CBag16SpaceGui::SetSlotRectHardCode();

	NTL_RETURN( TRUE );
}	

VOID CNetpyStoreBagGui::OnMouseDown( const CKey& key )
{
	MouseDown( key );
}

VOID CNetpyStoreBagGui::OnMouseUp( const CKey& key )
{
	MouseUp( key );
}

VOID CNetpyStoreBagGui::OnMouseOut( gui::CComponent* pComponent )
{
	MouseOut();
}

VOID CNetpyStoreBagGui::OnMouseMove( RwInt32 nKey, RwInt32 nXPos, RwInt32 nYPos )
{
	MouseMove( nKey, nXPos, nYPos );
}

VOID CNetpyStoreBagGui::OnPaint()
{
	Paint();
}

VOID CNetpyStoreBagGui::OnMove( RwInt32 nX, RwInt32 nY )
{
	Move( nX, nY );
}

VOID CNetpyStoreBagGui::OnExitButtonClick( gui::CComponent* pComponent )
{
	ExitButtonClick( pComponent );
}
...
RwBool CBag20SpaceGui::Create(VOID)
{
	...
	if (!CNtlPLGui::Create("", "gui\\Inventory.srf", "gui\\20Bag.frm"))
		NTL_RETURN(FALSE);
...
RwBool CBag24SpaceGui::Create(VOID)
{
	...
	if (!CNtlPLGui::Create("", "gui\\Inventory.srf", "gui\\24Bag.frm"))
		NTL_RETURN(FALSE);
...
RwBool CBag28SpaceGui::Create(VOID)
{
	...
	if (!CNtlPLGui::Create("", "gui\\Inventory.srf", "gui\\28Bag.frm"))
		NTL_RETURN(FALSE);
...
RwBool CBag32SpaceGui::Create(VOID)
{
	...
	if (!CNtlPLGui::Create("", "gui\\Inventory.srf", "gui\\32Bag.frm"))
		NTL_RETURN(FALSE);
		...
		if( m_hBagSerial[i] != hSerial )
		{
			...
			else
			{
				...
				case ITEM_BAG_SIZE_4_BY_4: 
					...
					else if( Logic_IsNetStore( i ) )
						m_pBag[i] = NTL_NEW CNetpyStoreBagGui;
					else 
