	...
	RwBool IsLearnable( CSkillGuiItem::TYPE eType, sTBLDAT* pItemTblData )
	{
		if( pItemTblData )
		{
			...
			case CSkillGuiItem::SKILL:
				if( SRQ_FLAG_CAN_LEARN_BY_SKILLMASTER == ExmineRQBySkill( pItemTblData, TRUE ) || SRQ_FLAG_CAN_LEARN_BY_SELF == ExmineRQBySkill(pItemTblData, TRUE))
					return TRUE;
				break;
