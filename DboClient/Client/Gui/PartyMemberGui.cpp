...
{	
	...
	m_pBuff->Create(CBuffDispGui::TYPE_2X8, CBuffDispGui::SMALL_ICONSIZE,
...
RwBool CPartyMemberGui::Create(SERIAL_HANDLE hSerial, WCHAR* pcText, RwUInt8 byClass)
{
	...
	if(!CNtlPLGui::Create("gui\\PartyInven.rsr", "gui\\PartyMember.srf", "gui\\PartyMember.frm"))
		NTL_RETURN(FALSE);
	...
	if( key.m_nID == UD_LEFT_BUTTON )
	{
		...
		if(pSobObj)
		{
			...
			if(m_hSerial != hTargetSerialId)
			{
				...
				GetDboGlobal()->SetTargetSerial(m_hSerial);
				CNtlSLEventGenerator::SobTargetSelect(m_hSerial);

				API_GetSLPacketGenerator()->SendCharTargetSelect(m_hSerial);
			}			
		}
        else    // 클라이언트에 객체가 없으면 CommuTaget으로 설정한다.
        {
            SERIAL_HANDLE hTargetSerialId = GetDboGlobal()->GetTargetSerial();
