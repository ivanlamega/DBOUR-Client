...
#define dDEFAULT_ALPHA					128
#define dMAX_ALPHA						(255 - dDEFAULT_ALPHA)
#define dMIN_ALPHA						(50)
...
RwBool CChatDisplayGui::Create(CChatGui* pChatGui)
{
	...
	Component_Locate();
	SetAlpha((RwUInt8)(dMIN_ALPHA * ((RwReal)m_AlphaInfo.byAlphaWeight * 0.01f)));

	for(RwInt8 i = 0 ; i < NUM_ALL_CHAT_TYPE ; ++i )
		m_EnableLogType[i] = TRUE;
...
VOID CChatDisplayGui::Update( RwReal fElapsed )
{
	...
	RwUInt32 iMinAlpha = (RwUInt8)(dMIN_ALPHA * ((RwReal)m_AlphaInfo.byAlphaWeight * 0.01f));
	RwUInt32 iMaxAlpha = (RwUInt8)(dMAX_ALPHA * ((RwReal)m_AlphaInfo.byAlphaWeight * 0.01f));
...
VOID CChatDisplayGui::AddText(RwUInt8 byChatType, const WCHAR* pwcText)
{
	...
	default:
		{
			DBO_FAIL("Wrong Chat type : " << byChatType);
...
{
	for( RwInt32 i = 0 ; i < m_pOutDisplay->GetVisibleLineCount() ; ++i ) //For Scrooling Stat screen - sangawku	
...
VOID CChatDisplayGui::OnCaptureWheelMove(RwInt32 iFlag, RwInt16 sDelta, CPos& pos)
{
	...
	// Enable 상태에서도 마우스 휠의 반응이 있어야 한다
