...
// avooo's comment : 실제 지도 비율로 아이콘을 조정하면 너무 작거나 크다. 임의로 조절한다.
#define dICON_RATE_0						1.6f
#define dICON_RATE_1						1.1f
#define dICON_RATE_2						1.0f
...
RwBool CMinimapGui::Create(VOID)
{
	...
	// 포포스톤
	m_surPopoStone.SetSurface( GetNtlGuiManager()->GetSurfaceManager()->GetSurface( "Minimap.srf", "srfPopoStone" ) );

	// 랜드마크 : LANDMARK_SPECIAL_NORMAL 경우는 미니맵에 표시하지 않기에 더미로 LANDMARK_SPECIAL_DUNGEON 경우의 서페이스를 넣어두었다
	m_surMarkLandMark[LANDMARK_SPECIAL_NORMAL].SetSurface( GetNtlGuiManager()->GetSurfaceManager()->GetSurface( "Minimap.srf", "srfLandMark_Dungeon" ) );
	m_surMarkLandMark[LANDMARK_SPECIAL_DUNGEON].SetSurface( GetNtlGuiManager()->GetSurfaceManager()->GetSurface( "Minimap.srf", "srfLandMark_Dungeon" ) );
	...
	for( ; it != pLandMarkTable->End() ; ++it )
	{
		...
		if( pLAND_MARK_TBLDAT->byLandmarkType == LANDMARK_SPECIAL_NORMAL )
			continue;

		sLANKMARK* pLandMark = NTL_NEW sLANKMARK;

		pLandMark->v2Pos.x				= pLAND_MARK_TBLDAT->LandmarkLoc.x;
		pLandMark->v2Pos.y				= pLAND_MARK_TBLDAT->LandmarkLoc.z;
		pLandMark->eLandMarkType		= (eLandMark_SpecialType)pLAND_MARK_TBLDAT->byLandmarkType;
		pLandMark->pwcName				= pLAND_MARK_TBLDAT->wszNameText;
		pLandMark->bShow				= false;

		m_listLandMark.push_back(pLandMark);
	}
...
VOID CMinimapGui::Update( RwReal fElapsed )
{
	...
	RwBool bExistFindDragonBallParts = TRUE;//Logic_ExistScouterPartsType(SCOUTER_PARTS_DRAGON_BALL_SEARCH);
	if( pSobGroup )
	{
		CNtlSobGroup::MapObject::iterator it;
		for( it = pSobGroup->GetObjects().begin() ; it != pSobGroup->GetObjects().end() ; ++it )
		{
			...
			monster.bInfoWindow		= true;
				...
				{
					if (bExistFindDragonBallParts && pMonsterAttr->IsHaveDragonBall())
	...
	if( pSobGroup )
	{
		...
		for( it = pSobGroup->GetObjects().begin(); it != pSobGroup->GetObjects().end() ; ++it )
		{
			...
			{
				if( NPC_JOB_TALKER == pNpcAttr->GetNpcTbl()->byJob )
					continue;
				...
				sNPC.ucJob			= pNpcAttr->GetNpcTbl()->byJob;
		...
		if( pEntityFastFinder )
		{
			...
			if( pWorldMng )
			{
				...
				if( pNPCSpawnTbl )
				{
					...
					// avooo's commnet : 이 부분 선처리된 파일에서 데이터를 찾는 것으로 수정 예정
					for each( sTSEntity* pTSEntity in FindEntityList )
					{
						...
						if( !bExist )
						{
							...
							if( pNPC_TBLDAT )
							{
								...
								if( NPC_JOB_TALKER == pNPC_TBLDAT->byJob )
									continue;
	...
	for( MAP_NEXTQUEST_ITER it = pOtherParam->GetNextQuestBegin() ; it != pOtherParam->GetNextQuestEnd(); ++it )
	{
		...
		if( SLNextQuest.eTargetType == eQMI_TARGET_TYPE_NPC )
		{
			...
			if( pNPC )
			{
				// 주변에 NPC가 있다면 Sob 객체로 부터 위치를 받아온다
				v3TempPos			= pNPC->GetPosition();
			}
			else
			{
				// Sob 객체가 없다면 ts 쪽에서 저장하고 있는 Spawn 테이블의 위치 데이터를 가지고 온다
	...
	if( m_MapItem.ReadyMakeTexture(fElapsed) )
	{
		if( MS_NORMAL == m_byMapStyle )
		{
			...
			for( i = 0 ; i < m_vecMob.size() ; ++i )
			{
				...
				if (monster.eMobMarkType == MMT_HAVE_DRAGONBALL)
					m_surMarkMob[monster.eMobMarkType].SetSize(14, 14);
			...
			for( i = 0 ; i < m_vecQuest.size() ; ++i )
			{
				...
				if( pOtherParam->IsExistNextQuest_of_NPC(pstQuest->pNPC_TBLDAT->tblidx) )
					continue;
			...
			for( i = 0 ; i < m_vectNextQuest.size() ; ++i )
			{
				...
				// avooo's comment : 바깥쪽을 가리키는 아이콘은 OnPaint() 함수에서 그린다
			}
...
VOID CMinimapGui::AdjustIconSize()
{
	...
	{
		m_surMarkMob[i].SetWidth((RwInt32)((RwReal)(pSurface->m_Original.rtRect.right - pSurface->m_Original.rtRect.left) * fRate));
		m_surMarkMob[i].SetHeight((RwInt32)((RwReal)(pSurface->m_Original.rtRect.bottom - pSurface->m_Original.rtRect.top) * fRate));
	}	
...
VOID CMinimapGui::OnPaint(VOID)
{
	if( MS_NORMAL == m_byMapStyle )
	{
		...
		for( i = 0 ; i < m_vectNextQuest.size() ; ++i )
		{
			...
			// avooo's commnet : 안쪽의 아이콘은 UpdateBeforeCamera()에서 그린다
		}
...
VOID CMinimapGui::OnMouseMove(RwInt32 nFlags, RwInt32 nX, RwInt32 nY)
{
	...
	for( i = 0 ; i < m_vecMob.size() ; ++i )
	{
		...
		{			
			...
			SMemoryScouterData* pScouterData = pScouterGroup->IsUseMonsterScouter( sMonster.pMOB_TBLDAT->dwMobGroup );

			if (pScouterData)//|| sMonster.pMOB_TBLDAT->bShow_Name )
	...
	iHalfWidth = ((RwInt32)((RwReal)m_surMarkNPC.GetWidth() / 2.f / fRate));
	iHalfHeight = ((RwInt32)((RwReal)m_surMarkNPC.GetHeight() / 2.f / fRate));
	...
	for( i = 0 ; i < m_vecNPC.size() ; ++i )
	{
		...
		{			
			...
			info.wcsString += L" / ";
			info.wcsString += Logic_GetNPCJobName(sNPC.ucJob);
	...
	for( i = 0 ; i < m_vecQuest.size() ; ++i )
	{
		...
		{			
			...
			info.wcsString += L" / ";
			info.wcsString += Logic_GetNPCJobName(sQuest.pNPC_TBLDAT->byJob);
	...
	for( i = 0 ; i < m_vectNextQuest.size() ; ++i )
	{
		...
		if( bInIcon )
		{
			...
			if( nextQuest.eTargetType == eQMI_TARGET_TYPE_NPC )
			{
				...
				info.wcsString += Logic_GetNPCJobName(pNPC_TBLDAT->byJob);
			}
			else
			{
