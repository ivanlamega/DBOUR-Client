...
RwBool COptionWindowGui::Create()
{
	...
	m_pTabButton->ChangeTabText(2, std::wstring(GetDisplayStringManager()->GetString(DST_OPTION_GMT_TAB)));
	...
	m_pGMTTab->Create( this );
	m_pGMTTab->Show( false );

	LinkMsg( g_EventActionMapClientNotify, 0 );
	LinkMsg( g_EventToggleFullScreen, 0 );
	LinkMsg( g_EventGmtInfo, 0 );
	LinkMsg( g_EventGmtUpdate, 0 );
	LinkMsg( g_EventGmtChanged, 0 );
	LinkMsg(g_EventMyNameplateChanged, 0);
	LinkMsg(g_EventOtherNameplateChanged, 0);
...
void COptionWindowGui::Destroy()
{
	...
	UnLinkMsg( g_EventGmtInfo );
	UnLinkMsg( g_EventGmtUpdate );
	UnLinkMsg( g_EventGmtChanged );
	UnLinkMsg(g_EventMyNameplateChanged);
	UnLinkMsg(g_EventOtherNameplateChanged);
...
VOID COptionWindowGui::HandleEvents(RWS::CMsg &msg)
{
	...
	else if (msg.Id == g_EventGmtInfo ||
		msg.Id == g_EventGmtUpdate ||
		msg.Id == g_EventGmtChanged)
	{
		m_pGMTTab->OnHandleEvents(msg);
	}
...
void COptionWindowGui::OnTabChanged(RwInt32 nIndex, RwInt32 nOldIndex)
{
	...
	case GMT:
		{
			for( RwInt32 i = 0; i < OPTION_NUM; ++i )
			{
				m_apOptionTab[i]->Show(false);
				m_apOptionButton[i]->Show(false);
			}
			
			m_pGMTTab->Show( true );
			m_ePresentOption = OPTION_NONE;
...
m_pScouterOn(NULL), m_pScouterOff(NULL), m_pMyNameOn(NULL), m_pMyNameOff(NULL),
m_pOtherNameOn(NULL), m_pOtherNameOff(NULL)
{
}
...
RwBool COptionInfo::Create(COptionWindowGui* pOptionWindow)
{
	...
	SetComponentGroup(2, GetDisplayStringManager()->GetString(DST_OPTION_INFO_SCOUTERMODEL),
	...
	m_pMyNameOn = (gui::CButton*)GetComponent("btnInfoMyNameOn");
	m_pMyNameOff = (gui::CButton*)GetComponent("btnInfoMyNameOff");
	SetComponentGroup(3, L"Char Nameplate",L"Turns off the nameplate for yourself",	
		GetComponent("stbMiddle4"), m_pMyNameOn, m_pMyNameOff, NULL);
	SetComponentOnOff(&m_onoffMyName, m_pMyNameOn, m_pMyNameOff);

	m_pOtherNameOn = (gui::CButton*)GetComponent("btnInfoOtherNameOn");
	m_pOtherNameOff = (gui::CButton*)GetComponent("btnInfoOtherNameOff");
	SetComponentGroup(4, L"Other Nameplates",L"Turns Off Nameplates for Other Characters",
		GetComponent("stbMiddle5"), m_pOtherNameOn, m_pOtherNameOff, NULL);
	SetComponentOnOff(&m_onoffOtherName, m_pOtherNameOn, m_pOtherNameOff);
...
void COptionInfo::OnInit()
{
	...
	SetOnOff(&m_onoffMyName, GetMyNameplateOnOff());
	SetOnOff(&m_onoffOtherName, GetOtherNameplateOnOff());
}

void COptionInfo::OnReset()
{
	SetOnOff(&m_onoffGuild, GetNtlStorageMTContainer()->GetDefaultBool(dSTORAGE_GAMEINFO_GUILD_EMBLEM));
	SetOnOff(&m_onoffDojo, GetNtlStorageMTContainer()->GetDefaultBool(dSTORAGE_GAMEINFO_DOJO_MARK));
	SetOnOff(&m_onoffScouter, GetNtlStorageMTContainer()->GetDefaultBool( dSTORAGE_GAMEINFO_SCOUTER_VISIBLE ));
	SetOnOff(&m_onoffMyName, GetNtlStorageMTContainer()->GetDefaultBool(dSTORAGE_GAMEINFO_MY_NAMEPLATE));
	SetOnOff(&m_onoffOtherName, GetNtlStorageMTContainer()->GetDefaultBool(dSTORAGE_GAMEINFO_OTHER_NAMEPLATE));
}
...
void COptionInfo::OnOk()
{
	...
	SetMyNameplateOnOff(m_onoffMyName.bValue);
	SetOtherNameplateOnOff(m_onoffOtherName.bValue);

	CNtlSLEventGenerator::MyNameplateMode(GetNtlSLGlobal()->GetSobAvatar()->GetSerialID());
}
...
void COptionInfo::SetMyNameplateOnOff(RwBool bOnOff)
{
	GetNtlStorageManager()->SetData(dSTORAGE_GAMEINFO_MY_NAMEPLATE, B2b(bOnOff));
}

RwBool COptionInfo::GetMyNameplateOnOff()
{
	return GetNtlStorageManager()->GetBoolData(dSTORAGE_GAMEINFO_MY_NAMEPLATE);
}

void COptionInfo::SetOtherNameplateOnOff(RwBool bOnOff)
{
	GetNtlStorageManager()->SetData(dSTORAGE_GAMEINFO_OTHER_NAMEPLATE, B2b(bOnOff));
}

RwBool COptionInfo::GetOtherNameplateOnOff()
{
	return GetNtlStorageManager()->GetBoolData(dSTORAGE_GAMEINFO_OTHER_NAMEPLATE);
}
...
RwBool COptionETC::Create(COptionWindowGui* pOptionWindow)
{
	...
	SetComponentGroup(3, GetDisplayStringManager()->GetString( DST_OPTION_ETC_EX_QUICKSLOT1 ),
		GetDisplayStringManager()->GetString(DST_OPTION_TOOLTIP_EX_QUICKSLOT1 ),
		GetComponent("stbMiddle4"), m_pExQuickSlot1On, m_pExQuickSlot1Off, NULL );
	...
	SetComponentGroup(4, GetDisplayStringManager()->GetString( DST_OPTION_ETC_EX_QUICKSLOT2 ),
		GetDisplayStringManager()->GetString(DST_OPTION_TOOLTIP_EX_QUICKSLOT2 ),
		GetComponent("stbMiddle5"), m_pExQuickSlot2On, m_pExQuickSlot2Off, NULL );
	...
	SetComponentGroup(5, GetDisplayStringManager()->GetString( DST_OPTION_ETC_EX_QUICKSLOT_LOCK ),
		GetDisplayStringManager()->GetString(DST_OPTION_TOOLTIP_QUICKSLOT_LOCK ),
		GetComponent("stbMiddle6"), m_pExQuickSlotLockOn, m_pExQuickSlotLockOff, NULL );
