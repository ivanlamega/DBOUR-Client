...
const WCHAR* CStatusAvatarTab::GetEquipSlotText( RwInt32 nSlotIdx )
{
	...
	case EQUIP_SLOT_TYPE_QUEST: nTextID = DST_EQUIP_SLOT_TYPE_QUEST; break;
	...
	case EQUIP_SLOT_TYPE_DOGI:	nTextID = DST_EQUIP_SLOT_TYPE_DOGI; break;
	case EQUIP_SLOT_TYPE_HAIR:	nTextID = DST_EQUIP_SLOT_TYPE_HAIR; break;
	case EQUIP_SLOT_TYPE_ACCESSORY_1:nTextID = DST_EQUIP_SLOT_TYPE_ACCESSORY_1; break;
	case EQUIP_SLOT_TYPE_ACCESSORY_2:nTextID = DST_EQUIP_SLOT_TYPE_ACCESSORY_2; break;
	case EQUIP_SLOT_TYPE_ACCESSORY_3: nTextID = DST_EQUIP_SLOT_TYPE_ACCESSORY_3; break;
	}
...
VOID CStatusAvatarTab::SetSlotRectHardCode(VOID)
{
	...
	m_rtEquipSlot[EQUIP_SLOT_TYPE_QUEST].SetRectWH( 118, 273, NTL_ITEM_ICON_SIZE, NTL_ITEM_ICON_SIZE ); //Sangawku
	...
	m_rtEquipSlot[EQUIP_SLOT_TYPE_RING_1].SetRectWH(20, 231, NTL_ITEM_ICON_SIZE, NTL_ITEM_ICON_SIZE);//These werent here? Weird - Sangawku
	m_rtEquipSlot[EQUIP_SLOT_TYPE_RING_2].SetRectWH(20, 273, NTL_ITEM_ICON_SIZE, NTL_ITEM_ICON_SIZE);// ^this
	//New TW Interface Items - Sangawku
	m_rtEquipSlot[EQUIP_SLOT_TYPE_DOGI].SetRectWH(207, 105, NTL_ITEM_ICON_SIZE, NTL_ITEM_ICON_SIZE);
	m_rtEquipSlot[EQUIP_SLOT_TYPE_HAIR].SetRectWH(207, 147, NTL_ITEM_ICON_SIZE, NTL_ITEM_ICON_SIZE);
	m_rtEquipSlot[EQUIP_SLOT_TYPE_ACCESSORY_1].SetRectWH(207, 189, NTL_ITEM_ICON_SIZE, NTL_ITEM_ICON_SIZE);
	m_rtEquipSlot[EQUIP_SLOT_TYPE_ACCESSORY_2].SetRectWH(207, 231, NTL_ITEM_ICON_SIZE, NTL_ITEM_ICON_SIZE);
	m_rtEquipSlot[EQUIP_SLOT_TYPE_ACCESSORY_3].SetRectWH(207, 273, NTL_ITEM_ICON_SIZE, NTL_ITEM_ICON_SIZE);
...
{
	const char* szFilename[] = { "stttSTR","stttDEX", "stttCON", "stttENG", "stttSOL", "stttFOC", "stttLPRegen",
	"stttLPSitRegen", "stttLPBattleRegen", "stttEPRegen", "stttEPSitRegen", "stttEPBattleRegen",
	"stttRPChargeSpd", "stttRPDim", "stttPhysOffCrit","stttPhysOff", "stttPhysDef", "stttEngCrit", "stttEngOff", "stttEngDef",
	"stttHitRate", "stttDodgeRate", "stttRunSpeed", "stttCurseSuccess","stttCurseTol","stttCTChangePer",
	"stttCoolChangePer","stttKeepPer","stttDOTPer",	"stttDOTAbs", "stttReqEPPer", "stttHonOff", "stttHonDef",
	"stttStrOff", "stttStrDef", "stttWldOff", "stttWldDef", "stttEleOff", "stttEleDef", "stttFunOff",
	"stttFunDef", "stttParTol", "stttTerTol", "stttConTol", "stttStoneTol", "stttCandyTol",
	"stttParKeepDown", "stttTerKeepDown", "stttConKeepDown", "stttStoneKeepDown", "stttCandyKeepDown",
	"stttBleedKeepDown", "stttPoisonKeepDown", "stttStomKeepDown", "stttCrtBlkRate", "stttGrdRate", "stttSkillDamBlock",
	"stttCurseBlockRate", "stttKDBlockRate", "stttHTBBlockRate", "stttSitLPBonus", "stttSitEPBonus",
	"stttPhysCritBonus", "stttEnergyCritBonus", "stttItemUpBonus", "stttItemBreakBonus" };


	m_pSelf = reinterpret_cast<gui::CDialog*>(pParent->GetComponent("dlgStatus"));
	m_dialogStats = reinterpret_cast<gui::CDialog*>(pParent->GetComponent("dlgStatus2"));
	for (RwInt32 i = 0; i < 66; ++i)
	{
		char buf[256];
		sprintf_s(buf, 256, "%s%s", szFilename[i], "Title");

		m_pstbAttributes[i] = reinterpret_cast<gui::CStaticBox*>(pParent->GetComponent(szFilename[i]));
		m_pstbAttributesTitle[i] = reinterpret_cast<gui::CStaticBox*>(pParent->GetComponent(buf));
	}
	m_pScrollBar = (gui::CScrollBar*)pParent->GetComponent("scbScroll");
	m_pScrollBar->SetRange(0, m_dialogStats->GetHeight() - m_pSelf->GetHeight() );
	m_pScrollBar->SetValue(0);

	m_slotScrollChange = m_pScrollBar->SigValueChanged().Connect(this, &CStatusStatsTab::OnScrollChanged);
	m_slotSliderMove = m_pScrollBar->SigSliderMoved().Connect(this, &CStatusStatsTab::OnScrollChanged);
	m_slotCaptureWheelMove = GetNtlGuiManager()->GetGuiManager()->SigCaptureWheelMove().Connect(this, &CStatusStatsTab::OnCaptureWheelMove);
...
VOID CStatusStatsTab::UpdateUI(VOID)
{
	for (RwInt32 i = 0; i < 66; ++i)
	{
		m_pstbAttributesTitle[i]->SetText(GetDisplayStringManager()->GetString(DST_STATUS_STAT_STR + i));
	}
...
VOID CStatusStatsTab::Refresh(VOID)
{
	...
	// BasicStat
	uiColor = GetGuiFuntor()->ColorDecision(pAvatarAttr->m_byBaseStr, pAvatarAttr->m_byLastStr);
	m_pstbAttributes[0]->SetTextColor(uiColor);
	m_pstbAttributes[0]->SetText(pAvatarAttr->m_byLastStr);
	uiColor = GetGuiFuntor()->ColorDecision(pAvatarAttr->m_byBaseDex, pAvatarAttr->m_byLastDex);
	m_pstbAttributes[1]->SetTextColor(uiColor);
	m_pstbAttributes[1]->SetText(pAvatarAttr->m_byLastDex);
	uiColor = GetGuiFuntor()->ColorDecision(pAvatarAttr->m_byBaseCon, pAvatarAttr->m_byLastCon);
	m_pstbAttributes[2]->SetTextColor(uiColor);
	m_pstbAttributes[2]->SetText(pAvatarAttr->m_byLastCon);
	uiColor = GetGuiFuntor()->ColorDecision(pAvatarAttr->m_byBaseEng, pAvatarAttr->m_byLastEng);
	m_pstbAttributes[3]->SetTextColor(uiColor);
	m_pstbAttributes[3]->SetText(pAvatarAttr->m_byLastEng);
	uiColor = GetGuiFuntor()->ColorDecision(pAvatarAttr->m_byBaseSol, pAvatarAttr->m_byLastSol);
	m_pstbAttributes[4]->SetTextColor(uiColor);
	m_pstbAttributes[4]->SetText(pAvatarAttr->m_byLastSol);
	uiColor = GetGuiFuntor()->ColorDecision(pAvatarAttr->m_byBaseFoc, pAvatarAttr->m_byLastFoc);
	m_pstbAttributes[5]->SetTextColor(uiColor);
	m_pstbAttributes[5]->SetText(pAvatarAttr->m_byLastFoc);
	uiColor = GetGuiFuntor()->ColorDecision(pAvatarInfo->sCharPf.avatarAttribute.wBaseLpRegen, pAvatarInfo->sCharPf.avatarAttribute.wLastLpRegen);
	m_pstbAttributes[6]->SetTextColor(uiColor);
	m_pstbAttributes[6]->SetText(pAvatarInfo->sCharPf.avatarAttribute.wLastLpRegen);
	uiColor = GetGuiFuntor()->ColorDecision(pAvatarInfo->sCharPf.avatarAttribute.wBaseLpSitdownRegen, pAvatarInfo->sCharPf.avatarAttribute.wBaseLpSitdownRegen);
	m_pstbAttributes[7]->SetTextColor(uiColor);
	m_pstbAttributes[7]->SetText(pAvatarInfo->sCharPf.avatarAttribute.wBaseLpSitdownRegen);
	uiColor = GetGuiFuntor()->ColorDecision(pAvatarInfo->sCharPf.avatarAttribute.wBaseLpBattleRegen, pAvatarInfo->sCharPf.avatarAttribute.wBaseLpBattleRegen);
	m_pstbAttributes[8]->SetTextColor(uiColor);
	m_pstbAttributes[8]->SetText(pAvatarInfo->sCharPf.avatarAttribute.wBaseLpBattleRegen);
	uiColor = GetGuiFuntor()->ColorDecision(pAvatarInfo->sCharPf.avatarAttribute.wBaseEpRegen, pAvatarInfo->sCharPf.avatarAttribute.wLastEpRegen);
	m_pstbAttributes[9]->SetTextColor(uiColor);
	m_pstbAttributes[9]->SetText(pAvatarInfo->sCharPf.avatarAttribute.wLastEpRegen);
	uiColor = GetGuiFuntor()->ColorDecision(pAvatarInfo->sCharPf.avatarAttribute.wBaseEpSitdownRegen, pAvatarInfo->sCharPf.avatarAttribute.wBaseEpSitdownRegen);
	m_pstbAttributes[10]->SetTextColor(uiColor);
	m_pstbAttributes[10]->SetText(pAvatarInfo->sCharPf.avatarAttribute.wBaseEpSitdownRegen);
	uiColor = GetGuiFuntor()->ColorDecision(pAvatarInfo->sCharPf.avatarAttribute.wBaseEpBattleRegen, pAvatarInfo->sCharPf.avatarAttribute.wBaseEpBattleRegen);
	m_pstbAttributes[11]->SetTextColor(uiColor);
	m_pstbAttributes[11]->SetText(pAvatarInfo->sCharPf.avatarAttribute.wBaseEpBattleRegen);

	uiColor = GetGuiFuntor()->ColorDecision(pAvatarInfo->sCharPf.avatarAttribute.wBaseRpRegen, pAvatarInfo->sCharPf.avatarAttribute.wLastRpRegen);
	m_pstbAttributes[12]->SetTextColor(uiColor);
	m_pstbAttributes[12]->SetText(pAvatarInfo->sCharPf.avatarAttribute.wLastRpRegen);

	m_pstbAttributes[13]->SetText(pAvatarInfo->sCharPf.avatarAttribute.wLastRpDimimutionRate);
	//Critical needs to go here. 
	uiColor = GetGuiFuntor()->ColorDecision(pAvatarInfo->sCharPf.avatarAttribute.wBasePhysicalCriticalRate, pAvatarInfo->sCharPf.avatarAttribute.wLastPhysicalCriticalRate);
	m_pstbAttributes[14]->SetTextColor(uiColor);
	m_pstbAttributes[14]->SetText(pAvatarInfo->sCharPf.avatarAttribute.wLastPhysicalCriticalRate);

	uiColor = GetGuiFuntor()->ColorDecision(pAvatarInfo->sCharPf.avatarAttribute.wBasePhysicalOffence, pAvatarInfo->sCharPf.avatarAttribute.wLastPhysicalOffence);
	m_pstbAttributes[15]->SetTextColor(uiColor);
	m_pstbAttributes[15]->SetText(pAvatarInfo->sCharPf.avatarAttribute.wLastPhysicalOffence);
	uiColor = GetGuiFuntor()->ColorDecision(pAvatarInfo->sCharPf.avatarAttribute.wBasePhysicalDefence, pAvatarInfo->sCharPf.avatarAttribute.wLastPhysicalDefence);
	m_pstbAttributes[16]->SetTextColor(uiColor);
	m_pstbAttributes[16]->SetText(pAvatarInfo->sCharPf.avatarAttribute.wLastPhysicalDefence);
	
	uiColor = GetGuiFuntor()->ColorDecision(pAvatarInfo->sCharPf.avatarAttribute.wBaseEnergyCriticalRate, pAvatarInfo->sCharPf.avatarAttribute.wLastEnergyCriticalRate);
	m_pstbAttributes[17]->SetTextColor(uiColor);
	m_pstbAttributes[17]->SetText(pAvatarInfo->sCharPf.avatarAttribute.wLastEnergyCriticalRate);

	//Move down one
	uiColor = GetGuiFuntor()->ColorDecision(pAvatarInfo->sCharPf.avatarAttribute.wBaseEnergyOffence, pAvatarInfo->sCharPf.avatarAttribute.wLastEnergyOffence);
	m_pstbAttributes[18]->SetTextColor(uiColor);
	m_pstbAttributes[18]->SetText(pAvatarInfo->sCharPf.avatarAttribute.wLastEnergyOffence);
	uiColor = GetGuiFuntor()->ColorDecision(pAvatarInfo->sCharPf.avatarAttribute.wBaseEnergyDefence, pAvatarInfo->sCharPf.avatarAttribute.wLastEnergyDefence);
	m_pstbAttributes[19]->SetTextColor(uiColor);
	m_pstbAttributes[19]->SetText(pAvatarInfo->sCharPf.avatarAttribute.wLastEnergyDefence);
	//Attack Stuffs
	uiColor = GetGuiFuntor()->ColorDecision(pAvatarInfo->sCharPf.avatarAttribute.wBaseAttackRate, pAvatarInfo->sCharPf.avatarAttribute.wLastAttackRate);
	m_pstbAttributes[20]->SetTextColor(uiColor);
	m_pstbAttributes[20]->SetText(pAvatarInfo->sCharPf.avatarAttribute.wLastAttackRate);

	//Defenseive stuffs
	uiColor = GetGuiFuntor()->ColorDecision(pAvatarInfo->sCharPf.avatarAttribute.wBaseDodgeRate, pAvatarInfo->sCharPf.avatarAttribute.wLastDodgeRate);
	m_pstbAttributes[21]->SetTextColor(uiColor);
	m_pstbAttributes[21]->SetText(pAvatarInfo->sCharPf.avatarAttribute.wLastDodgeRate);

	uiColor = GetGuiFuntor()->ColorDecision(pAvatarInfo->sCharPf.avatarAttribute.wBaseBlockRate, pAvatarInfo->sCharPf.avatarAttribute.wLastBlockRate);
	m_pstbAttributes[22]->SetTextColor(uiColor);
	m_pstbAttributes[22]->SetText(pAvatarInfo->sCharPf.avatarAttribute.wLastBlockRate);

	m_pstbAttributes[23]->SetText(pAvatarInfo->sCharPf.avatarAttribute.fLastRunSpeed);

	uiColor = GetGuiFuntor()->ColorDecision(pAvatarInfo->sCharPf.avatarAttribute.wBaseCurseSuccessRate, pAvatarInfo->sCharPf.avatarAttribute.wLastCurseSuccessRate);
	m_pstbAttributes[24]->SetTextColor(uiColor);
	m_pstbAttributes[24]->SetText(pAvatarInfo->sCharPf.avatarAttribute.wLastCurseSuccessRate);

	uiColor = GetGuiFuntor()->ColorDecision(pAvatarInfo->sCharPf.avatarAttribute.wBaseCurseToleranceRate, pAvatarInfo->sCharPf.avatarAttribute.wLastCurseToleranceRate);
	m_pstbAttributes[25]->SetTextColor(uiColor);
	m_pstbAttributes[25]->SetText(pAvatarInfo->sCharPf.avatarAttribute.wLastCurseToleranceRate);

	m_pstbAttributes[26]->SetText(pAvatarInfo->sCharPf.avatarAttribute.fCastingTimeChangePercent);
	m_pstbAttributes[27]->SetText(pAvatarInfo->sCharPf.avatarAttribute.fCoolTimeChangePercent);
	m_pstbAttributes[28]->SetText(pAvatarInfo->sCharPf.avatarAttribute.fKeepTimeChangePercent);
	m_pstbAttributes[29]->SetText(pAvatarInfo->sCharPf.avatarAttribute.fDotValueChangePercent);
	m_pstbAttributes[30]->SetText(pAvatarInfo->sCharPf.avatarAttribute.fDotTimeChangeAbsolute);
	m_pstbAttributes[31]->SetText(pAvatarInfo->sCharPf.avatarAttribute.fRequiredEpChangePercent);

	m_pstbAttributes[32]->SetText(pAvatarInfo->sCharPf.avatarAttribute.fHonestOffence);
	m_pstbAttributes[33]->SetText(pAvatarInfo->sCharPf.avatarAttribute.fHonestDefence);

	m_pstbAttributes[34]->SetText(pAvatarInfo->sCharPf.avatarAttribute.fStrangeOffence);
	m_pstbAttributes[35]->SetText(pAvatarInfo->sCharPf.avatarAttribute.fStrangeDefence);

	m_pstbAttributes[36]->SetText(pAvatarInfo->sCharPf.avatarAttribute.fWildOffence);
	m_pstbAttributes[37]->SetText(pAvatarInfo->sCharPf.avatarAttribute.fWildDefence);

	m_pstbAttributes[38]->SetText(pAvatarInfo->sCharPf.avatarAttribute.fEleganceOffence);
	m_pstbAttributes[39]->SetText(pAvatarInfo->sCharPf.avatarAttribute.fEleganceDefence);

	m_pstbAttributes[40]->SetText(pAvatarInfo->sCharPf.avatarAttribute.fFunnyOffence);
	m_pstbAttributes[41]->SetText(pAvatarInfo->sCharPf.avatarAttribute.fFunnyDefence);

	m_pstbAttributes[42]->SetText(pAvatarInfo->sCharPf.avatarAttribute.wParalyzeToleranceRate);
	m_pstbAttributes[43]->SetText(pAvatarInfo->sCharPf.avatarAttribute.wTerrorToleranceRate);
	m_pstbAttributes[44]->SetText(pAvatarInfo->sCharPf.avatarAttribute.wStoneToleranceRate);
	m_pstbAttributes[45]->SetText(pAvatarInfo->sCharPf.avatarAttribute.wCandyToleranceRate);

	m_pstbAttributes[46]->SetText(pAvatarInfo->sCharPf.avatarAttribute.fParalyzeKeepTimeDown);
	m_pstbAttributes[47]->SetText(pAvatarInfo->sCharPf.avatarAttribute.fTerrorKeepTimeDown);
	m_pstbAttributes[48]->SetText(pAvatarInfo->sCharPf.avatarAttribute.fConfuseKeepTimeDown);
	m_pstbAttributes[49]->SetText(pAvatarInfo->sCharPf.avatarAttribute.fStoneKeepTimeDown);
	m_pstbAttributes[50]->SetText(pAvatarInfo->sCharPf.avatarAttribute.fCandyKeepTimeDown);
	m_pstbAttributes[51]->SetText(pAvatarInfo->sCharPf.avatarAttribute.fBleedingKeepTimeDown);
	m_pstbAttributes[52]->SetText(pAvatarInfo->sCharPf.avatarAttribute.fPoisonKeepTimeDown);
	m_pstbAttributes[53]->SetText(pAvatarInfo->sCharPf.avatarAttribute.fStomachacheKeepTimeDown);
	m_pstbAttributes[54]->SetText(pAvatarInfo->sCharPf.avatarAttribute.fCriticalBlockSuccessRate);

	m_pstbAttributes[55]->SetText(pAvatarInfo->sCharPf.avatarAttribute.wGuardRate);
	m_pstbAttributes[56]->SetText(pAvatarInfo->sCharPf.avatarAttribute.fSkillDamageBlockModeSuccessRate);
	m_pstbAttributes[57]->SetText(pAvatarInfo->sCharPf.avatarAttribute.fCurseBlockModeSuccessRate);
	m_pstbAttributes[58]->SetText(pAvatarInfo->sCharPf.avatarAttribute.fKnockdownBlockModeSuccessRate);
	m_pstbAttributes[59]->SetText(pAvatarInfo->sCharPf.avatarAttribute.fHtbBlockModeSuccessRate);
	m_pstbAttributes[60]->SetText(pAvatarInfo->sCharPf.avatarAttribute.fSitDownLpRegenBonusRate);
	m_pstbAttributes[61]->SetText(pAvatarInfo->sCharPf.avatarAttribute.fSitDownEpRegenBonusRate);
	m_pstbAttributes[62]->SetText(pAvatarInfo->sCharPf.avatarAttribute.fPhysicalCriticalDamageBonusRate);
	m_pstbAttributes[63]->SetText(pAvatarInfo->sCharPf.avatarAttribute.fEnergyCriticalDamageBonusRate);

	m_pstbAttributes[64]->SetText(pAvatarInfo->sCharPf.avatarAttribute.fItemUpgradeBonusRate);
	m_pstbAttributes[65]->SetText(pAvatarInfo->sCharPf.avatarAttribute.fItemUpgradeBreakBonusRate);
}
...
{
	CRectangle rtPos = m_dialogStats->GetPosition();

	m_dialogStats->SetPosition(rtPos.left, -nNewOffset);
}

VOID CStatusStatsTab::OnCaptureWheelMove(RwInt32 nFlag, RwInt16 sDelta, CPos & pos)
{
	if (!m_dialogStats->IsVisible())
		return;

	if (m_dialogStats->GetParent()->GetChildComponentReverseAt(pos.x - m_dialogStats->GetParent()->GetScreenRect().left, pos.y - m_dialogStats->GetParent()->GetScreenRect().top) != m_dialogStats)
		return;

	if (m_dialogStats->PosInRect(pos.x, pos.y) != gui::CComponent::INRECT)
		return;
...
RwBool CStatusWindowGui::Create(VOID)
{
	...
	//New Gui for All Important Stats
...
VOID CStatusWindowGui::SetBasicUISetting(VOID)
{
	...
	m_ptabStatus->AddTab(std::wstring(GetDisplayStringManager()->GetString(DST_STATUS_DETAIL_STATS)));
	m_ptabStatus->SelectTab( (RwInt32)TAB_AVATAR );		
	SelectTab( TAB_AVATAR );	
}
