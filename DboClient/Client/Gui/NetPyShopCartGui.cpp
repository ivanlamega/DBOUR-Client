...
RwBool CNetPyShopCartGui::Create()
{
	...
	m_slotPostPaint		= m_pThis->SigPaint().Connect( this, &CNetPyShopCartGui::OnPostPaint );
...
VOID CNetPyShopCartGui::ClickedBuyButton(gui::CComponent* pComponent)
{
	...
	for( RwInt32 i = 0 ; i < MAX_SLOT ; ++i )
	{
		if( m_BuySlotInfo[i].NPCShopBuyInfo.byItemPos != INVALID_BYTE )
		{
			...
			totalCost += m_BuySlotInfo[i].slot.GetPrice() * m_BuySlotInfo[i].NPCShopBuyInfo.byStack;
			byCount++;
		}
	}
	...
	// 사는 패킷 보내기
	GetDboGlobal()->GetGamePacketGenerator()->SendShopNetPyItemBuyReq( byCount, aBuyCart,  totalCost);	
}
...
VOID CNetPyShopCartGui::CalcTotalBuyPrice()
{
	...
	for( RwInt32 i = 0 ; i < MAX_SLOT ; ++i )
	{
		if( m_BuySlotInfo[i].slot.GetSerial() != INVALID_SERIAL_ID )
		{
			...
			iTotalPrice += m_BuySlotInfo[i].slot.GetPrice() * m_BuySlotInfo[i].NPCShopBuyInfo.byStack;
		}
	}		

	m_uiTotalBuyPrice = iTotalPrice;
	m_pTotalBuyMoney->SetText((RwInt32)m_uiTotalBuyPrice);
...
VOID CNetPyShopCartGui::OnPostPaint()
{
