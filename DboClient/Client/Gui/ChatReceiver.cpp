...
WCHAR* CChatReceiver::GetChatName(RwUInt8 byType)
{
	static WCHAR wchBuffer[30];
	memset(wchBuffer, 0, sizeof(wchBuffer));

	switch (byType)
	{
	case CHAT_TYPE_SYSTEM:
	case CHAT_TYPE_NOTIFY:
		swprintf_s(wchBuffer, 30, L"%s", L"System");
		break;
	case CHAT_TYPE_GENERAL:
		swprintf_s(wchBuffer, 30, L"%s",L"General");
		break;
	case CHAT_TYPE_GUILD:
		swprintf_s(wchBuffer, 30, L"%s", L"Guild");
		break;
	case CHAT_TYPE_SHOUT:
		swprintf_s(wchBuffer, 30, L"%s", L"Shout");
		break;
	case CHAT_TYPE_TRADE:
		swprintf_s(wchBuffer, 30, L"%s", L"Trade");
		break;
	case CHAT_TYPE_PARTY:
		swprintf_s(wchBuffer, 30, L"%s", L"Party");
		break;
	case CHAT_TYPE_WHISPER:
		swprintf_s(wchBuffer, 30, L"%s", L"Whisper");
		break;
	 default:
		swprintf_s(wchBuffer, 30, L"%s", L"Unknown");
		break;
	}
	return wchBuffer;
}
...
VOID CChatReceiver::HandleEvents( RWS::CMsg &msg )
{
	...
	if( msg.Id == g_EventSayMessage )
	{
		...
		{
			if (pSayMessage->bIsGameMaster)
			{
				swprintf_s(wchBuffer, constnBufLength, L"[%s][GM]%s : %s", GetChatName(pSayMessage->byMsgType), pSayMessage->wchUserName, pSayMessage->wchMessage);
			}
			else
			{
				swprintf_s(wchBuffer, constnBufLength, L"[%s] %s : %s", GetChatName(pSayMessage->byMsgType), pSayMessage->wchUserName, pSayMessage->wchMessage);
			}
			// 말풍선 띄우기
		...
		if( pSayMessage->uiSerial != Logic_GetAvatarHandle() &&
			...
			pSayMessage->byMsgType != CHAT_TYPE_SYSTEM)
