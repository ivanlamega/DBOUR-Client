...
RwBool CAccountGui::Create()
{
	...
	m_Background.SetSurface(0, GetNtlGuiManager()->GetSurfaceManager()->GetSurface( "CharSelect_Account.srf", "srfBackgroundLeft" ));
	m_Background.SetSurface(1, GetNtlGuiManager()->GetSurfaceManager()->GetSurface( "CharSelect_Account.srf", "srfBackgroundCenter" ));
	m_Background.SetSurface(2, GetNtlGuiManager()->GetSurfaceManager()->GetSurface( "CharSelect_Account.srf", "srfBackgroundRight" ));	
	m_Background.SetSize(233, 172);

	// title surface
	//m_srfTitle.SetSurface(GetNtlGuiManager()->GetSurfaceManager()->GetSurface( "CharSelect_Account.srf", "srfAccountTitle" ));
	...
	m_pTitle->SetText(GetDisplayStringManager()->GetString(DST_LOBBY_SERVER));
...
VOID CAccountGui::SetServerInfo()
{
	...
	if( pLobby->GetSelectedChannelIndex() == INVALID_BYTE )
	{
		m_pServerName->Format(L"%s", pLobby->GetServerName());
	}
	else
	{
		m_pServerName->Format(L"%s %d", pLobby->GetServerName(), pLobby->GetSelectedChannelIndex());
	}	
}
