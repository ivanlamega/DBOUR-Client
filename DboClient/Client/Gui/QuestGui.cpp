...
RwBool CQuestGui::Create(VOID)
{
	...
	LinkMsg( g_EventTMQBoardNotify, 0 );		  // QuestIndicator hide
...
VOID CQuestGui::HandleEvents( RWS::CMsg &msg )
{
	...
	else if( msg.Id == g_EventUpdateQuestState_Nfy )
	{
		...
		if( pQuestTitle )
			wstrQuestTitle = L"("+ std::to_wstring(pData->uiQuestTitle) + L") "+ pQuestTitle->wstrText;
		else
		{
			WCHAR wBuf[256];
	...
	else if( msg.Id == g_EventMsgBoxResult )
	{
		...
		if( pMsgBoxResult->uiWorkId == MBW_QUEST_GIVEUP )
		{
