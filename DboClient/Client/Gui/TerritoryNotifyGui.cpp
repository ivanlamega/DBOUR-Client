...
VOID CTerritoryNotifyGui::HandleEvents( RWS::CMsg& msg )
{
	if( msg.Id == NPEI_IS_MAPNAME_CHANGE )
	{
		...
		std::wstring discordZone, discordSector;
		...
		if (TRUE == pText->GetZoneName(tblidx, &discordZone))
		{
			if (TRUE == pText->GetSectorName(tblidx, &discordSector))
			{
				//if(CDboGlobal::GetInstance()->m_pDiscordPresence.state != NULL)
				DiscordRichPresence passMe = { 0 };
				GetDboGlobal()->UpdateGameStageLocation(passMe, discordZone, discordSector);
			}
		}
	}
	else if( msg.Id == g_EventSobServerUpdateState )
	{
		...
		if( pMOB_TBLDAT->byProperty != MOB_PROPERTY_DUNGEON_BOSS )
			return;
		...
		Logic_PlayGUISound(GSD_FINISH_BOSS_MOB);
