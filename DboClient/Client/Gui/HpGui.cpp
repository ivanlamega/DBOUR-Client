...
RwBool CHpGui::Create(void)
{
	...
	//Initalize the sword icon. You're welcome Atidot3
	m_ppnlBattleCombating = (gui::CPanel*)GetComponent("pnlBattleCombating");
	
	//Make sure sword icon doesn't show when you first load.
	m_ppnlBattleCombating->Show(false);

	//Add a tooltip so dumb people know what this is... 
	m_ppnlBattleCombating->SetToolTip("Fighting");
	...
	m_feBattleCombating.SetSurface(GetNtlGuiManager()->GetSurfaceManager()->GetSurface("StatusBar.srf", "srfBattleCombatingFlicker"));
	m_feBattleCombating.SetTime(0.0f, 0.5f);
	m_feBattleCombating.SetAlpha(0, 150);
...
VOID CHpGui::Update( RwReal fElapsed )
{
	...
	m_feBattleCombating.Update( fElapsed );
}
...
void CHpGui::HandleEvents(RWS::CMsg &pMsg)
{
	...
	{
		SNtlEventSobFightingMode* pData = reinterpret_cast<SNtlEventSobFightingMode*>(pMsg.pData);

		if (pData->hSerialId == GetNtlSLGlobal()->GetSobAvatar()->GetSerialID())
		{
			if (pData->bFightingMode)
			{

				if (!m_feBattleCombating.IsWork())
				{
					m_feBattleCombating.SetRenderTop(true);
					m_feBattleCombating.StartProc(TRUE);
					m_ppnlBattleCombating->Show();
...
VOID CHpGui::OnMove( RwInt32 nX, RwInt32 nY )
{
	...
	nCenterX = rect.left + ((rect.right - rect.left) / 2);
	nCenterY = rect.top + ((rect.bottom - rect.top) / 2);
	m_feBattleCombating.SetCenterPos(nCenterX, nCenterY);
