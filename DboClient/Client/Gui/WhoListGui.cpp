...
VOID CWhoListGui::HandleEvents( RWS::CMsg &pMsg ) 
{
    ...
    else if(pMsg.Id == g_EventMsgBoxResult)     // 친구 삭제에서 확인 버튼 클릭시
    {
        ...
        {
            if(pMsgBoxResult->uiWorkId == MBW_FRIENDLIST_DEL_ASK)       // 친구 삭제 요청
            {
                GetDboGlobal()->GetChatPacketGenerator()->SendFriend_Del_Req(m_uiCurrentCharID);
            }
            else if(pMsgBoxResult->uiWorkId == MBW_BLACKLIST_DEL_ASK)   // 블랙 리스트 삭제 요청
            {
                GetDboGlobal()->GetChatPacketGenerator()->SendFriend_Black_Del_Req(m_uiCurrentCharID);
            }
            else if(pMsgBoxResult->uiWorkId == MBW_FRIENDLIST_ADD_ASK)  // 친구 등록 요청
            {
            ...
            else if(pMsgBoxResult->uiWorkId == MBW_BLACKLIST_ADD_ASK)   // 블랙리스트 등록 요청
            {
...
/// 버튼 리스트의 버튼이 토글 상태가 변할때 호출되는 함수 
/// 토글시에 모두 선택해제 되었는지 확인하고, 토글상태된 버튼이 없으면 현재 선택을 INVALID로 한다.
