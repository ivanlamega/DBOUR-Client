...
VOID CDialogManager::Update(RwReal fElapsed)
{
	...
	// NPC창을 연 대상과 일정 거리 이상 멀어질 때 NPC 다이얼로그를 닫는다
...
VOID CDialogManager::OpenDefaultDialog()
{
	...
	// 열릴 수 있는 사이드 다이얼로그가 전부 열리고 사이드 다이얼로그 컨트롤로가
	// 열려야 한다
...
VOID CDialogManager::RaiseDialogbyUser(RwInt32 iDialog)
{
	...
	else
	{
		...
		{
			if( iDialog != DIALOG_MULTIDIALOG && iDialog != DIALOG_PORTAL )
				return;
		}			
	}
...
VOID CDialogManager::LocateBag(RwInt32 iScreenWidth, RwInt32 iScreenHeight, RwBool bForce /* = FALSE */)
{
	...
	for( RwInt32 i = DIALOG_CAPSULE_1 ; i <=  DIALOG_CAPSULE_NETPY ; ++i )
	{
		...
		if( byBagCount == 0 )
		{
			...
			iY = iScreenHeight - 105;
		}		

		iY = iY - pGui->GetHeight();
		...
		if( byBagCount == 1 )
		{
			iX -= 3 + pGui->GetWidth(); // 3: 가방의 좌우 간격
			iY = iScreenHeight - 105;
		}
		if (byBagCount == 3)
		{
...
VOID CDialogManager::LocationDialogs(RwInt32 iWidth, RwInt32 iHeight)
{
	...
	if( pGui )
		pGui->SetPosition(iWidth/2 - pGui->GetWidth()/2, dDIALOG_CLEINT_EDGE_GAP );	
	...
	pGui = GetDialog(DIALOG_SCOUTER_SLOT);
	if( pGui )
	{
		iX -= pGui->GetWidth() + 2;
		iY -= pGui->GetHeight();
	...
	pGui = GetDialog(DIALOG_SCOUTER_MENU);
	if( pGui )
		pGui->SetPosition(iWidth - 329, 278);
	...
	if( pGui )
		pGui->SetPosition(iWidth - 10 - pGui->GetWidth(), iY - pGui->GetHeight() );
	...
	pGui = GetDialog(DIALOG_SCOUTER_BACKGROUND);
	if( pGui )
		pGui->SetPosition(CRectangle(0, 0, iWidth, iHeight));
	...
	pGui = GetDialog( DIALOG_PRIVATESHOP_TRADEBOX );
	if(pGui)
		pGui->SetPosition( (iWidth/2) - (pGui->GetWidth()/2), (iHeight/2) - (pGui->GetHeight()/2) );
	...
	if( pGui )
	{		
		pGui->SetPosition((iWidth - pGui->GetWidth())/2, iHeight - 170);
	}	
    ...
    // 넷마블 PC방 로고
    pGui = GetDialog(DIALOG_NETMARBLEPCBANG);
    if(pGui)
    {
        pGui->SetPosition(iWidth - CNetMarblePCBangGui::E_GAME, CNetMarblePCBangGui::E_POSY);
    }
...
{	
	...
	// avooo's comment : 클라이언트 리사이즈시 화면상에 레귤러 다이얼로그가 열릴 수 있는
	// 제한 갯수를 넘으면 LocationDialogs() 함수에서 즉각적으로 적정수의
	// 레귤러의 다이얼로그가 닫히도록 한다.
	// 때문에 여기서는 열려있는 레귤러 다이얼로그가 화면상에 열릴 수 있는 적정수 일때
	// 하나의 레귤러 다이얼로그를 닫을 수만 있다면 새로운 다이얼로그를
	// 열 수 있다
