...
,m_pTMQSideIconGui(NULL)
,m_pTMQSideViewGui(NULL)
...
RwBool CSideIconGui::Create()
{
	...
	// TMQ
	RegisterSideIcon( m_pTMQSideIconGui, CTMQSideIconGui, "TMQSideIconGui", SIDEICON_TMQ );
	RegisterSideView( m_pTMQSideViewGui, CTMQSideViewGui, "TMQSideViewGui", SIDEVIEW_TMQ );
	...
	RegisterSideIcon( m_pSCSSideIconGui, CSCSSideIconGui, "CSCSSideIconGui", SIDEICON_DOJO );
	RegisterSideView( m_pSCSSideViewGui, CSCSSideViewGui, "CSCSSideViewGui", SIDEVIEW_DOJO );
	
    // NetPy
    // 실제 사용하기 전까지는 막아 놓는다. (by agebreak)
    if((GetServiceInterface()->GetPublisherID() == eDBO_SERVICE_PUBLISHER_CJI_KOR) || (GetServiceInterface()->GetPublisherID() == eDBO_SERVICE_PUBLISHER_DBONU))
    {
        RegisterSideIcon( m_pNetPySideIconGui, CNetPySideIconGui, "NetPySideIconGui", SIDEICON_NETPY);
        RegisterSideView( m_pNetPySideViewGui, CNetPySideViewGui, "NetPySideViweGui", SIDEVIEW_NETPY);
    }

	RegisterSideIcon(m_pHLSSideIconGui, CHLSSideIconGui, "HLSSideIconGui", SIDEICON_HLS);
	RegisterSideView(m_pHLSSideViewGui, CHLSSideViewGui, "HLSSideViewGui", SIDEVIEW_HLS);
	...
	if (pAvatarInfo->sCharPf.bIsGameMaster)
	{
		...
		m_pHLSSideIconGui->Show(true);
		CSideIconBase* pSideIcon = GetSideIcon(SIDEICON_HLS);
		pSideIcon->Show(true);
	}
...
void CSideIconGui::Destroy()
{
	...
	UnRegisterSideIcon( m_pTMQSideIconGui, SIDEICON_TMQ );
	UnRegisterSideView( m_pTMQSideViewGui, SIDEVIEW_TMQ );
	...
	UnRegisterSideIcon( m_pSCSSideIconGui, SIDEICON_DOJO );
	UnRegisterSideView( m_pSCSSideViewGui, SIDEVIEW_DOJO );
...
void CSideIconGui::HandleEvents(RWS::CMsg &msg)
{
	...
	else if( msg.Id == g_EventChangeWorldConceptState )
	{
		...
		if( pEvent->uiState == WORLD_STATE_EXIT )
		{
			...
			pSideIcon = GetSideIcon(SIDEICON_HLS);
			if (!pSideIcon->IsDisplay())
				pSideIcon->Show(true);
