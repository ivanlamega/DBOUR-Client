...
VOID CInfoWindowGui::SetItemInfo_Name( gui::CMDStaticBox* pmdBox, sITEM_TBLDAT* pData, RwUInt8 ucGrade, RwUInt8 ucRank )
{
	...
	std::wstring textName = API_GetTableContainer()->GetTextAllTable()->GetItemTbl()->GetText(pData->Name);
	std::wstring IDText = L"ID(";
	std::wstring strBuf;

	strBuf += IDText;
	
	strBuf += std::to_wstring((uint32_t)pData->Name) + L")\n";
	strBuf += textName;
...
VOID CInfoWindowGui::SetItemInfo_EquipLimit( gui::CMDStaticBox* pmdBox, sITEM_TBLDAT* pData, CNtlSobAvatarAttr* pAvatarAttr )
{
	...
	if( pData->dwNeed_Class_Bit_Flag != NTL_ITEM_ALL_USE_FLAG || 
		...
		pData->wEquip_Slot_Type_Bit_Flag != INVALID_DWORD )
		pmdBox->SetBlankLine( INFOWND_BLANKLINE_HEIGHT );
	...
	if( pData->wEquip_Slot_Type_Bit_Flag != INVALID_DWORD )
	{
		std::wstring strBuf;
		...
		if (flagSlotType & EQUIP_SLOT_FLAG_DOGI)
		{
			strBuf += GetDisplayStringManager()->GetString(DST_EQUIP_SLOT_TYPE_DOGI);
			strBuf += L", ";
		}
		if (flagSlotType & EQUIP_SLOT_FLAG_HAIR)
		{
			strBuf += GetDisplayStringManager()->GetString(DST_EQUIP_SLOT_TYPE_HAIR);
			strBuf += L", ";
		}
		if (flagSlotType & EQUIP_SLOT_FLAG_ACCESSORY_1)
		{
			strBuf += GetDisplayStringManager()->GetString(DST_EQUIP_SLOT_TYPE_ACCESSORY_1);
			strBuf += L", ";
		}
		if (flagSlotType & EQUIP_SLOT_FLAG_ACCESSORY_2)
		{
			strBuf += GetDisplayStringManager()->GetString(DST_EQUIP_SLOT_TYPE_ACCESSORY_2);
			strBuf += L", ";
		}
		if ((flagSlotType & EQUIP_SLOT_FLAG_ACCESSORY_3) || (flagSlotType == 0))
		{
			strBuf += GetDisplayStringManager()->GetString(DST_EQUIP_SLOT_TYPE_ACCESSORY_3);
			strBuf += L", ";
		}
...
RwBool CInfoWindowGui::SetItemInfo_Option( gui::CMDStaticBox* pmdBox, sITEM_TBLDAT* pData, RwUInt32 uiOptionTblIdx, RwBool bShowTitle, RwBool bBlankLine, RwInt32 nOrder /* = 0 */, RwUInt32 uiColor /* = INFOCOLOR_7  */ )
{
	...
	if( pItemOptionData->byScouterInfo != INVALID_BYTE )
	{
		...
		{
			pmdBox->Format("ScouterInfo", FONT_TEXT, COMP_TEXT_LEFT, uiColor, 0, FALSE, GetDisplayStringManager()->GetString(DST_SCOUTERPARTS_ITEM_IDENTIFICATION + pItemOptionData->byScouterInfo - SCOUTER_INFO_BEGIN_ONLY_ALLPY_MAX), pData->ScouterPartsType2);
		}
...
VOID CInfoWindowGui::SetSkillInfo_NameGradeClass( sSKILL_TBLDAT* pData, RwBool bLearned )
{
	...
	WCHAR* pString = NULL;	
	std::wstring textName = pSkillText->GetText(pData->Skill_Name);
	std::wstring IDText = L"ID(";
	std::wstring strBuf;

	strBuf += IDText;

	strBuf += std::to_wstring((uint32_t)pData->Skill_Name) + L")\n";
	strBuf += textName;

	// 이름
	m_pmdBox->SetItem(strBuf.c_str(), "SkillName", FONT_TITLE, COMP_TEXT_LEFT, uiColor );
...
VOID CInfoWindowGui::SetHTBInfo_NameGradeClass( sHTB_SET_TBLDAT* pData, RwBool bLearned )
{
	...
	RwUInt32 uiColor = bLearned ? INFOCOLOR_2 : INFOCOLOR_1;	
	WCHAR* pString = NULL;	
	std::wstring textName = pHTBTextTable->GetText(pData->HTB_Skill_Name);
	std::wstring IDText = L"ID(";
	std::wstring strBuf;

	strBuf += IDText;

	strBuf += std::to_wstring((uint32_t)pData->HTB_Skill_Name) + L")\n";
	strBuf += textName;
...
VOID CInfoWindowGui::EquipInfoProc( sITEM_TBLDAT* pData )
{
	...
	else if (flagSlotType & EQUIP_SLOT_FLAG_DOGI)
	{
		SetEquipInfo(m_pmdEquipInfo[0], pInventory->GetEquipItem(EQUIP_SLOT_TYPE_DOGI));
	}
	else if (flagSlotType & EQUIP_SLOT_FLAG_HAIR)
	{
		SetEquipInfo(m_pmdEquipInfo[0], pInventory->GetEquipItem(EQUIP_SLOT_TYPE_HAIR));
	}
	else if (flagSlotType & EQUIP_SLOT_FLAG_ACCESSORY_1)
	{
		SetEquipInfo(m_pmdEquipInfo[0], pInventory->GetEquipItem(EQUIP_SLOT_TYPE_ACCESSORY_1));
	}
	else if (flagSlotType & EQUIP_SLOT_FLAG_ACCESSORY_2)
	{
		SetEquipInfo(m_pmdEquipInfo[0], pInventory->GetEquipItem(EQUIP_SLOT_TYPE_ACCESSORY_2));
	}
	else if ((flagSlotType & EQUIP_SLOT_FLAG_ACCESSORY_3) || (flagSlotType == 0))
	{
		SetEquipInfo(m_pmdEquipInfo[0], pInventory->GetEquipItem(EQUIP_SLOT_TYPE_ACCESSORY_3));
	}
}
