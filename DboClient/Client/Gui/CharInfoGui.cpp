...
RwBool CCharacterInfoGui::Create()
{
	...
	m_pTitle->SetText(GetDisplayStringManager()->GetString(DST_LOBBY_CHARACTER_INFO));
		...
		if(pEvent->byMessage == LMT_SELECT_CHARACTER)
		{
			...
			else
			{
				...
				else
				{
	   			...
	   			// Last location Avatar Played
				char acBuffer[256] = "";
				if( FALSE == pMapNameTable->GetAreaName(uiMapNameIndex, &wstrText) )
				{
					sprintf_s(acBuffer, 256, "pMapNameTable->GetAreaName(uiMapNameIndex), Not exist %d uiIndex", uiMapNameIndex);
					NtlLogFilePrint(acBuffer);
					if( FALSE == pMapNameTable->GetSectorName(uiMapNameIndex, &wstrText) )
					{
						sprintf_s(acBuffer, 256, "pMapNameTable->GetSectorName(uiMapNameIndex), Not exist %d uiIndex", uiMapNameIndex);
						NtlLogFilePrint(acBuffer);
						if( FALSE == pMapNameTable->GetZoneName(uiMapNameIndex, &wstrText) )
						{
							sprintf_s(acBuffer, 256, "pMapNameTable->GetZoneName(uiMapNameIndex), Not exist %d uiIndex", uiMapNameIndex);
							NtlLogFilePrint(acBuffer);
