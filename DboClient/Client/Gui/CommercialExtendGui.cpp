...
void CCommercialExtendMode::SetCurrItemInfo(sITEM_TBLDAT* pITEM_TBLDAT)
{
	...
	if(pNtlSobItem)
	{
		...
		Logic_CaculDayHourMinSecond( pItemAttr->GetRemainTime(), &TimeRes );
...
void CCommercialExtendZennyMode::SetCurrItemInfo(sITEM_TBLDAT* pITEM_TBLDAT)
{
	...
	/// 소모 제니
	WCHAR text[GUI_TEXT_BUFFER_SIZE];
	swprintf_s( text, GUI_TEXT_BUFFER_SIZE, L"%d%s", pITEM_TBLDAT->dwCost, GetDisplayStringManager()->GetString( DST_ZENNY ) );
	text[GUI_TEXT_BUFFER_SIZE - 1] = NULL;
	...
	swprintf_s( text, GUI_TEXT_BUFFER_SIZE, L"%d%s", pAvatarAttr->m_uiZenny, GetDisplayStringManager()->GetString( DST_ZENNY ) );
	text[GUI_TEXT_BUFFER_SIZE - 1] = NULL;
...
RwBool CCommercialExtendZennyMode::ConFirm()
{
	...
	if(m_pTargetNtlSobItem)
	{
		...
		{
			GetDboGlobal()->GetGamePacketGenerator()->SendDurationRenewReq( m_pTargetNtlSobItem->GetSerialID() );
...
void CCommercialExtendNetphyMode::SetCurrItemInfo(sITEM_TBLDAT* pITEM_TBLDAT)
{
	...
	/// 소모 넷피
	WCHAR text[GUI_TEXT_BUFFER_SIZE];
	swprintf_s( text, GUI_TEXT_BUFFER_SIZE, L"%d%s", pITEM_TBLDAT->CommonPoint, GetDisplayStringManager()->GetString( DST_NETPY ) );
	text[GUI_TEXT_BUFFER_SIZE - 1] = NULL;
	...
	swprintf_s( text, GUI_TEXT_BUFFER_SIZE, L"%d%s", pAvatarAttr->m_dwNetPy, GetDisplayStringManager()->GetString( DST_NETPY ) );
	text[GUI_TEXT_BUFFER_SIZE - 1] = NULL;
...
void CCommercialExtendCashMode::Init()
{
	...
	/// 소모 넷피
	WCHAR text[82];
	swprintf_s( text, 82, L"%s", pStr );
	text[41] = NULL;
	m_AttributeControlSet.pStatic_stbDescript1->SetText( text );
	swprintf_s( text, 82, L"%s", &pStr[41] );
	text[41] = NULL;
	m_AttributeControlSet.pStatic_stbDescript2->SetText( text );
...
VOID CCommercialExtendGui::StartBuyDurItemProcessNPC( RwUInt32 uiItemIdx,  RwUInt32 uiNPCSerial, sSHOP_BUY_CART& ShopBuyCart )
{
	...
	{	/// Start MessageBox 시작		MBW_BUY_DUR_ITEM
		if( CCommercialExtendGui::ZENNY_EXTEND == m_eCurrExtendGuiMode )
			GetAlarmManager()->FormattedAlarmMessage(DST_COMMERCIAL_MB_DIRECT_BUY_MSG, FALSE, NULL, 
			pITEM_TBLDAT->wszNameText, pITEM_TBLDAT->dwCost);
...
VOID CCommercialExtendGui::StartBuyDurItemProcessNetpy( RwUInt32 uiItemIdx, RwUInt32 uiMerchantTblidx, BYTE byMerchanSlotPos )
{
	...
	{	/// Start MessageBox 시작		MBW_BUY_DUR_ITEM
		if( CCommercialExtendGui::NETPY_EXTEND == m_eCurrExtendGuiMode )
			GetAlarmManager()->FormattedAlarmMessage(DST_COMMERCIAL_MB_DIRECT_BUY_NETPY_MSG, FALSE, NULL, 
				pITEM_TBLDAT->wszNameText, pITEM_TBLDAT->CommonPoint);
...
{	/// Yardrat로부터 옮겨옴
	GetDboGlobal()->GetGamePacketGenerator()->SendCashitemMoveReq( m_NetSendData.DataSendMoveDurItemFromYardrat.m_uiProductId );
}
...
VOID CCommercialExtendGui::SetCurrItemTbl(sITEM_TBLDAT* pITEM_TBLDAT)
{
	...
	if( pITEM_TBLDAT )	/// icon setting
	{
		...
		m_pStatic_stbItemName->SetText( pITEM_TBLDAT->wszNameText );
