...
VOID CScouterGui::UpdatePartsInfo()
{
	...
	for( RwInt32 i = 0 ; i < pITEM_TBLDAT->byBag_Size ; ++i )
	{
		...
		if(pScouterParts)
		{
			...
			{
				// 파츠 정보를 가공한다
				ReadyPartsExplain(pPART_ITEM_TBLDAT->ScouterPartsType2, pPART_ITEM_TBLDAT->Item_Option_Tblidx);
	...
	for( ; it != m_mapPartsExplain.end() ; ++it )
	{
		if( it->first < SCOUTER_INFO_BEGIN_SPECIAL_FUNCTION )
		{
				...
				SCOUTER_INFO_ITEM_BREAK_RATE_DOWN	== byPartsInfo )
			{
				...
				swprintf_s(awcBuffer, 250, GetDisplayStringManager()->GetString(iType), (RwReal)it->second/100.f + 1.f);
			}
			else
				swprintf_s(awcBuffer, 250, GetDisplayStringManager()->GetString(iType), it->second);
