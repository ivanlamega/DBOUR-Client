...
RwBool CScouterPCInfoViewerGui::Create()
{
	...
	// 도복
	m_Item[EQUIP_SLOT_TYPE_DOGI].SlotItem.Create(m_pThis, SDIALOG_SCOUTER_PC_VIEWER, REGULAR_SLOT_ITEM_TABLE);
	m_Item[EQUIP_SLOT_TYPE_DOGI].SlotItem.SetPosition_fromParent(44, 285); //Change this back to EQUIP_SLOT_TYPE_COSTUME to see where to add new items - Sangawku

	// 스카우터
	m_Item[EQUIP_SLOT_TYPE_SCOUTER].SlotItem.Create(m_pThis, SDIALOG_SCOUTER_PC_VIEWER, REGULAR_SLOT_ITEM_TABLE);
	m_Item[EQUIP_SLOT_TYPE_SCOUTER].SlotItem.SetPosition_fromParent(88, 285);

	//Add Costume Items to Scouter Gui - SanGawku
