...
void COptionControlActionNode::OnClickBtnSetKey(gui::CComponent* pComponent)
{
	...
	{
		pMgr->GetParentTab()->SetGuideText(dOPTIONCONTROL_STATICBOX_COLOR, DST_OPTION_CONTROL_TEXT_READYGUIDE );
		pMgr->UpdateNode();
...
void COptionControlList::CreateTree() 
{
	...
	RwUInt32 nActionTitle = DST_OPTION_CONTROL_ACTION_AVATAR_FORWARD;
	for(RwUInt32 nActionID = ACTION_AVATAR_FORWARD; nActionID <= ACTION_AVATAR_BLOCKING; ++nActionID)
	{
		pNode = NTL_NEW COptionControlActionNode(this,
			GetDisplayStringManager()->GetString( nActionTitle ),								// Action 제목
			nActionID );															// Action ID
	...
	nActionTitle = DST_OPTION_CONTROL_ACTION_TARGET_SELF;
	for(RwUInt32 nActionID = ACTION_TARGET_SELF; nActionID <= ACTION_TARGET_4THPARTY; ++nActionID)
	{
		pNode = NTL_NEW COptionControlActionNode(this,
			GetDisplayStringManager()->GetString( nActionTitle ),
			nActionID );
	...
	nActionTitle = DST_OPTION_CONTROL_ACTION_TARGET_AUTO;
	...
	nActionTitle = DST_OPTION_CONTROL_ACTION_QUICKSLOT_QUICK1;
	...
	// 확장
	// 80 ~ 159 까지 빈자리 예약
	nActionTitle = DST_OPTION_CONTROL_ACTION_QUICKSLOT_QUICK1_EX;
	...
	// 퀵슬롯 조작 관련
	nActionTitle = DST_OPTION_CONTROL_ACTION_QUICKSLOT_PREV;
	...
	nActionTitle = DST_OPTION_CONTROL_ACTION_WINDOW_PROFILE;
...
{	
	...
	//GetInputActionMap()->CancleActionMap();	   ///< 액션맵의 임시 저장된것을 로드 Testing Fix - Sangawku
...
void COptionControl::OnOk()
{
	...
	if( GetInputActionMap()->ApplyActionMap() )
	{
		...
		SetGuideText(dOPTIONCONTROL_STATICBOX_WARNING_COLOR, DST_OPTION_CONTROL_TEXT_WAITGUIDE );
}
...
void COptionControl::OnHandleEvents( RWS::CMsg &pMsg ) 
{
	...
	if( pMsg.Id == g_EventActionMapClientNotify )
	{
		...
		case SNtlEventActionMapClientNotify::ACTIONMAP_FIXEDKEY:						// 키가 고정된 키라고 알림
			SetGuideText( dOPTIONCONTROL_STATICBOX_GUIDE_COLOR, DST_OPTION_CONTROL_TEXT_FIXEDKEYGUIDE );
			break;
		case SNtlEventActionMapClientNotify::ACTIONMAP_NOTCOMBINE:						// 조합키로는 지정못한다고 알림
			SetGuideText( dOPTIONCONTROL_STATICBOX_GUIDE_COLOR, DST_OPTION_CONTROL_TEXT_NOTCOMBINEGUIDE );
			break;
		case SNtlEventActionMapClientNotify::ACTIONMAP_SAMEKEY:
			SetGuideText( dOPTIONCONTROL_STATICBOX_GUIDE_COLOR, DST_OPTION_CONTROL_TEXT_SAMEKEYGUIDE );
			break;
		default:
			break;
		}
...
void COptionControl::SetGuideText( RwUInt32 uiColor,  RwUInt32 uiStringID )
{
	m_pStbControlText->SetTextColor( uiColor );
	m_pStbControlText->SetText( GetDisplayStringManager()->GetString( uiStringID ) );
}
...
{
	RwInt32 iDisplayStringNum = GetActionDisplayStringNum( nAction );

	WCHAR awcTemp[512];
	swprintf_s(awcTemp, 512, 
		GetDisplayStringManager()->GetString(DST_OPTION_CONTROL_TEXT_CHANGEGUIDE),
		GetDisplayStringManager()->GetString(iDisplayStringNum),
		GetDisplayStringManager()->GetString(iDisplayStringNum),
		GetDisplayStringManager()->GetString(DST_OPTION_CONTROL_TEXT_EMPTYKEY));
...
void COptionControl::SetGuideFixed( RwUInt32 nAction ) 
{
	...
	swprintf_s( awcTemp, 512, GetDisplayStringManager()->GetString(DST_OPTION_CONTROL_TEXT_NOTCHANGEGUIDE),
		GetDisplayStringManager()->GetString( GetActionDisplayStringNum( nAction ) ) );
...
/**
* \brief Action의 ID로 DisplayManager에서의 문자열의 번호를 계산
*
* \param uiAction (RwUInt32) Action의 ID
* \returns DisplayManager에서의 ID
*/
RwUInt32 COptionControl::GetActionDisplayStringNum( RwUInt32 uiAction ) 
{
	int nOffsetString = 0;

	// 텍스트 설명창에 디스플레이될 안내문 결정
	if( uiAction >= ACTION_GLOBAL_CANCLE )
	{
		nOffsetString += (DST_OPTION_CONTROL_ACTION_GLOBAL_CANCLE - DST_OPTION_CONTROL_ACTION_AVATAR_FORWARD)
			+ (uiAction - ACTION_GLOBAL_CANCLE);
	}
	else if( uiAction >= ACTION_CHAT_PGUP )
	{
		nOffsetString += (DST_OPTION_CONTROL_ACTION_CHAT_PGUP - DST_OPTION_CONTROL_ACTION_AVATAR_FORWARD)
			+ (uiAction - ACTION_CHAT_PGUP);
	}
	else if( uiAction >= ACTION_MINIMAP_ZOOMOUT )
	{
		nOffsetString += (DST_OPTION_CONTROL_ACTION_MINIMAP_ZOOMOUT - DST_OPTION_CONTROL_ACTION_AVATAR_FORWARD)
			+ (uiAction - ACTION_MINIMAP_ZOOMOUT );
	}
	else if( uiAction >= ACTION_WINDOW_PROFILE )
	{
		nOffsetString += (DST_OPTION_CONTROL_ACTION_WINDOW_PROFILE - DST_OPTION_CONTROL_ACTION_AVATAR_FORWARD)
			+ (uiAction - ACTION_WINDOW_PROFILE );
	}
	else if( uiAction >= ACTION_QUICK_1 )
	{
		nOffsetString += (DST_OPTION_CONTROL_ACTION_QUICKSLOT_QUICK1 - DST_OPTION_CONTROL_ACTION_AVATAR_FORWARD)
			+ (uiAction - ACTION_QUICK_1 );
	}
	else if( uiAction >= ACTION_TARGET_SELF )
	{
		nOffsetString += (DST_OPTION_CONTROL_ACTION_TARGET_SELF - DST_OPTION_CONTROL_ACTION_AVATAR_FORWARD)
			+ (uiAction - ACTION_TARGET_SELF );
	}
	else
		nOffsetString = uiAction;

	return DST_OPTION_CONTROL_ACTION_AVATAR_FORWARD+nOffsetString;
}