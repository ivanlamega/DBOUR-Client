...
CDisplayParser::CDisplayParser(const std::string &strDefFileName, const std::string &strDataFileName)
{
	/*if(!ParseDef(strDefFileName))
		return;

	if(!ParseData(strDataFileName))
		return;*/	

	if( !ParseUnicodeDef(strDefFileName) )
		return;
...
RwBool CDisplayParser::ParseUnicodeData( const std::string &strFilename )
{
	...
	while(1)
	{
		...
		case PARSE_DATA_VARIABLE_TYPE:
			...
			break;
		case PARSE_DATA_VARIABLE_STRING:
			m_DataOptionW.AddOption(chType, wstrToken); 
			break;
		}
