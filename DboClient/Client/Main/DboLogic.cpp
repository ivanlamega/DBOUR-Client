...
eEQUIP_SLOT_TYPE Logic_ConvertEquipSlotIdxToType( RwUInt8 ucEquipSlotIdx )
{
	...
	case 12: return EQUIP_SLOT_TYPE_DOGI;
	case 13: return EQUIP_SLOT_TYPE_HAIR;
	case 14: return EQUIP_SLOT_TYPE_ACCESSORY_1;
	case 15: return EQUIP_SLOT_TYPE_ACCESSORY_2;
	case 16: return EQUIP_SLOT_TYPE_ACCESSORY_3;
...
{
	RwUInt8 equipType = (eEquipType - EQUIP_SLOT_TYPE_HAND);
	return equipType;
}

RwBool Logic_EquipSlotTypeFlagCheck( RwUInt32 usTypeFlag1, RwUInt32 usTypeFlag2 )
{
	//More logic for New Items - Sangakwu
...
RwInt32 Logic_ConvertEquipSlotIdxToFlag( RwUInt8 ucEquipSlotIdx )
{
	...
	if( eEquipSlotType == EQUIP_SLOT_TYPE_UNKNOWN )
		...
		return (RwInt32)( 0x01 << eEquipSlotType );
}
...
RwBool Logic_ItemDirectEquipProc( SERIAL_HANDLE hSrcSerial, EPlace eSrcPlace, RwUInt8 ucSrcSlotIdx )
{
	...
	else
	{
		...
		{	
			...
			{
				RwUInt32 wSlotFlag = Logic_ConvertEquipSlotIdxToFlag( i );
...
RwBool Logic_IsMailRemainTimeOverDay( DBOTIME tmTime )
{
	...
	if( 0 != errid )
	{
		DBO_FAIL( "Server time Logic_IsMailRemainTimeOverDay - "<< serverTime << "error number: " << errid );
		return FALSE;
	}
	...
	if( 0 != errid )
	{
		DBO_FAIL( "Remain time Logic_IsMailRemainTimeOverDay - "<< remainTime << "error number: " << errid);
		return FALSE;
	}
...
gui::CSurface& Logic_GetNPCClassIconSurface(RwUInt8 byClass, RwBool bOutLine /* TRUE */ )
{
	...
	default:
		printf("Not exist class %d", byClass);
	}
...
RwUInt32 Logic_CalcPrice_for_Sell(sITEM_TBLDAT* pITEM_TBLDAT, sITEM_OPTION_TBLDAT* pITEM_OPTION_TBLDAT, RwUInt32 uiCount)
{
	...
	{
		uiSellPrice = pITEM_TBLDAT->bySell_Price + (pITEM_OPTION_TBLDAT->dwCost / 5);// (RwUInt32)NTL_SELL_PRICE(pITEM_TBLDAT->bySell_Price, pITEM_TBLDAT->dwCost, pITEM_OPTION_TBLDAT->dwCost);
	}
	else
	{
		uiSellPrice = pITEM_TBLDAT->bySell_Price;//(RwUInt32)NTL_SELL_PRICE(pITEM_TBLDAT->bySell_Price, pITEM_TBLDAT->dwCost, 0);
	}
...
RwUInt8 Logic_GetCounterpartGradeType(CNtlSobAttr* pOriginalSobAttr, CNtlSobAttr* pCounterpartSobAttr)
{
	...
	if( byLevel[0] + 6 < byLevel[1] )
	{
		return CGT_STRONG_1;
	}
	else if(byLevel[0] - 3 > byLevel[1])
	{
		return CGT_WEAK_1;
	}
