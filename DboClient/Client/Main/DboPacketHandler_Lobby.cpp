...
void PacketHandler_LSLoginRes(void *pPacket)
{
	...
	{
		if (eDBO_SERVICE_PUBLISHER_DBONU == eService)
		{
		...
		else 
		{
			...
			GetAlarmManager()->AlarmMessage(pLoginRes->wResultCode, TRUE);
			//Buffer for debugging the alarm message that should be output
			char buffer[256];
			wsprintf(buffer, "Login message should be: %u\n", pLoginRes->wResultCode);
...
void PacketHandler_CSCharConnectWaitCheckRes(void *pPacket)
{
	...
	// 게임 서버에 들어갈 수 있는 인원 제한에 걸리지 않았다면
	// 게임 서버의  접속 패킷들을 주고 받기 시작한다
	...
	{
		// 게임 서버 접속 대기자가 있다. 이후 CU_CONNECT_WAIT_COUNT_NFY 패킷으로 대기 순번을 알려준다
...
{
	sCU_CHARACTER_EXIT_RES* pResult = (sCU_CHARACTER_EXIT_RES*)pPacket;
	if (pResult->wResultCode == CHARACTER_SUCCESS)
	{
		SConnectData *pConnectData = GetDboGlobal()->GetConnectData();
...
void PacketHandler_GSAvatarWorldInfo(void *pPacket)
{
	...
	if( strStageName == CHARACTER_STAGE_NAME )
	{
		// 서버로 부터 패킷을 기다린다는 메세지 박스가 떠 있다면 닫는다
		GetMsgBoxManager()->DeleteNetConnectBox();
	}	
	...
	// game stage에 들어와 있는지 아닌지를 검사한다.
	// game stage에 들어와 있는 경우에는 teleport or rescus 의해서 호출된다.		
...
{
	// game stage에 들어와 있는지 아닌지를 검사한다.
	// game stage에 들어와 있는 경우에는 teleport or rescus or worldchange에 의해서 호출된다.
