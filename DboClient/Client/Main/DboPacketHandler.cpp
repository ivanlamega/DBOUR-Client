...
void PacketHandler_GSCharActionAttack(void *pPacket)
{
	...
	std::wstring combatMessage = L"";
	pSobObj = GetNtlSobManager()->GetSobObject(pActAttack->hSubject);
	...
	if (!pSobObj)
	{
		...
		switch (pActAttack->byAttackResult)
		{
			case BATTLE_ATTACK_RESULT_DODGE:
			{
				format = L" Misses";
				break;
			}
			case BATTLE_ATTACK_RESULT_CRITICAL_HIT:
			{
				format = L"'s attack critically hits for ";
				break;
			}
			case BATTLE_ATTACK_RESULT_BLOCK:
			{
				format = L"'s attack is blocked ";
			}
			default:
			{
				format = L"'s attack hits ";
			}
		}
		switch (pSobObj->GetClassID())
		{
			case SLCLASS_AVATAR:
			{
				CNtlSobAvatar* pAvatar = reinterpret_cast<CNtlSobAvatar*>(pSobAvatar);
				CNtlSobAvatarAttr* pAvatarAttr = reinterpret_cast<CNtlSobAvatarAttr*>(pAvatar->GetSobAttr());

				const WCHAR* name = pAvatarAttr->GetName();

				if (pActAttack->byAttackResult == BATTLE_ATTACK_RESULT_DODGE)
					combatMessage += name + std::wstring(format);
				else
					combatMessage += name + std::wstring(format) + std::to_wstring(pActAttack->wAttackResultValue);

				CNtlSLEventGenerator::SysMsg(combatMessage.c_str(), SNtlEventSysStringMsg::TYPE_CHAT_WARN);
				break;
			}
			case SLCLASS_MONSTER:
			{
				CNtlSobMonster* pMonster = reinterpret_cast<CNtlSobMonster*>(pSobObj);
				CNtlSobMonsterAttr* pMonsterAttr = reinterpret_cast<CNtlSobMonsterAttr*>(pMonster->GetSobAttr());

				const WCHAR* name = pMonsterAttr->GetName();
				if (pActAttack->byAttackResult == BATTLE_ATTACK_RESULT_DODGE)
					combatMessage += name + std::wstring(format);
				else
					combatMessage += name + std::wstring(format) + std::to_wstring(pActAttack->wAttackResultValue);
			...
			case SLCLASS_PLAYER:
			{
				...
				const WCHAR* name = pPlayerAttr->GetName();
...
void PacketHandler_GSCharSkillAction(void *pPacket)
{
	...
	else // 성공
	{
		...
		//else
			...
			std::wstring combatMessage = L"";
			CNtlSob* pSobObj = GetNtlSobManager()->GetSobObject(pSkillAction->handle);
			const WCHAR* format = L"";
			
			switch (pSkillAction->aSkillResult->byAttackResult)
			{
				case BATTLE_ATTACK_RESULT_DODGE:
				{
					format = L" Misses";
				...
				case BATTLE_ATTACK_RESULT_CRITICAL_HIT:
				{
					format = L"'s skill critically hits for ";
					break;
				}
				default:
				{
					format = L"'s skill hits for ";
				}
			}
			...
			switch (pSobObj->GetClassID())
			{
				case SLCLASS_AVATAR:
				{
					...
					if (pSkillAction->aSkillResult->byAttackResult == BATTLE_ATTACK_RESULT_DODGE)
						combatMessage += name + std::wstring(format);
					else
					{
						if(pSkillAction->aSkillResult->effectResult1.eResultType == DBO_SYSTEM_EFFECT_RESULT_TYPE_GENERAL ||
							pSkillAction->aSkillResult->effectResult1.eResultType == DBO_SYSTEM_EFFECT_RESULT_TYPE_DD_DOT )
						combatMessage += name + std::wstring(format) + std::to_wstring((uint32_t)pSkillAction->aSkillResult->effectResult1.fResultValue);
						else
						{
							format = L"'s skill heals for ";
							combatMessage += name + std::wstring(format) + std::to_wstring((uint32_t)pSkillAction->aSkillResult->effectResult1.fResultValue);
						}
					}
...
void PacketHandler_GSCharUpdateLp(void *pPacket)
{
	...
	//Fix for Party Invalidated Crash
	if(pParty &&  pParty->IsHaveGroup())
	{	
...
void PacketHandler_GSCharUpdateItemEquip(void *pPacket)
{
	...
	//Fix this crash too. dumb shit -sangawku
	if (pSobPlayer == NULL)
	{
		NTL_ASSERTFAIL("PacketHandler_GSCharUpdateItemEquip => pSobPlayer doesn't exist");
		return;
	}
	CNtlInventoryBase *pInventory = pSobPlayer->GetEquipInventory();
	if (pInventory == NULL)
	{
		NTL_ASSERTFAIL("PacketHandler_GSCharUpdateItemEquip => pInventory doesn't exist");
		return;
	}
...
void PacketHandler_GSCharUpdateState(void *pPacket)
{
	...
	{
		std::string logInfo = "unavailable avatar position. x : " + std::to_string(v3PosTemp.x) + " y : " + std::to_string(v3PosTemp.y) + " z : " + std::to_string(v3PosTemp.z);
		NtlLogFilePrint((RwChar*)logInfo.c_str());
	}
...
void PacketHandler_GUSoundPlaySFX(void* pPacket)
{
	sGU_PLAY_SFX* pResult = (sGU_PLAY_SFX*)pPacket;

	CNtlSobAvatar *pSobAvatar = GetNtlSLGlobal()->GetSobAvatar();
	CNtlSob* origin = GetNtlSobManager()->GetSobObject(pResult->originHandle);
	if(origin != nullptr)
	{
		sNtlSoundPlayParameta tSoundParam;

		tSoundParam.iChannelGroup = CHANNEL_GROUP_VOICE_SOUND;
		tSoundParam.pcFileName = pResult->szName;
	
		tSoundParam.fXPos = origin->GetPosition().x;
		tSoundParam.fYPos = origin->GetPosition().y;
		tSoundParam.fZPos = origin->GetPosition().z;
		tSoundParam.bLoop = false;
		tSoundParam.fMaxDistance = 100.f;
		tSoundParam.fVolume = 1.0f;
...
void PacketHandler_GSDestroyAllObjects(void *pPacket)
{
	sGU_DESTROY_ALL_OBJECTS_NFY *pObjDestroy = (sGU_DESTROY_ALL_OBJECTS_NFY*)pPacket;
	CNtlSobAvatar *pSobAvatar = GetNtlSLGlobal()->GetSobAvatar();
	CNtlSob *pSobObj;

	GetNtlSobManager()->RemoveAllObjects(pSobAvatar->GetSerialID());
