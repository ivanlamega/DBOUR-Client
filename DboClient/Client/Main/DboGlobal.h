...
{
	WCHAR wchUserID[NTL_MAX_SIZE_USERID_UNICODE+1];			// user id
	WCHAR wchPassword[NTL_MAX_SIZE_USERPW_UNICODE+1];		// user password
	...
	void Clear(void)
	{
		memset(wchUserID, 0, sizeof(WCHAR)*(NTL_MAX_SIZE_USERID_UNICODE+1));
		memset(wchPassword, 0, sizeof(WCHAR)*(NTL_MAX_SIZE_USERPW_UNICODE+1));
	}
...
typedef struct _SGameData
{
	...
	void Init(void)
	{
		...
		bInputActive		= TRUE;
		bDebugDisplay		= FALSE;
		hTargetSerial		= INVALID_SERIAL_ID;
...
private:
	...
	*/
	...
	std::string		m_sGuiFolderName;					/** 종족별 Gui 폴더 이름 **/	
...
private:
	...
	RwBool LoadUserOptions(const RwChar *pFileName);
	RwBool LoadUIOptions(const RwChar *pFileName);	
...
public:
	...
	* 
	*/
	void			SetTargetSerial( SERIAL_HANDLE hSerial );
	SERIAL_HANDLE	GetTargetSerial(void)						{ return m_sGame.hTargetSerial; }
	...
	*/
	...
	void			SetGuiFolderName(char* pcText)				{ m_sGuiFolderName = pcText; }
	char*			GetGuiFolderName()							{ return (char*)m_sGuiFolderName.c_str(); }
...
//Discord Shits
	static const char* DISCORD_APPLICATION_ID;
	static int64_t m_pStartTime;
	static DiscordRichPresence m_pDiscordPresence;
	static char discordCharName[17];
	static char discordClassName[20];

	void InitDiscord();
	void UpdateLoginStage(DiscordRichPresence presence);
	void UpdateCharStage(DiscordRichPresence presence);
	void UpdateGameStage(DiscordRichPresence presence, WCHAR charName[17], ePC_CLASS charClass);
	void UpdateGameStageLocation(DiscordRichPresence presence, std::wstring zoneName, std::wstring sectorName);
	std::string GetCharClassIcon(ePC_CLASS charClass);
	//void UpdateGameStage(DiscordRichPresence presence, WCHAR charName[17], uint8_t channelNumber);
};
