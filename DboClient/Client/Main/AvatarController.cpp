...
void CAvatarController::HandleEvents(RWS::CMsg &pMsg)
{
	...
	{
		GetAlarmManager()->AlarmMessage( L"Feature Called Auto-Care System!!" ,CAlarmManager::ALARM_ACTION_NOTIFY);	
	}
}
...
int	CAvatarController::KeyboardDownHandler(unsigned int uiKeyData)
{
	...
	{        
		...
		if (Logic_IsDevUser())
		{
			sGU_UPDATE_CHAR_EXP sPacket;
			memset(&sPacket, 0, sizeof(sGU_UPDATE_CHAR_EXP));
			sPacket.wOpCode = GU_UPDATE_CHAR_EXP;
			sPacket.dwAcquisitionExp = 1000;
			sPacket.dwBonusExp = 10;
			sPacket.dwIncreasedExp = sPacket.dwAcquisitionExp + sPacket.dwBonusExp;
			sPacket.dwCurExp = GetNtlSLGlobal()->GetAvatarInfo()->sCharPf.dwCurExp + sPacket.dwIncreasedExp;
			sPacket.handle = GetNtlSLGlobal()->GetSobAvatar()->GetSerialID();

			PacketHandler_GSCharUpdateExp(&sPacket);
		}
	...
	else if(pData->uiChar == NTL_KEY_F3 )
	{
		...
		if (Logic_IsDevUser())
		{
			sGU_DESTROY_ALL_OBJECTS_NFY sPacket;
			memset(&sPacket, 0, sizeof(sGU_DESTROY_ALL_OBJECTS_NFY));
			sPacket.wOpCode = GU_DESTROY_ALL_OBJECTS_NFY;			
			PacketHandler_GSDestroyAllObjects(&sPacket);
		}
	}
	...
	else if (pData->uiChar == NTL_KEY_F9 )//NTL_KEY_NUMLOCK )
	{		
