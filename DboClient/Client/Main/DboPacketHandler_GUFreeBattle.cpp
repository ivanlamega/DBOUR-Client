...
void PacketHandler_GSFreePVPStartNfy(void *pPacket)
{
	sGU_WORLD_FREE_PVP_ZONE_ENTERED_NFY  *pStartNfy = (sGU_WORLD_FREE_PVP_ZONE_ENTERED_NFY *)pPacket;

	if (!GetNtlWorldConcept()->IsActivePlayConcept(WORLD_PLAY_OPENPVP))
	{
		GetNtlWorldConcept()->AddWorldPlayConcept(WORLD_PLAY_OPENPVP);
	}
	CNtlWorldConceptOpenPVP* pWorldConceptPVP = (CNtlWorldConceptOpenPVP*)GetNtlWorldConcept()->GetWorldConceptController(WORLD_PLAY_OPENPVP);
	CNtlSLEventGenerator::TSOpenBattleStart(pStartNfy->handle);
}

void PacketHandler_GSFreePVPEndNfy(void *pPacket)
{
	sGU_WORLD_FREE_PVP_ZONE_LEFT_NFY *pEndNfy = (sGU_WORLD_FREE_PVP_ZONE_LEFT_NFY *)pPacket;
	
	CNtlWorldConceptOpenPVP* pWorldConceptPVP = (CNtlWorldConceptOpenPVP*)GetNtlWorldConcept()->GetWorldConceptController(WORLD_PLAY_OPENPVP);
	if (GetNtlWorldConcept()->IsActivePlayConcept(WORLD_PLAY_OPENPVP))
	{
		GetNtlWorldConcept()->RemoveWorldPlayConcept(WORLD_PLAY_OPENPVP);
	}
	CNtlSLEventGenerator::TSOpenBattleEnd(pEndNfy->handle);
