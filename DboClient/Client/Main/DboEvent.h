...
extern RWS::CEventId g_EventCharm;					// 파티 부적 이벤트

extern RWS::CEventId g_EventCharmVictim;			// 파티 부적 제물 이벤트

extern RWS::CEventId g_EventPartyInventory;			// 파티 인벤토리 이벤트
...
extern RWS::CEventId g_EventScouterUpgradePredict;	// 스카우터 업그레이드 예측

extern RWS::CEventId g_EventESCinNarrationMode;		// 나레이션 모드에서 ESC처리.
...
// Hoipoi Mix
extern RWS::CEventId g_EventHoipoiMixItemCheckRes;		///< 만들 아이템 선택 ( 필요한 아이템 수량 체크
extern RWS::CEventId g_EventHoipoiMixItemMakeRes;		///< 아이템 만들기
extern RWS::CEventId g_EventHoipoiMixItemMakeEpRes;		///< 아이템 만들기 중 EP 넣기
...
{
	LOGIN_EVENT_SUCCESS_LOGIN,						// 로그인을 성공했다
	...
	LOGIN_EVENT_RETRY_LOGIN_REQ,					// 로그인을 실패하여 다시 시도한다
};
...
{	
	...
	LMT_RENAME_CHARACTER,								// 캐릭터 이름이 바뀌었다
	LMT_SELECT_CHARACTER,								// 캐릭터를 선택했다	
	...
	LMT_START_CONNECT_GAME_SERVER,						// Game Server에 연결을 시작한다
...
{
	RwBool			bActive;
	RwUInt32		uiStringID;		///< 문자열 인덱스
	const WCHAR*	pString;		///< 문자열
	RwReal			fLifeTime;		///< 지속시간
};
...
struct SDboEventShop
{
	...
	// for Server
	RwInt32			iPlace;			///< NPC상점의 탭, 혹은 가방의 번호
	RwInt32			iPosition;		///< 탭안 등록된 아이템의 위치, 가방 안의 슬롯 번호
	RwInt32			iOverlapCount;	///< 갯수
};
...
{
	PCET_OPEN,							///< 파티 부적 슬롯을 연다
	PCET_CLOSE,							///< 파티 부적 슬롯을 닫는다
	PCET_ACTIVATE,						///< 파티 부적을 발동시킨다.
	PCET_DISPLAY_CHARM_BUFF,			///< 파티 버프를 보여준다
	PCET_UNDISPLAY_CHARM_BUFF,			///< 파티 버프가 취소되었다
};
...
{
	PVI_REG_CHARM,						///< 부적을 등록한다
	PVI_UNREG_CHARM,					///< 부적을 등록을 해제한다
	PVI_ZENNY,							///< 제니를 등록한다.
	PVI_START_CHARM_DICE,				///< 파티 부적 주사위를 연다
};
...
{
	PINVEN_ADD_ITEM,					///< 아이템을 추가한다
	PINVEN_DEL_ITEM,					///< 아이템을 삭제한다

	PINVEN_CHANGE_SAVED_ITEM_GRADE,		///< 파티 인벤에 들어오는 아이템의 등급을 변경했다

	PINVEN_DIVIDE_BY_LEADER,			///< 파티 인벤의 아이템을 파티장이 직접 분배했다
	
	PINVEN_DIVIDE_BY_OCTION_LOCK,		///< 파티 인벤의 아이템의 경매 락/언락
	PINVEN_DIVIDE_BY_OCTION_INVEST_NOTIFY,	///< 파티 인벤의 아이템 경매 호가 정보를 알린다
	PINVEN_DIVIDE_BY_OCTION_RESET,		///< 파티 인벤의 아이템 경매 정보가 초기화되었다
	PINVEN_DIVIDE_BY_OCTION_SUCCESSFUL_BID,	///< 파티 인벤의 아이템 경매가 낙찰되었다

	PINVEN_DIVIDE_BY_DICE_START,		///< 파티 인벤의 주사위 굴리기가 시작 되었다
	PINVEN_DIVIDE_BY_DICE_RESULT,		///< 파티 인벤의 주사위 굴리기 종료
};

struct SDboPartyInvetoryItem
{
	RwUInt32		iType;				///< 이벤트 타입
	RwUInt32		uiSerial;			///< 아이템 테이블 시리얼 or 파티 맴버 시리얼
	RwUInt8			byPos;				///< 아이템 위치
	// bNeedToIdentify가 true일 때는 byStackCount, byRank, byCurDur, option은 의미가 없는 값이다.
	// byStackCount, byRank, byCurDur and option's values are meaningless if bNeedToIdentify is true.
	// by YOSHIKI(2006-10-31)
	RwUInt32		uiStackCount;		///< 갯수
	RwUInt8			byRank;				///< 아이템 랭크

	RwUInt32		uiOptionSerial1;	///< 옵션 테이블 인덱스
	RwUInt32		uiOptionSerial2;
	RwBool			bTrue;				///< 미확인 아이템인지 여부 or 내가 파티 인벤의 아이템을 받았는지 여부 or 파티 인벤 경매 Lock 여부
	RwUInt8			byGrade;			///< 등급
...
struct SDboEventPrivateShopBusinessStart
{
	...
	WCHAR					awcTargetName[NTL_MAX_SIZE_CHAR_NAME_UNICODE + 1];	///< 상대방의 캐릭터 이름
...
{
	SDIALOG_MESSAGE_CONTROLLER_ADD_BUTTON,		///< Side dialog controller에 버튼을 추가한다
	SDIALOG_MESSAGE_CONTROLLER_DEL_BUTTON,		///< Side dialog controller에 버튼을 삭제한다
	SDIALOG_MESSAGE_MINIMIZE,					///< Side dialog 최소화
	SDIALOG_MESSAGE_INACTIVE,					///< Side dialog 최소화창을 닫느다
};
