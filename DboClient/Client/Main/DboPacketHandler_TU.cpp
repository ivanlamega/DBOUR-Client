...
void PacketHandler_TSChatMsgSay(void *pPacket)
{
    ...
    if(GetNtlSLGlobal()->GetSobAvatar()->GetBlackList()->GetMemberbyName(pMsgSay->awchSenderCharName))
        return;
	CNtlSobAvatar* pSob = reinterpret_cast<CNtlSobAvatar*>(GetNtlSobManager()->GetSobObject(pMsgSay->hSubject));
	if (pSob)
	{
		CNtlSobPlayerAttr* pPlayerAttr = reinterpret_cast<CNtlSobPlayerAttr*>(pSob->GetSobAttr());
		CDboEventGenerator::SayMessage(CHAT_TYPE_GENERAL, pMsgSay->awchSenderCharName, pMsgSay->wMessageLengthInUnicode, pMsgSay->awchMessage, pMsgSay->hSubject, pPlayerAttr->GetIsGameMaster());
	}
	else
...
void PacketHandler_TSChatMsgShout(void *pPacket)
{
	...
	CNtlSobAvatar* pSob = reinterpret_cast<CNtlSobAvatar*>(GetNtlSobManager()->GetSobObject(pMsgShout->hSubject));
	if (pSob)
	{
		CNtlSobPlayerAttr* pPlayerAttr = reinterpret_cast<CNtlSobPlayerAttr*>(pSob->GetSobAttr());
		CDboEventGenerator::SayMessage(CHAT_TYPE_SHOUT, pMsgShout->awchSenderCharName, pMsgShout->wMessageLengthInUnicode, pMsgShout->awchMessage, pMsgShout->hSubject, pPlayerAttr->GetIsGameMaster());
	}
	else
...
void PacketHandler_TSChatMsgParty(void *pPacket)
{
	...
	CNtlSobAvatar* pSob = reinterpret_cast<CNtlSobAvatar*>(GetNtlSobManager()->GetSobObject(pMsgParty->hSubject));
	if (pSob)
	{
		CNtlSobPlayerAttr* pPlayerAttr = reinterpret_cast<CNtlSobPlayerAttr*>(pSob->GetSobAttr());
		CDboEventGenerator::SayMessage(CHAT_TYPE_PARTY, pMsgParty->awchSenderCharName, pMsgParty->wMessageLengthInUnicode, pMsgParty->awchMessage, pMsgParty->hSubject, pPlayerAttr->GetIsGameMaster());
	}
	else
