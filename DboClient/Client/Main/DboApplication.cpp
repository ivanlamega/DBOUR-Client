...
CDboApplication::CDboApplication() : m_pCharset(NULL)
{
	...
	// frame rate 보간.
	m_uiFrameRate = 10000;
	CNtlTimer::ActiveInterpolation(false);
}
...
RwBool CDboApplication::CreateSubSystem(RwUInt32 iWidth, RwUInt32 iHeight, RwBool zBuffer)
{
	...
	RwBool bDevUser = Logic_IsDevUser();//Make sure to change this shit back eventually. -_- - Sangawku
...
//		GetNtlResourceManager()->SetTextureLoadLevel(CNtlPLResourceManager::RES_TEX_EFFECT_DICT, 2);
	...
	// display string load
	GetAlarmManager()->ParseWorkIDW( ".\\language\\msgboxworkid.dat" );

	SConfigData *pConfigData = m_DboGlobal.GetConfigData(); 
	CDisplayParser parser(".\\language\\local_def.dat", pConfigData->strLocalDsp.c_str());
	CDisplayStringManager::GetInstance()->InsertOptions(parser);  
	GetAlarmManager()->ParseDataW( parser, ".\\language\\local_msg_type.dat", FALSE );

	CDisplayParser syncparser(".\\language\\local_sync_def.dat", pConfigData->strLocalSyncDsp.c_str());
	CDisplayStringManager::GetInstance()->InsertOptions(syncparser, CDisplayStringManager::SERVER_SYNC_RESULT_MSG);  
	GetAlarmManager()->ParseDataW( syncparser, ".\\language\\local_sync_msg_type.dat", TRUE );	
	...
	if(Logic_IsDevUser())
	{
		...
		{
			if(LOAD_PROP_MODE == DATA_MODE_XML)
				bLoadPropertyContainer = CNtlPLPropertyContainer::GetInstance()->Load();
			else if(LOAD_PROP_MODE == DATA_MODE_BIN)
				bLoadPropertyContainer = CNtlPLPropertyContainer::GetInstance()->LoadSerialize();
			else if(LOAD_PROP_MODE == DATA_MODE_ENC)
	...
	/*	if (LOAD_PROP_MODE == DATA_MODE_XML)
		{
			bLoadPropertyContainer = CNtlPLPropertyContainer::GetInstance()->Load();
			CNtlPLPropertyContainer::GetInstance()->SaveSerialize();
			CNtlPLPropertyContainer::GetInstance()->SaveSerialize(TRUE, ENC_DATA_PASSWORD);
		}*/
		//else if (LOAD_PROP_MODE == DATA_MODE_BIN)
		bLoadPropertyContainer = CNtlPLPropertyContainer::GetInstance()->LoadSerialize();//Set this for loading the property files from xml. Change this back
		//else if (LOAD_PROP_MODE == DATA_MODE_ENC)
		//	bLoadPropertyContainer = CNtlPLPropertyContainer::GetInstance()->LoadSerialize(TRUE, ENC_DATA_PASSWORD);//Set this for loading the property files
		//else
		//	bLoadPropertyContainer = CNtlPLPropertyContainer::GetInstance()->Load();
	}
	...
	case eDBO_SERVICE_PUBLISHER_CJI_KOR:
		{
			...
			CDboEventGenerator::StageCreate(SECURITY_STAGE_NAME);
			break;
		}
	case eDBO_SERVICE_PUBLISHER_DBONU:
		{
			GetDboGlobal()->LoadingContractInfo();
			CDboEventGenerator::StageCreate(DBONU_STAGE_NAME);
			break;
		}
	default:
		{
			NTL_ASSERT(false, "CDboApplication::CreateSubSystem, Invalid Service publisher type");
			NTL_RETURN(FALSE);
		}
	}
...
#define GET_KEY_REPEAT(lParam)	(lParam & 0x0000ffff)
	...
	// lleo52 - 추후 삭제 필요
	{
		...
		static sMovieInfo sInfo = { false, 0, 0, 200, 200, true, true, ".\\movie\\AGE1000_DBOUR.mp4" };
...
{
	// network process
	// picking 처리 문제 때문에 input을 나중에 둔다.
	// 빠른 picking 처리로 인하여 rendering시 culling된 atomic을 pick atomic container에 담아 둔다.
	// network이 input보다 먼저 처리되는 경우 서버에서 spawn object를 삭제할 경우 atomic이 삭제될 수 있다.
...
void CDboApplication::DisplayInfo()
{
	...
	{
		CDboGlobal::GetInstance()->ToggleDebugDisplay(); 
	}
...
RwBool CDboApplication::Create( HINSTANCE hInstance, RwInt32 posX, RwInt32 posY, RwInt32 sizeX, RwInt32 sizeY, RwBool bFullScreen )
{
	...
	// Debug 모드는 WinMain에서 이 함수를 호출한 bFullScreen 변수를 참조하여 윈도우를 생성한다.
#ifndef NDEBUG
	RwInt32 nPosX = 0;
	RwInt32 nPosY = 0;
...
#else
	// Release 버전에서는 사용자의 컴퓨터에서 옵션을 읽어와 적용한다.
	...
	return CNtlApplication::Create( hInstance, nPosX+1, nPosY, nScreenWidth, nScreenHeight, !bUserScreen );
#endif
}