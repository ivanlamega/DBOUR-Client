...
#define VIRTUAL_AVATAR_LEVEL		55
...
//DWORD GetClassFlag(int Class, int ItemType)
//{
//
									...
									RwUInt8 byFace, RwUInt8 byHair, RwUInt8 byHairColor, RwUInt8 bySkinColor, RwUInt8 byBlood)
{
	...
	wcscpy_s(pPcData->wchName, NTL_MAX_SIZE_CHAR_NAME_UNICODE, pName);
...
void CDboVirtualServer::RecvServerCommand(void *pPacket)
{
	...
	while(1)
	{
		...
		case SERVER_CMD_KEY:
			...
			else if (strToken == "@allskills")
			{
				strToken = lexer.PeekNextToken(NULL, &iLine);
				unsigned int tblidx = (unsigned int)atof(strToken.c_str());
				for (auto it = API_GetTableContainer()->GetSkillTable()->Begin(); it != API_GetTableContainer()->GetSkillTable()->End(); it++)
				{
					sSKILL_TBLDAT* skillData = (sSKILL_TBLDAT*)it->second;
					if (skillData != NULL)
					{
						SVirtualPcData* myClass = FindPcData(m_bySelCharId);
						
						//
						//if (GetClassFlag(myClass->byClass, ITEM_TYPE_UNKNOWN) == skillData->dwPC_Class_Bit_Flag)
						//{
						//	if (skillData->bySkill_Grade == 1 && skillData->bySkill_Class != eSKILL_CLASS::NTL_SKILL_CLASS_HTB )
						//	{
						//		//sLog.outDetail("Learn the skill \n");
						//		SendCharLearnSkillRes(skillData->tblidx);
						//	}
						//}
					}
				}
...
void CDboVirtualServer::SendLoginCreateUserRes(void *pPacket)
{
	...
	memcpy(sPacket.awchUserId, pRecv->awchUserId, NTL_MAX_SIZE_USERID_UNICODE);
...
void CDboVirtualServer::SendServerListRes(void *pPacket)
{
	...
	wcscpy_s(sPackeFarmInfo.serverFarmInfo.wszGameServerFarmName, NTL_MAX_SIZE_SERVER_FARM_NAME_UNICODE, L"Test Farm");
...
void CDboVirtualServer::SendOneServerRes(void *pPacket)
{
	...
	sPackeFarmInfo.wOpCode = CU_SERVER_FARM_INFO;
	sPackeFarmInfo.serverFarmInfo.serverFarmId = 0;
	wcscpy_s(sPackeFarmInfo.serverFarmInfo.wszGameServerFarmName, NTL_MAX_SIZE_SERVER_FARM_NAME_UNICODE, L"TestServer");
...
void CDboVirtualServer::SendCharAddRes(void *pPacket)
{
	...
	memcpy(sPacket.sPcDataSummary.awchName, pRecv->awchCharName, sizeof(wchar_t)* NTL_MAX_SIZE_CHAR_NAME_UNICODE);
...
void CDboVirtualServer::SendConnectWaitCheckRes(void *pPacket)
{
	sCU_CONNECT_WAIT_CHECK_RES sPacket;
	memset(&sPacket, 0, sizeof(sCU_CONNECT_WAIT_CHECK_RES));
...
void CDboVirtualServer::SendConnectWaitCountNfy(void)
{
	sCU_CONNECT_WAIT_COUNT_NFY sPacket;
	memset(&sPacket, 0, sizeof(sCU_CONNECT_WAIT_COUNT_NFY));

	sPacket.wOpCode = CU_CONNECT_WAIT_COUNT_NFY;
	sPacket.dwCountWaiting = 0;
...
void CDboVirtualServer::SendAvatarCharInfo(void)
{
	...
	memcpy(sPacket.sPcProfile.awchName, pPcData->wchName, sizeof(WCHAR)*NTL_MAX_SIZE_CHAR_NAME_UNICODE); 
	...
	sPacket.sPcProfile.dwSpPoint = 40;
...
void CDboVirtualServer::SendAvatarItemInfo(void)
{
	...
	sPacket.byItemCount = 1;

	sPacket.aItemProfile[0].handle = AcquireSerialId();
	sPacket.aItemProfile[0].tblidx = 19901;
	...
	sPacket.aItemProfile[1].tblidx = 13001;
	...
	sPacket.aItemProfile[1].byCurDur = 100;

	sPacket.aItemProfile[2].handle = AcquireSerialId();
	sPacket.aItemProfile[2].tblidx = 19905;
	sPacket.aItemProfile[2].byPlace = CONTAINER_TYPE_BAG1;
	sPacket.aItemProfile[2].byPos = 1;
	sPacket.aItemProfile[2].byStackcount = 1;
	sPacket.aItemProfile[2].byRank = 1;
	sPacket.aItemProfile[2].byCurDur = 100;

	sPacket.aItemProfile[3].handle = AcquireSerialId();
	sPacket.aItemProfile[3].tblidx = 13002;
	sPacket.aItemProfile[3].byPlace = CONTAINER_TYPE_BAG1;
	sPacket.aItemProfile[3].byPos = 2;
	sPacket.aItemProfile[3].byStackcount = 1;
	sPacket.aItemProfile[3].byRank = 1;
	sPacket.aItemProfile[3].byCurDur = 100;

	sPacket.aItemProfile[4].handle = AcquireSerialId();
	sPacket.aItemProfile[4].tblidx = 13003;
	sPacket.aItemProfile[4].byPlace = CONTAINER_TYPE_BAG1;
	sPacket.aItemProfile[4].byPos = 3;
	sPacket.aItemProfile[4].byStackcount = 0;
	sPacket.aItemProfile[4].byRank = 1;
	sPacket.aItemProfile[4].byCurDur = 100;

	sPacket.aItemProfile[5].handle = AcquireSerialId();
	sPacket.aItemProfile[5].tblidx = 14001;
	sPacket.aItemProfile[5].byPlace = CONTAINER_TYPE_BAG1;
	sPacket.aItemProfile[5].byPos = 4;
	sPacket.aItemProfile[5].byStackcount = 0;
	sPacket.aItemProfile[5].byRank = 4;
	sPacket.aItemProfile[5].byCurDur = 100;
	sPacket.aItemProfile[5].byGrade = 15;

	sPacket.aItemProfile[6].handle = AcquireSerialId();
	sPacket.aItemProfile[6].tblidx = 14002;
	sPacket.aItemProfile[6].byPlace = CONTAINER_TYPE_BAG1;
	sPacket.aItemProfile[6].byPos = 5;
	sPacket.aItemProfile[6].byStackcount = 0;
	sPacket.aItemProfile[6].byRank = 4;
	sPacket.aItemProfile[6].byCurDur = 100;
	sPacket.aItemProfile[6].byGrade = 15;

	sPacket.aItemProfile[7].handle = AcquireSerialId();
	sPacket.aItemProfile[7].tblidx = 14003;
	sPacket.aItemProfile[7].byPlace = CONTAINER_TYPE_BAG1;
	sPacket.aItemProfile[7].byPos = 6;
	sPacket.aItemProfile[7].byStackcount = 0;
	sPacket.aItemProfile[7].byRank = 4;
	sPacket.aItemProfile[7].byCurDur = 100;
	sPacket.aItemProfile[7].byGrade = 15;

	sPacket.aItemProfile[8].handle = AcquireSerialId();
	sPacket.aItemProfile[8].tblidx = 15001;
	sPacket.aItemProfile[8].byPlace = CONTAINER_TYPE_BAG1;
	sPacket.aItemProfile[8].byPos = 7;
	sPacket.aItemProfile[8].byStackcount = 0;
	sPacket.aItemProfile[8].byRank = 1;
	sPacket.aItemProfile[8].byCurDur = 100;

	sPacket.aItemProfile[9].handle = AcquireSerialId();
	sPacket.aItemProfile[9].tblidx = 15002;
	sPacket.aItemProfile[9].byPlace = CONTAINER_TYPE_BAG1;
	sPacket.aItemProfile[9].byPos = 8;
	sPacket.aItemProfile[9].byStackcount = 0;
	sPacket.aItemProfile[9].byRank = 1;
	sPacket.aItemProfile[9].byCurDur = 100;

	sPacket.aItemProfile[10].handle = AcquireSerialId();
	sPacket.aItemProfile[10].tblidx = 15003;
	sPacket.aItemProfile[10].byPlace = CONTAINER_TYPE_BAG1;
	sPacket.aItemProfile[10].byPos = 9;
	sPacket.aItemProfile[10].byStackcount = 0;
	sPacket.aItemProfile[10].byRank = 1;
	sPacket.aItemProfile[10].byCurDur = 100;
	
	sPacket.aItemProfile[11].handle = AcquireSerialId();
	sPacket.aItemProfile[11].tblidx = 4400205;
	sPacket.aItemProfile[11].byPlace = CONTAINER_TYPE_BAG1;
	sPacket.aItemProfile[11].byPos = 10;
	sPacket.aItemProfile[11].byStackcount = 1;
	sPacket.aItemProfile[11].byRank = 1;
	sPacket.aItemProfile[11].byCurDur = 100;

	sPacket.aItemProfile[12].handle = AcquireSerialId();
	sPacket.aItemProfile[12].tblidx = 5002001;
	sPacket.aItemProfile[12].byPlace = CONTAINER_TYPE_BAG1;
	sPacket.aItemProfile[12].byPos = 11;
	sPacket.aItemProfile[12].byStackcount = 1;
	sPacket.aItemProfile[12].byRank = 4;
	sPacket.aItemProfile[12].byCurDur = 100;
	sPacket.aItemProfile[12].byGrade = 15;*/

	/*sPacket.aItemProfile[13].handle = AcquireSerialId();
	sPacket.aItemProfile[13].tblidx = 5000001;
	sPacket.aItemProfile[13].byPlace = CONTAINER_TYPE_BAG1;
	sPacket.aItemProfile[13].byPos = 12;
	sPacket.aItemProfile[13].byStackcount = 1;
	sPacket.aItemProfile[13].byRank = 4;
	sPacket.aItemProfile[13].byCurDur = 100;
	sPacket.aItemProfile[13].byGrade = 15;*/
...
void CDboVirtualServer::SendPlayerCreate(void)
{
	...
	wcscpy_s(sPacket.sObjectInfo.pcBrief.awchName, NTL_MAX_SIZE_CHAR_NAME_UNICODE, L"하하하");
...
void CDboVirtualServer::SendMonsterCreate(void)
{
	...
	sPacket.sObjectInfo.mobState.sCharStateDetail.sCharStateDestMove.vSecondDestLoc.x = sPacket.sObjectInfo.mobState.sCharStateBase.vCurLoc.x + 100;
	sPacket.sObjectInfo.mobState.sCharStateDetail.sCharStateDestMove.vSecondDestLoc.y = sPacket.sObjectInfo.mobState.sCharStateBase.vCurLoc.y;
	sPacket.sObjectInfo.mobState.sCharStateDetail.sCharStateDestMove.vSecondDestLoc.z = sPacket.sObjectInfo.mobState.sCharStateBase.vCurLoc.z;
...
void CDboVirtualServer::SendNpcCreate(RwUInt32 uiTblId, RwReal fDist)
{
	sGU_OBJECT_CREATE sPacket;
	...
	sPacket.sObjectInfo.npcBrief.tblidx = uiTblId;
	sPacket.sObjectInfo.npcBrief.wCurLP = VIRTUAL_MAX_LP;
	sPacket.sObjectInfo.npcBrief.wMaxLP = VIRTUAL_MAX_LP;
...
void CDboVirtualServer::SendCharSitDown(void)
{
	...
	sGU_UPDATE_CHAR_STATE sPacket2;
	memset(&sPacket2, 0, sizeof(sGU_UPDATE_CHAR_STATE));

	sPacket2.wOpCode = GU_UPDATE_CHAR_STATE;
	sPacket2.sCharState.sCharStateBase.byStateID = CHARSTATE_SITTING;
	sPacket2.handle = GetNtlSLGlobal()->GetSobAvatar()->GetSerialID();
	SendEvent(sizeof(sPacket2), &sPacket2);
...
void CDboVirtualServer::SendCharSkillCasting(void)
{
	...
	sGU_CHAR_ACTION_SKILL packet;
	CSkillTable *pSkillTbl = API_GetTableContainer()->GetSkillTable();
	memset(&packet, 0, sizeof(sGU_CHAR_ACTION_SKILL));

	//packet.skillId = SkillNow->tblidx;
...
void CDboVirtualServer::SendCharLearnSkillRes(RwUInt32 uiTblId)
{
	...
	sPacket.wResultCode = GAME_SUCCESS;
	sGU_SKILL_LEARNED_NFY packet2;
	memset(&packet2, 0, sizeof(sGU_SKILL_LEARN_RES));
	...
	for(RwInt32 i = 0; i < iSkillCount; ++i)
	{
		...
		{
			packet2.bySlot = m_iBySkillCount;
	...
	packet2.wOpCode = GU_SKILL_LEARNED_NFY;
	packet2.skillId = uiTblId;
	SendEvent(sizeof(packet2), &packet2);
	SendEvent(sizeof(sPacket), &sPacket);
}
...
//		SendMobMove();
	...
	if(m_bySkillActiveType == VIRTUAL_SKILL_TYPE_CASTING)
	{
		...
		if(timeGetTime() - m_uiSkillTime >= (RwUInt32)(pSkillTblData->dwCastingTimeInMilliSecs))
		{
			SendCharSkillAction();
			m_bySkillActiveType = VIRTUAL_SKILL_TYPE_NONE;
		}
...
void CDboVirtualServer::SendCancelTransform(void *pPacket)
{
	sUG_TRANSFORM_CANCEL_REQ * req = (sUG_TRANSFORM_CANCEL_REQ*)pPacket;
	
	sGU_TRANSFORM_CANCEL_RES sPacket;
	memset(&sPacket, 0, sizeof(sGU_TRANSFORM_CANCEL_RES));

	sPacket.wOpCode = GU_TRANSFORM_CANCEL_RES;
	sPacket.wResultCode = GAME_SUCCESS;

	//Update Char State
	sGU_UPDATE_CHAR_ASPECT_STATE packet2;
	memset(&packet2, 0, sizeof(sGU_UPDATE_CHAR_ASPECT_STATE));

	packet2.handle = m_uiAvatarSerialId;
	packet2.wOpCode = GU_UPDATE_CHAR_ASPECT_STATE;
	packet2.aspectState.sAspectStateBase.byAspectStateId = 255;//Don't see any Const then i send 0 because i'm not going transform ^^
...
void CDboVirtualServer::PacketProc(void *pPacket)
{
	...
	case UC_CONNECT_WAIT_CHECK_REQ:
		SendConnectWaitCheckRes(pPacket);
		SendConnectWaitCountNfy();
		break;
	...
	case UG_ENTER_WORLD:
		...
		break;
	case UG_CHAR_MOVE:
		NtlLogFilePrint("UG_CHAR_MOVE:");
		break;
	case UG_CHAR_DEST_MOVE:
		NtlLogFilePrint("UG_CHAR_DEST_MOVE:");
		break;
	...
	case UG_TRANSFORM_CANCEL_REQ:
		SendCancelTransform(pPacket);
		break;
	case UG_PARTY_CREATE_REQ:
		//SendCreatePartyReq(pPacket);
		break;

	default:
		char buffer[255];
		sprintf(buffer,"Vitrualserver PacketProc unknown opcode %i", wOpCode);
