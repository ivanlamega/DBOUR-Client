...
#else
	strcpy(data.chStageName, pStageName); 
#endif
...
void CDboEventGenerator::NetConnectBoxShow( const WCHAR *pString, RwInt32 eWorkId /* = MBW_NONE */, RwUInt32 flagStyle /* = MBTF_NONE */,
										   RwReal fShowTime /* = 0.0f */, sMsgBoxData* pData /* = NULL  */, std::list<sMsgBoxCustomBtn>* pCustomListData /* = NULL */,
										   RwBool bAcceptDuplicate /* = TRUE */,  RwInt32 eStringID /* = -1 */, RwBool bUpdate /* = FALSE */ )
{
	...
	data.eStringID	= eStringID;
	data.eWorkId	= eWorkId;
	data.flagStyle	= flagStyle;	
	data.fShowTime	= fShowTime;
	data.bAcceptDuplicate = bAcceptDuplicate;
	data.pData		= pData;
	data.plistCustomBtn = pCustomListData;
	data.bUpdate	= bUpdate;
	
	RWS::CMsg msg;
	msg.Id = g_EventConnectBoxShow;
...
void CDboEventGenerator::MsgBoxShow( const WCHAR *pString, RwInt32 eWorkId, RwUInt32 flagStyle, RwBool bPopup /* = FALSE */, RwReal fShowTime /* = 0.0f */, 
									 sMsgBoxData* pData /* = NULL */, std::list<sMsgBoxCustomBtn>* plistCustomBtn /* = NULL  */,  RwBool bAcceptDuplicate /* = TRUE */, RwInt32 eStringID /* = -1 */, RwBool bUpdate /* = FALSE */ )
{
	SDboEventMsgBoxShow data;
...
void CDboEventGenerator::MsgBoxResult( RwInt32 eResult, RwUInt32 uiWorkId, sMsgBoxData* pData )
{
	SDboEventMsgBoxResult data;
	data.eResult = eResult;
	data.uiWorkId = uiWorkId;
	data.pData = pData;

	RWS::CMsg msg;
	msg.Id = g_EventMsgBoxResult;
	msg.pData = reinterpret_cast<void*>(&data);
...
void CDboEventGenerator::SayMessage(RwUInt8 byMsgType, const WCHAR *pUserName, RwUInt16 wMsgLen, const WCHAR *pMessage, RwUInt32 hObject /* = INVALID_SERIAL_ID */, bool bIsGameMaster)
{
	SDboEventSayMessage data;
	memset(&data, 0, sizeof(data));
	data.byMsgType = byMsgType;
	data.uiSerial = hObject;
	data.bIsGameMaster = bIsGameMaster;
	if( pUserName )
		memcpy(data.wchUserName, pUserName, sizeof(WCHAR)*wcslen(pUserName)); 

	if( pMessage )
		memcpy(data.wchMessage, pMessage, sizeof(WCHAR)*wMsgLen);
...
void CDboEventGenerator::TimeFixedNotifyMessage(RwBool bActive, RwUInt32 uiStringID, const WCHAR* pString)
{
	SDboEventTimeFixedNotify data;
	memset(&data, 0, sizeof(data));
	data.bActive = bActive;
	data.uiStringID = uiStringID;
...
void CDboEventGenerator::CautionSideNotify( RwBool bActive, RwUInt32 uiStringID, const WCHAR* pString, RwReal fLifeTime ) 
{
...
void CDboEventGenerator::NotifyMessage( RwInt32 eType, const WCHAR* pMessage )
{
	...
	memset( &data, 0, sizeof( data ) );
	data.eType = eType;
	memcpy( data.wchMessage, pMessage, sizeof( WCHAR ) * NTL_MAX_LENGTH_OF_CHAT_MESSAGE_UNICODE );
	
	RWS::CMsg msg;
	msg.Id = g_EventNotify;
	msg.pData = reinterpret_cast<void*>( &data );
...
void CDboEventGenerator::CalcPopupShow(RwBool bShow, RwUInt32 uiSerial, RwInt32 nSrcPlace, RwInt32 nXPos, RwInt32 nYPos, RwUInt32 uiMaxValue)
{
...
void CDboEventGenerator::PartyCharm(RwUInt32 iType, RwInt32 iValue /* = 0 */, RwUInt32 uiValue2 /* = 0 */)
{
	RWS::CMsg msg;
	SDboEventPartyCharm sPacket;

	sPacket.iType = iType;
	sPacket.iValue = iValue;
	sPacket.uiValue2 = uiValue2;

	msg.Id = g_EventCharm;
	msg.pData = (void*)&sPacket;

	_SendMsg(msg);
}

void CDboEventGenerator::PartyVictim(RwUInt32 iType, RwUInt32 uiValue /* = 0 */, RwUInt32 uiValue2 /* = 0 */,
									RwUInt32 uiValue3 /* = 0 */, RwUInt32 uiValue4 /* = 0 */, RwBool bByMyself /* = false */)
{
	RWS::CMsg msg;
	SDboEventPartyVictim sPacket;

	sPacket.iType = iType;
	sPacket.uiValue = uiValue;
	sPacket.uiValue2 = uiValue2;
	sPacket.uiValue3 = uiValue3;
	sPacket.uiValue4 = uiValue4;
	sPacket.bByMyself = bByMyself;

	msg.Id = g_EventCharmVictim;
	msg.pData = (void*)&sPacket;

	_SendMsg(msg);
}

void CDboEventGenerator::PartyInventory(RwUInt32 iType, RwUInt8 byPos, RwUInt32 uiSerial /* = INVALID_SERIAL_ID */, RwUInt32 uiCount /* = 0 */, RwUInt8 byRank /* = 0 */,
										RwUInt32 uiOptionSerial1 /* = INVALID_TBLIDX */, RwUInt32 uiOptionSerial2 /* = INVALID_TBLIDX */, RwBool bTrue /* = false */,
										RwUInt8 byGrade /* = 0 */, RwUInt8 byBattleAttribute /*= BATTLE_ATTRIBUTE_UNKNOWN */)
{
	RWS::CMsg msg;
	SDboPartyInvetoryItem sPacket;

	sPacket.iType				= iType;
	sPacket.uiSerial			= uiSerial;
	sPacket.byPos				= byPos;
	sPacket.uiStackCount		= uiCount;
	sPacket.byRank				= byRank;
	sPacket.uiOptionSerial1		= uiOptionSerial1;
	sPacket.uiOptionSerial2		= uiOptionSerial2;
	sPacket.bTrue				= bTrue;
	sPacket.byGrade				= byGrade;
	sPacket.byBattleAttribute	= byBattleAttribute;

	msg.Id = g_EventPartyInventory;
	msg.pData = (void*)&sPacket;
...
void CDboEventGenerator::ItemUpgradeResult( SDboItemUpgradeResult::RESULT eResult, RwUInt8 byBattleAttribute /* = 0 */, RwInt32 nGrade /* = 0  */ )
{
	SDboItemUpgradeResult sResult;
	sResult.eResult				= eResult;
	sResult.nGrade				= nGrade;
	sResult.byBattleAttribute	= byBattleAttribute;
	
	RWS::CMsg msg;
	msg.Id = g_EventItemUpgradeResult;
	msg.pData = &sResult;
	_SendMsg( msg );
}
...
void CDboEventGenerator::RegistItemUpgrade( RwUInt32 hSerialID, RwUInt32 eSrcPlace, RwUInt32 uiSlotIdx )
{
	...
	RWS::CMsg msg;
	msg.Id = g_EventRegisterItemUpgrade;
	msg.pData = &sData;
	_SendMsg( msg );
}

void CDboEventGenerator::ScouterUpgradePredict( RwUInt8 byPosNum, RwUInt8 byRetRate )
{
	SDboScouterUpgradePredict sPredict;
	sPredict.byPosNum = byPosNum;
	sPredict.byRetRate = byRetRate;

	RWS::CMsg msg;
	msg.Id = g_EventScouterUpgradePredict;
	msg.pData = &sPredict;
	_SendMsg( msg );
}

void CDboEventGenerator::ESCinNarrationMode(void)
{
	RWS::CMsg msg;
	msg.Id = g_EventESCinNarrationMode;
	msg.pData = NULL;
	_SendMsg( msg );
}
...
void CDboEventGenerator::QuickSlotDelNfy( RwUInt8 bySlotIdx )
{
	SDboEventQuickSlotDelNfy sData;
	sData.bySlotIdx = bySlotIdx;

	RWS::CMsg msg;
	msg.Id = g_EventQuickSlotDelNfy;
...
void CDboEventGenerator::NPCDialogOpen( RwUInt32 hSerialID, RwInt32 eDialog )
{
	...
	RWS::CMsg msg;
	msg.Id = g_EventNPCDialogOpen;
	msg.pData = reinterpret_cast<void*>( &sNPCDialog );
	_SendMsg( msg );
}
...
void CDboEventGenerator::MapEvent(RwInt32 iMessage, RwUInt32 uiValue /* = 0 */)
{
	...
	RWS::CMsg msg;
	msg.Id = g_EventMap;
	msg.pData = reinterpret_cast<void*>( &sEventMap );
	_SendMsg( msg );
}
									...
									RwUInt32 uiHandle /* = 0xffffffff*/, RwUInt8 byCount /* = 0*/, VOID* pData /* = NULL */)
{
	...
	RWS::CMsg msg;
	msg.Id = g_EventUserTrade;
	msg.pData = reinterpret_cast<void*>( &sEventUserTrade );
	_SendMsg( msg );
}
...
void CDboEventGenerator::ServerCharDialog(RwUInt32 uiSerial, RwUInt8 byDialogType, RwUInt32 uiTextTblIdx, RwBool bIsRefSpeechTable, RwUInt16 wTextLen, WCHAR* pTextData)
{
	...
	RWS::CMsg msg;
	msg.Id = g_EventServerCharDialog;
	msg.pData = reinterpret_cast<void*>( &sCharDialog );
	_SendMsg( msg );
}
...
void CDboEventGenerator::OpenHelpWindow(RwUInt32 uiTableIndex)
{
	...
	RWS::CMsg msg;
	msg.Id		= g_EventOpenHelpWindow;
	msg.pData	= reinterpret_cast<void*>(&sOpenHelpWindow);
	_SendMsg(msg);
}
...
void CDboEventGenerator::PrivateShopBusinessStart( RwUInt8 uiType, RwUInt32 uiShopOwner, WCHAR* pwcTargetName, sPRIVATESHOP_ITEM_DATA* pPrivateShopItemData, void* pGui ) 
{
	...
	swprintf_s(sPrivateShopBusinessStart.awcTargetName, NTL_MAX_SIZE_CHAR_NAME_UNICODE+1, L"%s", pwcTargetName);
...
void CDboEventGenerator::OpenSendMail(const WCHAR* pwcTargetName)
{
	...
	wcscpy_s(sOpenSendMail.awcTargetName, NTL_MAX_SIZE_CHAR_NAME_UNICODE + 1, pwcTargetName);
...
void CDboEventGenerator::HoipoiMixItemCheck( HOBJECT hObject, TBLIDX recipeTblIdx )
{
	SDboEventHoipoiMixItemCheckRes sData;
	sData.hObject = hObject;
	sData.idxRecipeTbl = recipeTblIdx;

	SEND_MSG( g_EventHoipoiMixItemCheckRes, &sData );
}

void CDboEventGenerator::HoipoiMixItemMake( RwUInt16 wResultCode, RwUInt8 bySuccessResult, HOBJECT hObject, TBLIDX idxRecipe, SERIAL_HANDLE hItem )
{
	SDboEventHoipoiMixItemMakeRes sData;
	sData.wResultCode = wResultCode;
	sData.bySuccessResult = bySuccessResult;
	sData.hObject = hObject;
...
void CDboEventGenerator::HoipoiMixItemMakeEp()
{	
	SEND_MSG( g_EventHoipoiMixItemMakeEpRes, NULL );
}
...
void CDboEventGenerator::GmtInfo( RwUInt8 byGMTResetCount, sGAME_MANIA_TIME_DATA& sCurrent, sGAME_MANIA_TIME_DATA& sNext )
{
	SDboEventGmtInfo sData;
	memset( &sData, 0, sizeof(SDboEventGmtInfo) );

	sData.byGMTResetCount = byGMTResetCount;
	memcpy( &sData.sCurrent, &sCurrent, sizeof( sGAME_MANIA_TIME_DATA ) );
	memcpy( &sData.sNext, &sNext, sizeof( sGAME_MANIA_TIME_DATA ) );

	SEND_MSG( g_EventGmtInfo, &sData );
}

void CDboEventGenerator::GmtUpdate( sGAME_MANIA_TIME& sNext )
{
	SDboEventGmtUpdate sData;
	memset( &sData, 0, sizeof(SDboEventGmtUpdate) );

	memcpy( &sData.sNext, &sNext, sizeof( sGAME_MANIA_TIME ) );

	SEND_MSG( g_EventGmtUpdate, &sData );
}

void CDboEventGenerator::GmtChanged( sGAME_MANIA_TIME& sCurrent )
{
	SDboEventGmtChanged sData;
	memset( &sData, 0, sizeof(SDboEventGmtChanged) );

	memcpy( &sData.sCurrent, &sCurrent, sizeof( sGAME_MANIA_TIME ) );

	SEND_MSG( g_EventGmtChanged, &sData );
}

void CDboEventGenerator::GmtResetOpen( RwBool bOpen, RwInt32 nLeft, RwInt32 nTop )
{
	SDboEventGmtResetOpen sData;
	memset( &sData, 0, sizeof(SDboEventGmtResetOpen) );
	
	sData.bOpen = bOpen;
	sData.nLeft = nLeft;
	sData.nTop = nTop;

	SEND_MSG( g_EventGmtResetOpen, &sData );
}