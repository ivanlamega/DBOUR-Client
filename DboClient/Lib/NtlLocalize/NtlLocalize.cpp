...
unsigned char	g_byLocalType = NTL_LOCALIZE_DEV; // NTL_LOCALIZE_DEV, NTL_LOCALIZE_CJ
...
bool Local_DetectDataCJ(const char *pKey)
{
	bool bSuccess = DetectLocalConfigCJ(g_pLocalConfig, pKey);
	if(bSuccess)
		g_byLocalType = NTL_LOCALIZE_CJ;
...
const char*	Local_DBOEncryptConfig(void)
{
	...
	if (g_pLocalConfig->byLocalType == DBONU_LOCALIZE)
		return NULL;
