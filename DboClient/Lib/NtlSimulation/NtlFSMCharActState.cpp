...
void CNtlFSMCharActIdleState::Enter(void)
{
	...
	if(!m_bWaterIn)
	{
		...
		if(m_pActor->GetFlags() & SLFLAG_OBJECT_COLLISION && !Logic_IsGroundFlyActor(m_pActor))
		{
			// 상태 chcek.
			RwV3d vPos = m_pActor->GetPosition();
			SWorldHeightStuff sHStuff;
			...
			if(vPos.y < sHStuff.fFinialHeight)
 			...
 			if(vPos.y >= sHStuff.fFinialHeight && vPos.y <= sHStuff.fFinialHeight + 0.1f)
				ChangeIdleState(STATE_IDLE_SLIP);
			else
				ChangeIdleState(STATE_IDLE_TOPDOWN);

            // 트리거 오브젝트 위에 있는 경우에는 오브젝트가 사라질수도 있기때문에 높이 체크를 한다.
...
{
	// Vehicle driver 인 경우 기본적인 Update 처리만 한다
...
RwUInt32 CNtlFSMCharActIdleState::HandleEvents(RWS::CMsg &pMsg)
{
	...
	if(pMsg.Id == g_EventSobEquipChange)
	{
		...
		if(uiCurrAnimKey == NML_IDLE_01 || uiCurrAnimKey == NML_IDLE_02 || 
			uiCurrAnimKey == NML_STAFF_IDLE_01 || uiCurrAnimKey == NML_IDLE_02 || uiCurrAnimKey == SWIM_IDLE)
		{
	...
	else if(pMsg.Id == g_EventTerrainClick || pMsg.Id == g_EventKeyboardMove || 
			pMsg.Id == g_EventKeyboardMoveStop)
	{
...
RwBool CNtlFSMCharActIdleState::CheckSwimming(void)
{
	...
	if(bWaterIn)
	{
		if(vPos.y >= sHStuff.fFinialHeight + 0.1f)
			ChangeIdleState(STATE_IDLE_TOPDOWN);
		else
			m_bWaterIn = bWaterIn;
	}
...
void CNtlFSMCharActIdleState::UpdateSlipping(RwReal fElapsed)
{
	...
	if(!CheckSwimming())
    {
        ...
        if( m_pActor->GetFlags() & SLFLAG_FALLING_ENABLE && 
            vNewPos.y - sHstuff.fFinialHeight > FALLING_CHECK_LEN)
        {
            CNtlBeCharData *pBeData = reinterpret_cast<CNtlBeCharData*>(m_pActor->GetBehaviorData()); 
            ...
            pCtrlStuff->uExtra.sFalling.byMoveFlags		= NTL_MOVE_NONE;
...
void CNtlFSMCharActIdleState::UpdateCalcHeight( RwReal fElapsed ) 
{
    ...
    if(m_fCalcHeightTime > 0.5f)        // 0.5초 단위로 계산한다.
    {
        m_fCalcHeightTime = 0.0f;
        ...
        if(abs(vPos.y - sHStuff.fFinialHeight) > 0.3f) // 실제 높이의 차이가 0.3보다 크면
        {
            m_pActor->SetActorFlags(m_pActor->GetActorFlags() & ~SLFLAG_ACTOR_CALC_HEIGHT);
            Finish();
            SetNextStateId(NTL_FSMSID_FALLING);
...
void CNtlFSMCharActMoveState::Update(RwReal fElapsed)
{
	...
	if(!IsFinish())
	{
        // 물인지를 검사하여 물이면 수영 상태로 간다. (물이 없는 경우는 Height가 -9999.0f이다)
...
void CNtlFSMCharActMoveSwimming::Update(RwReal fElapsed)
{
	...
	if(!IsFinish())
	{
		// 물인지를 검사하여 물이 아니면 ground 이동 상태로 간다.
...
void CNtlFSMCharActAdjustMoveState::Enter(void)
{
	...
	RwV3d vPos = m_pActor->GetPosition(); 
	RwV3d vDir;
	RwV3dSub(&vDir, &pMoveStuff->vDest, &vPos);
	vDir.y = 0.0f;
...
// 형석
//	dash 도중 여러가지 action을 queuing 해야 하므로 문제가 생겼슴.
...
void CNtlFSMCharActDashState::Update(RwReal fElapsed)
{
	...
	{
		if(m_pGroundMove->GetDashState() == CNtlBehaviorCharGroundMove::DASH_STATE_END)
		{
...
RwUInt32 CNtlFSMCharActJumpState::HandleEvents(RWS::CMsg &pMsg)
{
	...
	else if(pMsg.Id == g_EventKeyboardMoveStop)
	{
		...
		{
			pMoveStuff->byMoveFlags = JumpKeyStopMapping(pMoveStuff->byMoveFlags);
		}
	...
	{
		// 현재 점프 상태이기 때문에.. 점프를 끝내라고 하면... 바로 끝내지 못한다.
		// 따라서 점프중에 이동 명령이 오면...  queueing을 해야 한다.
...
{
	CNtlFSMCharActStateBase::Enter();
	//Change Aura color for charging here. 
	CNtlSobProxy *pSobProxy = m_pActor->GetSobProxy();
	...
	if(m_pActor->GetFlags() & SLFLAG_SERVER_SENDER)
	{
		...
		if(m_pPLEntity == NULL)
		{
			m_pPLEntity = m_pActor->GetSobProxy()->CreatePLChildEffect("COM_SKL_AVE_002", "nullroot");
		}
...
{
	// 아무것도 처리하지 않으므로써 상태를 빠져나가지 않게 한다.
...
void CNtlFSMCharActFightingFollowState::Enter(void)
{
    ...
    {
        API_GetSLPacketGenerator()->SendCharFollowMove(pCtrlStuff->hTargetSerialId, pCtrlStuff->fFollowRange, NTL_FOLLOW_FIGHTING, m_pActor->GetServerSyncAvatarType());
...
void CNtlFSMCharActFightingPoseState::SetPose2IdleAnim() 
{
    ...
    if(uiClassId == SLCLASS_AVATAR || uiClassId == SLCLASS_PLAYER)
    {
        ...
        else if(pItemTblData && Logic_IsEquipedStaffWeapon(pItemTblData))    // 스태프를 들고 있을때
        {
...
void CNtlFSMCharActKnockDownState::Enter(void)
{
	...
	{
		// 상태의 끝으로 바로 갈 경우.
		// 서버에서 캐릭터 생성할 때 상태가 knockdown 상태일 경우.
...
void CNtlFSMCharActStunState::Enter(void)
{
    ...
    case DBO_STUN_TYPE_CANDY:
        // 캔디는 이벤트를 쏘기 때문에 Enter에서 처리하지 않는다.        
        break;
    }        
}
...
{
    CNtlBeCharData *pBeData = reinterpret_cast<CNtlBeCharData*>(m_pActor->GetBehaviorData()); 
    switch(pBeData->GetCtrlStuff()->uExtra.sStun.byStunType)
    {
...
CNtlFSMCharActSkillActionState::CNtlFSMCharActSkillActionState()
{
	...
	SetFlags(NTL_FSMSF_NOT_MOVE | NTL_FSMSF_NOT_JUMP | NTL_FSMSF_NOT_SIT | NTL_FSMSF_NOT_SKILLUSE | NTL_FSMSF_NOT_SOCIALUSE | 
			NTL_FSMSF_NOT_CAHNGE_HEADING | NTL_FSMSF_NOT_HURT_STATE | NTL_FSMSF_SKIP_STANDUP | NTL_FSMSF_SKIP_ATTACK_CLICK | NTL_FSMSF_VISUAL_RPBOUNS);
...
void CNtlFSMCharActSkillActionState::Enter(void)
{
    ...
    //////////////////////////////////////////////////////////////////////////
    // 스킬 이펙트들 관련 Behavior
    if(Logic_IsSkillSystemEffectCode(m_uiSkillTblId, ACTIVE_WARP_BACK_ATTACK))        
    {
        CNtlBehaviorCharActiveWarpBackAttack* pSkill = NTL_NEW CNtlBehaviorCharActiveWarpBackAttack();
        AddBehavior(pSkill);
    }
    else if(Logic_IsSkillSystemEffectCode(m_uiSkillTblId, ACTIVE_WARP))    
    {
        CNtlBehaviorCharActiveWarp* pSkill = NTL_NEW CNtlBehaviorCharActiveWarp();
        AddBehavior(pSkill);
    }
    else if(Logic_IsSkillSystemEffectCode(m_uiSkillTblId, ACTIVE_THROUGH_ATTACK))    
    {
        ...
        pSkill->SetAnimation(pSkillTblData->wAction_Animation_Index, 
                             ...
                             pSkillTblData->wAction_End_Animation_Index);
        pSkill->SetRange(pSkillTblData->fUse_Range_Max  + 3.0f);
        AddBehavior(pSkill);
    }
    else if(Logic_IsSkillSystemEffectCode(m_uiSkillTblId, ACTIVE_MONSTER_TRANSFORM))
    {
        // 몹 변신 전용
	...
	if(pSkillTblData->wFunction_Bit_Flag & SKILL_FUNCTION_FLAG_STOP_AUTO_ATTACK)
	{
		if(m_pActor->GetFlags() & SLFLAG_SERVER_SENDER)
		{
			CNtlFSMCharActAgent *pCharActAgent = reinterpret_cast<CNtlFSMCharActAgent*>(GetFSMAgent());
...
void CNtlFSMCharActSkillActionState::Exit(void)
{
    ...
    // 스킬 상태에서 빠져나오면 무조건 서브웨폰은 Deactive한다.
    CNtlPLEventGenerator::AnimEventSubWeaponDeActive(m_pActor->GetSerialID());

    // 캔슬되었거나 넉백당했을때를 위해서 남은 이벤트들을 모두 재생한다.    
...
void CNtlFSMCharKeepingEffect::Enter( void ) 
{
    ...
    if(pSkillTblData)
    {
        if(m_pActor)
        {
            m_pActor->GetSobProxy()->SetBaseAnimation(pSkillTblData->wAction_Loop_Animation_Index);
...
void CNtlFSMCharActHTBState::NextStepProc(void)
{
	...
	if(uiSkillTblIdx != INVALID_TBLIDX)
	{
		...
		if(pActRes->effectResult1.eResultType == DBO_SYSTEM_EFFECT_RESULT_TYPE_GENERAL)
		{
			sDamageStuff.iDamage			= (DWORD)((pActRes->effectResult1).fResultValue) + (DWORD)((pActRes->effectResult2).fResultValue);
			sDamageStuff.iAbsorbedDamage	= 0;
			sDamageStuff.iReflectedDamage	= 0;
		}
		else
		{
			sDamageStuff.iDamage			= (DWORD)((pActRes->effectResult1).DD_DOT_fDamage) + (DWORD)((pActRes->effectResult2).DD_DOT_fDamage);
		...
		FillAttackStuff(pAttStuff, m_pActor->GetSerialID(), pHTBStuff->hTargetSerialId, pActRes->byAttackResult,
						FALSE, 0, pActRes->byBlockedAction, sDamageStuff, vPushDir, NTL_ATTACK_LOGIC_SKILL, FALSE, uiSkillTblIdx, INVALID_SYNCEVENT_ID);
...
RwUInt32 CNtlFSMCharActHTBState::HandleEvents(RWS::CMsg &pMsg)
{
	...
	{
        if(!IsFinish()) // 종료시에는 자동 공격이 가능해야한다.
        {
		    NTL_RETURN(NTL_FSM_EVENTRES_BLOCK);
        }
	}
...
void CNtlFSMCharActHTBSandbagState::Exit(void)
{
    ...
    {
        // 그냥 Faint로 바로 전이하면 넉다운 후에 다시 Faint 애니메이션을 재생하기 때문에, 걍 알파로 처리해서 안보이게 한다. (어차피 죽은거니까)
...
void CNtlFSMCharActItemCastingState::Enter(void)
{
	...
	{
        // casting gui를 위한 event를 보낸다.
		if(wcslen(pUseItemTblData->szCasting_Effect) > 0)
		{
			RwV3d vOffset;
			...
			std::wstring ws(pUseItemTblData->szCasting_Effect);
			std::string effect(ws.begin(), ws.end());
			m_pPLEntity = m_pActor->GetSobProxy()->CreatePLChildEffect(effect.c_str(), vOffset);
		}
...
void CNtlFSMCharActDirectionState::Enter(void)
{
	...
	// 연출 animatin play시 무조건 animation 보간을 끈다(형석)
	// 다르게 생각하는 사람은 물어볼 것.
	CNtlSobCharProxy *pSobProxy = reinterpret_cast<CNtlSobCharProxy*>(m_pActor->GetSobProxy());
	pSobProxy->SetAnimBlendNoneOneTime();
...
RwUInt32 CNtlFSMCharActDirectionState::HandleEvents(RWS::CMsg &pMsg)
{
	...
	else if ( pMsg.Id == g_EventTMQBoard )
	{
		SNtlEventTMQBoard* pEvtData = (SNtlEventTMQBoard*)pMsg.pData;

		if ( pEvtData->uiEventType == TBT_ROOM_SERVER_EXCEPTION )
		{
			SetNextStateId(NTL_FSMSID_IDLE);
			Finish();
		}
...
//////////////////////////////////////////////////////////////////////////
//////////////////////////////////////////////////////////////////////////
DEFINITION_MEMORY_POOL(CNtlFSMCharActScouterState)

CNtlFSMCharActScouterState::CNtlFSMCharActScouterState() 
{
    SetStateId(NTL_FSMSID_SCOUTER);
    SetStateName(NTL_FSMSN_SCOUTER);

    RwUInt32 uiFlag = NTL_FSMSF_NOT_ALL;
    uiFlag &= ~NTL_FSMSF_NOT_HURT_CAN;      // 데미지는 받는다.
    uiFlag &= ~NTL_FSMSF_NOT_HURT_STATE;
    uiFlag &= ~NTL_FSMSF_NOT_STATECHANGE;
    uiFlag &= ~NTL_FSMSF_NOT_INPUT;

    SetFlags(uiFlag);

    m_bIsUIOpen = FALSE;
    m_fFPSInterTime = 0.0f;
    m_bIsAlphaBlend = FALSE;
    m_bExit = FALSE;
    m_hLoopSound = INVALID_SOUND_HANDLE;
    pSystemEffect = NULL;
}

void CNtlFSMCharActScouterState::Enter( void ) 
{
    CNtlFSMCharActStateBase::Enter();
    ...
    {
        // 카메라 변경    
        RwUInt32 nTargetSerialId = m_pActor->GetTargetID();
        CNtlSob* pSobTarget = GetNtlSobManager()->GetSobObject(nTargetSerialId);
        if(nTargetSerialId != INVALID_SERIAL_ID && pSobTarget && pSobTarget->GetClassID() != SLCLASS_AVATAR)
        {
            RwV3d vLookAt = GetNtlSobManager()->GetSobObject(nTargetSerialId)->GetPosition();            
            vLookAt.y += GetNtlSobManager()->GetSobObject(nTargetSerialId)->GetSobProxy()->GetPLEntityHeight() * 0.8f;
            CNtlSLEventGenerator::CameraFPS(GetNtlGameCameraManager()->GetLookAt(), vLookAt);
        }
        else
        {
            // 타겟이 없는 경우에는 현재 방향에서 수평만 맞춘다.
            RwV3d vLookAt = GetNtlGameCameraManager()->GetLookAt() - *GetNtlGameCameraManager()->GetCameraPos();
            vLookAt.y = 0.0f;
            RwV3dNormalize(&vLookAt, &vLookAt);                
            vLookAt = m_pActor->GetPosition() + vLookAt * 30.0f;
            vLookAt.y = m_pActor->GetPosition().y + m_pActor->GetSobProxy()->GetPLEntityHeight() * 0.8f;        

            if(GetNtlGameCameraManager()->GetLookAt().y > GetNtlGameCameraManager()->GetCameraPos()->y)
            {
                CNtlSLEventGenerator::CameraFPS(vLookAt, vLookAt);
            }
            else
            {
                CNtlSLEventGenerator::CameraFPS(GetNtlGameCameraManager()->GetLookAt(), vLookAt);
		...
		tSoundParam.pcFileName		= WORLD_SOUND_SCOUTER_LOOP;
		tSoundParam.bLoop			= true;

		GetSoundManager()->Play(&tSoundParam);

		m_hLoopSound = tSoundParam.hHandle;

        // 서버 전송
        API_GetSLPacketGenerator()->SendScouterOnOff(TRUE);
    ...
    SetAnim(NML_SCOUTER_ON_OFF);    
}

void CNtlFSMCharActScouterState::Exit( void ) 
{
    CNtlFSMCharActStateBase::Exit();

    DestroyEffect();
    
    if(!m_bExit)     // 몹에게 공격받아서 상태가 전이되는 경우
    {
        API_GetSLPacketGenerator()->SendScouterOnOff(FALSE);

        // UI에 이벤트를 보낸다.
        CNtlSLEventGenerator::ScouterEvent(SCOUTER_EVENT_FSM_EXIT, m_pActor->GetSerialID());
    }
}

void CNtlFSMCharActScouterState::Update( RwReal fElapsed ) 
{
    if(!m_bIsUIOpen)        // UI는 여기서 연다
    {
        CNtlSLEventGenerator::ScouterEvent(SCOUTER_EVENT_FSM_ENTER);
        m_bIsUIOpen = TRUE;
    }

    if(!m_bExit && !m_bIsAlphaBlend && m_pActor->GetClassID() == SLCLASS_AVATAR)
    {
        if(m_fFPSInterTime < FPS_INTERPOLATION_TIME)
        {
            m_pActor->GetSobProxy()->SetInkThicknessWeight(0.0f);
            RwReal fAlpha = 1.0f - m_fFPSInterTime / FPS_INTERPOLATION_TIME;
            m_pActor->GetSobProxy()->SetAlpha((RwUInt8)(fAlpha * 255.0f));

            m_fFPSInterTime += fElapsed;
        }
        else
        {
            m_pActor->GetSobProxy()->SetAlpha(0);
            m_pActor->GetSobProxy()->SetInkThicknessWeight(0.0f);
            m_bIsAlphaBlend = TRUE;
        }
    }
...
RwUInt32 CNtlFSMCharActScouterState::HandleEvents( RWS::CMsg &pMsg ) 
{
    if(pMsg.Id == g_EventScouter)
    {
        SDboEventScouter* data = (SDboEventScouter*)pMsg.pData;
        switch(data->iType)
        {
        case SCOUTER_EVENT_EXIT:                        
            if(!IsFinish() && m_pActor->GetClassID() == SLCLASS_AVATAR)
            {
                // 서버 전송
                m_bExit = TRUE;
                SetAnim(NML_SCOUTER_ON_OFF);
        ...
        case SCOUTER_EVENT_NFY_BROKEN:
            ...
            {
                m_bExit = TRUE;
                SetAnim(NML_SCOUTER_BURST);            
            }
            break;        
        }        
    }
    else if(pMsg.Id == g_EventActionMapScouterUse)      // 스카우터 사용상태에서 키입력을 받음
    {
        if(m_bIsUIOpen)
        {
            if(!m_bExit)
            {
                // 종료시킨다.
                CNtlSLEventGenerator::ScouterEvent(SCOUTER_EVENT_EXIT, m_pActor->GetSerialID());
                NTL_RETURN(NTL_FSM_EVENTRES_BLOCK);
            }
            else
            {
    ...
    else if(pMsg.Id == g_EventAnimEnd)
    {
        SNtlEventAnimEnd *pAnimEnd = reinterpret_cast<SNtlEventAnimEnd*>( pMsg.pData );
        RwUInt32 uiAnimKey = pAnimEnd->uiBaseAnimKey;
        if(uiAnimKey == NML_SCOUTER_ON_OFF)
        {
            if(m_bExit)
            {
                Finish();                
        ...
        else if(uiAnimKey == NML_SCOUTER_BURST)
        {
            Finish();
            return NTL_FSM_EVENTRES_CHANGE_STATE;
        }
...
void CNtlFSMCharActScouterState::SetAnim( RwUInt32 uiAnimKey ) 
{
    CNtlSobCharProxy *pSobProxy = reinterpret_cast<CNtlSobCharProxy*>(m_pActor->GetSobProxy());
    if(!pSobProxy)
        return;

    // 수영중에도 폭발 애니메이션만은 나온다.
    if(uiAnimKey == NML_SCOUTER_BURST)
    {
        DestroyEffect();        
        pSobProxy->SetBaseAnimation(uiAnimKey, FALSE);        
    }

    // 아래의 애니메이션들은 수영상태에서는 재생되지 않는다.
    if(pSobProxy->GetBaseAnimationKey() == SWIM_IDLE)
    {
        if(m_bExit)
        {
            Finish();
    ...
    if(uiAnimKey == 0)  // IDLE
    {
        SetAnim(Logic_GetIdleAnimationID(m_pActor));
    }
    else if(uiAnimKey == NML_SCOUTER_ON_OFF)
    {
        if(m_bExit)  
        {  
            DestroyEffect();                        
        ...
        if(m_pActor->GetClassID() == SLCLASS_AVATAR)
        {
            sNtlSoundPlayParameta tSoundParam;
            tSoundParam.iChannelGroup	= CHANNEL_GROUP_UI_SOUND;
            tSoundParam.pcFileName		= m_bExit ? WORLD_SOUND_SCOUTER_OFF : WORLD_SOUND_SCOUTER_ON;
            GetSoundManager()->Play(&tSoundParam);           
        }

        pSobProxy->SetBaseAnimation(uiAnimKey, FALSE, m_pActor->GetClassID() == SLCLASS_AVATAR ? 0.3f : 0.0f);
    }    
	else
	{
        pSobProxy->SetBaseAnimation(uiAnimKey, FALSE);
...
void CNtlFSMCharActScouterState::DestroyEffect() 
{
    if(m_pActor->GetClassID() == SLCLASS_AVATAR)
    {
        CNtlSLEventGenerator::CameraControlDelete(CAMERA_CONTROL_FPS);
        m_pActor->GetSobProxy()->SetAlpha(255);
        m_pActor->GetSobProxy()->SetInkThicknessWeight(1.0f);

        if(m_hLoopSound != INVALID_SOUND_HANDLE)
        {
            GetSoundManager()->Stop(m_hLoopSound);
            m_hLoopSound = INVALID_SOUND_HANDLE;
        }             
...
void CNtlFSMCharSpinAttack::Enter() 
{
    ...
    CNtlBeCharData *pBeData = reinterpret_cast<CNtlBeCharData*>(m_pActor->GetBehaviorData()); 
    SMoveStuff *pMoveStuff = pBeData->GetMoveStuff();
    pMoveStuff->Reset();    // 기존의 이동 정보를 없애기 위해서 리셋한다.    

    m_pMoveBehavior = NTL_NEW CNtlBehaviorCharGroundMove();
    m_pMoveBehavior->SetNotAnimChange(TRUE);
...
{
    // MOVE 처리를 막아놨기 때문에, 여기서 MOVE에 따른 처리를 따로 한다.
    if(pMsg.Id == g_EventKeyboardMove)
    {
        SNtlEventKeyboardMove *pKeyboardMove = reinterpret_cast<SNtlEventKeyboardMove*>(pMsg.pData);
    ...
    else if(pMsg.Id == g_EventTerrainClick)
    {
        ...
        FSMEvent_CharActTerrainClickDir(m_pActor, pEvent);
        UpdateBehavior();

        RwV3d vDir = pEvent->vPickPos - m_pActor->GetPosition();
        RwV3dNormalize(&vDir, &vDir);
        m_pActor->SetDirection(&vDir);
        
        API_GetSLPacketGenerator()->SendKeyboardMove(m_pActor->GetPosition(), m_pActor->GetDirection(), NTL_MOVE_F, m_pActor->GetServerSyncAvatarType());
    }
    else if(pMsg.Id == g_EventSobMove)
    {
	...
	case ASPECTSTATE_INVALID:
		{
			...
			RwV3d vDir = m_pActor->GetDirection(); vDir.y = 0.f;
			RwV3dNormalize( &vDir, &vDir );

			// PC의 방향을 보정한다

			RwReal fXAngle, fYAngle;
			CNtlMath::LineToAngleXY( &vDir, fXAngle, fYAngle );
			((CNtlPLCharacter*)m_pActor->GetSobProxy()->GetPLMainEntity())->SetAngleX( fXAngle );
			((CNtlPLCharacter*)m_pActor->GetSobProxy()->GetPLMainEntity())->SetAngleY( fYAngle );

			CNtlSobVehicle* pVehicle = (CNtlSobVehicle*) CNtlSobManager::GetInstance()->GetSobObject( hVehicleID );
