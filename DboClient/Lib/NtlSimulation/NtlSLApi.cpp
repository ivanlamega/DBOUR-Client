...
CNtlGameRuleConcept			*g_pGameRuleConcept		= NULL;
...
RwBool CreateTableContainer(RwUInt8 byLoadMethod)
{
    ...
    if(GetNtlResourcePackManager()->GetActiveFlags() & NTL_PACK_TYPE_FLAG_TABLE)
    {
		...
		if (std::experimental::filesystem::exists("mods/data/Table_Text_All_Data.rdf"))
		{
			CNtlBitFlagManager flagManager2;
			if (false == flagManager2.Create(CTableContainer::TABLE_COUNT))
			{
				return false;
			}

			CTableFileNameList fileNameList2;
			if (false == fileNameList2.Create())
			{
				return false;
			}

			flagManager2.Set(CTableContainer::TABLE_QUEST_TEXT_DATA);
			flagManager2.Set(CTableContainer::TABLE_TEXT_ALL);
			fileNameList2.SetFileName(CTableContainer::TABLE_QUEST_TEXT_DATA, "Table_Quest_Text_Data");
			fileNameList2.SetFileName(CTableContainer::TABLE_TEXT_ALL, "Table_Text_All_Data");

			bResult = g_pTableContainer->Create(flagManager2, (char*)"mods/data/", &fileNameList2, eLoadMethod, GetACP(), &tableLoad);
    ...
    else
    {
		...
		//g_pTableContainer->SaveToFile(flagManager, &fileNameList, true);
    }	
...
RwBool API_SLInit(RwBool bVirtualNet, CNtlCamera *pCamera, RwUInt8 byLoadMethod, RwBool bDTDataLoad /*= FALSE*/)
{
	...
	// game rule concept
	g_pGameRuleConcept = NTL_NEW CNtlGameRuleConcept;
	if(!g_pGameRuleConcept->Create())
	{
		NTL_RETURN(FALSE);
	}
...
void API_SLReset(void)
{
	...
	if(g_pGameRuleConcept)
	{
		g_pGameRuleConcept->Destroy();
		NTL_DELETE(g_pGameRuleConcept);
	}
