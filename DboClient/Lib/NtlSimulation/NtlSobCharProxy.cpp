...
void CNtlSobCharProxy::SetDirection(const RwV3d *pDir)
{
	if(m_pCharacter)
	{
		...
		if ( pActor && ( Logic_IsBus( pActor ) || Logic_IsVehicleDriver( pActor ) ) )
		{
...
CNtlPLCharacter* CNtlSobCharProxy::CreatePLCharacter(RwBool bNotShading)
{
	...
	if(pSobActor->GetActorFlags() & SLFLAG_ACTOR_HAVE_SOCIAL)
	{
		...
		fScale		= pSobSocialAttr->GetScale();
	}
	else
	{
...
void CNtlSobCharProxy::SetBaseAnimation(RwUInt32 uiAnimKey, RwBool bLoop /*= TRUE*/, RwReal fStartTime /*= 0.0f*/)
{
	...
	if(m_pUIPcStatusWnd)
	{
		if(uiAnimKey == NML_IDLE_LOOP || uiAnimKey == NML_STAFF_IDLE_LOOP ||
			uiAnimKey == NML_DGUN_IDLE_LOOP)
		{
...
{
	m_pDecorationProxy->SetNameColor(pwcName, nameColor, pwcGuildName, guildColor, isGamemaster);
}
