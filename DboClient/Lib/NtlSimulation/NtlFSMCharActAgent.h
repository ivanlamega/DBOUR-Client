...
union UCharActModeFlag
{
	struct 
	{
		...
		RwUInt32 bitClientUIDisable : 1;	// 클라이언트 UI 출력 금지.
...
protected:
	...
	RwUInt8							m_byServerState;
	RwUInt32						m_uiServerCondition;
	RwV3d							m_vServerPos;
	RwV3d							m_vServerDir;
...
protected:
	...
	virtual void ServerConditionUpdateAnalysis(RwUInt8 byServerState, DWORD uiCondition);
	...
	void ServerStateCreateMoving(sCHARSTATE *pServerState, RwBool isFlying = FALSE);
...
public:
    ...
    RwUInt32                GetServerCondition() { return m_uiServerCondition;}
	...
	void					ConditionVisualColorFlickering(RwUInt32 uiServerCondition);
	void					ConditionVisualColorFlickeringPop(void);
	void					ConditionDiff(RwUInt32 uiServerCondition);
