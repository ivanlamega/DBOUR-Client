...
void CNtlSubWeaponController::UpdateIdle(RwReal fElapsed)
{
    ...
    // Å×ÀÌºí¿¡¼­ Ãß°¡ ¿ÀÇÁ¼Â µ¥ÀÌÅÍ¸¦ ÀÐ¾î¿Â´Ù.
	...
	RwV3dAssign(&m_matWorld.pos, &vPos);
...
CNtlSobCharEquipProxy& CNtlSobCharEquipProxy::operator = (CNtlSobCharEquipProxy& EquipProxy)
{
	...
	for(RwInt32 i = 0 ; i < NTL_MAX_EQUIP_ITEM_SLOT; ++i)
	{
		...
		if((i == EQUIP_SLOT_TYPE_QUEST || i == EQUIP_SLOT_TYPE_DOGI || i == EQUIP_SLOT_TYPE_HAIR
			|| i == EQUIP_SLOT_TYPE_ACCESSORY_1 || i == EQUIP_SLOT_TYPE_ACCESSORY_2 || i == EQUIP_SLOT_TYPE_ACCESSORY_3)) //This is where you add more equipment types - SanGakwu
		{
	...
	for(RwUInt8 i = 0; i < NTL_MAX_EQUIP_ITEM_SLOT; i++)
	{
		...
		if(bCostume)
		{
			//This is where you add more equipment types - SanGakwu
			if ((i != EQUIP_SLOT_TYPE_QUEST || i != EQUIP_SLOT_TYPE_DOGI || i != EQUIP_SLOT_TYPE_HAIR || i != EQUIP_SLOT_TYPE_ACCESSORY_1 || i != EQUIP_SLOT_TYPE_ACCESSORY_2 ||
				i != EQUIP_SLOT_TYPE_ACCESSORY_3) && Logic_IsHideCostumeItem(wCostumeBitFlags, (void*)m_arrSlotItem[i].pPLItem))
			{
		...
		if (bCostume && (i == EQUIP_SLOT_TYPE_QUEST || i == EQUIP_SLOT_TYPE_DOGI || i == EQUIP_SLOT_TYPE_HAIR
			|| i == EQUIP_SLOT_TYPE_ACCESSORY_1 || i == EQUIP_SLOT_TYPE_ACCESSORY_2 || i == EQUIP_SLOT_TYPE_ACCESSORY_3))
		{
			if(m_arrSlotItem[i].bCharacterAdd)
            {
				m_pPLCharacter->SetChangeEquipItem(m_arrSlotItem[i].pPLItem, TRUE, wCostumeBitFlags);
	...
	if(m_pSobObj->GetClassID() == SLCLASS_AVATAR)
	{
		...
		for(RwUInt8 i = 0; i < NTL_MAX_EQUIP_ITEM_SLOT; i++)
		{
			...
			if(EQUIP_TYPE_ACCESSORY != pItemTblData->byEquip_Type)
			{
				...
				if (std::regex_search(strModelName, re))
				{
					strModelName = "";
				}
			...
			if(bCostume)
			{
				//This is where you add more equipment types - SanGakwu
				if ((i != EQUIP_SLOT_TYPE_QUEST || i != EQUIP_SLOT_TYPE_DOGI || i != EQUIP_SLOT_TYPE_HAIR || i != EQUIP_SLOT_TYPE_ACCESSORY_1 || i != EQUIP_SLOT_TYPE_ACCESSORY_2 ||
					i != EQUIP_SLOT_TYPE_ACCESSORY_3)
					&& Logic_IsHideCostumeItem(wCostumeBitFlags, (void*)m_arrSlotItem[i].pPLItem))
				{
					GetSceneManager()->DeleteEntity(m_arrSlotItem[i].pPLItem);
					m_arrSlotItem[i].pPLItem = NULL;
					continue;
...
void CNtlSobCharEquipProxy::CreateEquipItem(RwUInt32 uiTblId, RwUInt8 bySlotIdx, RwUInt8 byGrade)
{
	...
	RwBool bCostume = FALSE;
	if((m_arrSlotItem[EQUIP_SLOT_TYPE_DOGI].uiItemTblId != INVALID_TBLIDX )||
		(m_arrSlotItem[EQUIP_SLOT_TYPE_HAIR].uiItemTblId != INVALID_TBLIDX) || 
		(m_arrSlotItem[EQUIP_SLOT_TYPE_ACCESSORY_1].uiItemTblId != INVALID_TBLIDX) || 
		(m_arrSlotItem[EQUIP_SLOT_TYPE_ACCESSORY_2].uiItemTblId != INVALID_TBLIDX) ||
		(m_arrSlotItem[EQUIP_SLOT_TYPE_ACCESSORY_3].uiItemTblId != INVALID_TBLIDX) ||
		(m_arrSlotItem[EQUIP_SLOT_TYPE_QUEST].uiItemTblId != INVALID_TBLIDX))
		bCostume = TRUE;
	...
	if(Logic_IsCostumeItem(pItemTblData))
	{
		for(RwInt32 i = 0; i < NTL_MAX_EQUIP_ITEM_SLOT; i++)
		{
			...
			if( !Logic_IsHideCostumeItem(pItemTblData->wCostumeHideBitFlag, (void*)m_arrSlotItem[i].pPLItem) )
				continue;
	...
	if(bCostume)
	{
		//This is where you add more equipment types - SanGakwu				
			if (m_arrSlotItem[EQUIP_SLOT_TYPE_DOGI].uiItemTblId != INVALID_TBLIDX)
			{
				sITEM_TBLDAT *pCostumeItemTblData = reinterpret_cast<sITEM_TBLDAT*>(pItemTbl->FindData(m_arrSlotItem[EQUIP_SLOT_TYPE_DOGI].uiItemTblId));
				if (pItemTbl)
				{
			...
			else if (m_arrSlotItem[EQUIP_SLOT_TYPE_HAIR].uiItemTblId != INVALID_TBLIDX)
			{
				sITEM_TBLDAT *pCostumeItemTblData = reinterpret_cast<sITEM_TBLDAT*>(pItemTbl->FindData(m_arrSlotItem[EQUIP_SLOT_TYPE_HAIR].uiItemTblId));
				if (pItemTbl)
				{
					if (Logic_IsHideCostumeItem(pCostumeItemTblData->wCostumeHideBitFlag, (void*)m_arrSlotItem[bySlotIdx].pPLItem))
					{
			...
			else if (m_arrSlotItem[EQUIP_SLOT_TYPE_ACCESSORY_1].uiItemTblId != INVALID_TBLIDX)
			{
				sITEM_TBLDAT *pCostumeItemTblData = reinterpret_cast<sITEM_TBLDAT*>(pItemTbl->FindData(m_arrSlotItem[EQUIP_SLOT_TYPE_ACCESSORY_1].uiItemTblId));
				if (pItemTbl)
				{
					if (Logic_IsHideCostumeItem(pCostumeItemTblData->wCostumeHideBitFlag, (void*)m_arrSlotItem[bySlotIdx].pPLItem))
					{
			...
			else if (m_arrSlotItem[EQUIP_SLOT_TYPE_ACCESSORY_2].uiItemTblId != INVALID_TBLIDX)
			{
				sITEM_TBLDAT *pCostumeItemTblData = reinterpret_cast<sITEM_TBLDAT*>(pItemTbl->FindData(m_arrSlotItem[EQUIP_SLOT_TYPE_ACCESSORY_2].uiItemTblId));
				if (pItemTbl)
				{
					if (Logic_IsHideCostumeItem(pCostumeItemTblData->wCostumeHideBitFlag, (void*)m_arrSlotItem[bySlotIdx].pPLItem))
					{
			...
			else  if (m_arrSlotItem[EQUIP_SLOT_TYPE_ACCESSORY_3].uiItemTblId != INVALID_TBLIDX)
			{
				sITEM_TBLDAT *pCostumeItemTblData = reinterpret_cast<sITEM_TBLDAT*>(pItemTbl->FindData(m_arrSlotItem[EQUIP_SLOT_TYPE_ACCESSORY_3].uiItemTblId));
				if (pItemTbl)
				{
					if (Logic_IsHideCostumeItem(pCostumeItemTblData->wCostumeHideBitFlag, (void*)m_arrSlotItem[bySlotIdx].pPLItem))
					{
			...
			else if (m_arrSlotItem[EQUIP_SLOT_TYPE_QUEST].uiItemTblId != INVALID_TBLIDX)
			{
				sITEM_TBLDAT *pCostumeItemTblData = reinterpret_cast<sITEM_TBLDAT*>(pItemTbl->FindData(m_arrSlotItem[EQUIP_SLOT_TYPE_QUEST].uiItemTblId));
				if (pItemTbl)
				{
					if (Logic_IsHideCostumeItem(pCostumeItemTblData->wCostumeHideBitFlag, (void*)m_arrSlotItem[bySlotIdx].pPLItem))
					{
...
void CNtlSobCharEquipProxy::SobSubWeaponActiveEventHandler(RWS::CMsg &pMsg)
{
	...
	// ±âÁ¸¿¡ ÀåÂøÇÏ°í ÀÖ´Â sub weaponÀ» remove ÇÑ´Ù.
	...
	// ¿òÁ÷ÀÌ´Â sub weapon ÀÌ°Å³ª È¤Àº active »óÅÂ¿¡¼­ attach µÇÁö ¾ÊÀ» °æ¿ì.
	...
	// sub weapon trace¸¦ setting ÇØ ÁØ´Ù.
	if(m_arrSlotItem[bySlotIdx].pPLItem)
...
void CNtlSobCharEquipProxy::SobSubWeaponDeActiveEventHandler(RWS::CMsg &pMsg)
{
	...
	// ÀåÂøÇÏ°í ÀÖ´Â sub weaponÀ» remove ½ÃÄÑÁØ´Ù.
