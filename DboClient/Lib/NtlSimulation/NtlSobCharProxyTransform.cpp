...
void CNtlSobCharProxyTransform::HandleEvents( RWS::CMsg &pMsg ) 
{
    if(pMsg.Id == g_EventTransform)
    {
        ...
        {
            // 스케쥴 로딩이 끝난후 이벤트를 처리한다. 
            m_bScheduleLoadingCheck = TRUE;
        }
    }
    else if(pMsg.Id == g_EventSobSkillAction)       // 스킬을 사용할때
    {
...
//				case ACTIVE_SUPER_SAIYAN_BLUE:        uiTransCode = ASPECTSTATE_SUPER_SAIYAN_BLUE;        break;
                }
...
void CNtlSobCharProxyTransform::EventSobTransform(SNtlEventTransform* pData)
{
    ...
    if(!bTransform) 
    {
        // 아바타는 Hit Event에서 변신하는데 이벤트 발생전에 해제 패킷이 먼저 날라오는 경우가 있다.        
        m_bReadyTransform = FALSE;
    }
    ...
    case ASPECTSTATE_KAIOKEN:
        OnTransform_KaiOuKen(bTransform, pData->sApsectState.sAspectStateDetail.sKaioken.bySourceGrade, pData->sApsectState.sAspectStateDetail.sKaioken.byRepeatingCount);
        break;
	case ASPECTSTATE_SUPER_SAIYAN_BLUE:
		OnTransform_SuperSaiyanBlue(bTransform);
		break;
    case ASPECTSTATE_INVALID:
        {
            ...
            case ASPECTSTATE_KAIOKEN:					OnTransform_KaiOuKen(FALSE, NULL, NULL); break;
			case ASPECTSTATE_SUPER_SAIYAN_BLUE:			OnTransform_SuperSaiyanBlue(FALSE); break;
            }
        }
        break;
    }
...
void CNtlSobCharProxyTransform::OnTransform_SuperSaiyan( RwBool bTransform ) 
{
    if(bTransform)
    {
        ...
        if(!m_pItemTransform)
        {
            m_pItemTransform = GetTransformItem(ASPECTSTATE_SUPER_SAIYAN);
        }
		...
		if(m_pPLCharacterUI)
        {
            m_pItemTransformUI = GetTransformItem(ASPECTSTATE_SUPER_SAIYAN);
            m_pPLCharacterUI->SetChangeEquipItem(m_pItemTransformUI);
        }   

		if(m_pPLCharacterUI_Dogi)
		{
			m_pItemTransformUI_Dogi = GetTransformItem(ASPECTSTATE_SUPER_SAIYAN);
...
void CNtlSobCharProxyTransform::OnTransform_SuperSaiyanBlue(RwBool bTransform)
{
	if (bTransform)
	{
		CNtlDefaultItemTable::GetInstance().Refresh();
		...
		if (!m_pItemTransform)
		{
			m_pItemTransform = GetTransformItem(ASPECTSTATE_SUPER_SAIYAN_BLUE);
		}
		...
		if (m_pPLCharacterUI)
		{
			m_pItemTransformUI = GetTransformItem(ASPECTSTATE_SUPER_SAIYAN_BLUE);
			m_pPLCharacterUI->SetChangeEquipItem(m_pItemTransformUI);
		}

		if (m_pPLCharacterUI_Dogi)
		{
			m_pItemTransformUI_Dogi = GetTransformItem(ASPECTSTATE_SUPER_SAIYAN_BLUE);
			m_pPLCharacterUI_Dogi->SetChangeEquipItem(m_pItemTransformUI_Dogi);
		}
		...
		// Effect        
		AttachEffectSuperSaiyanBlue(m_pPLCharacter);

		// color
		m_pActor->GetSobProxy()->SetInkColor(255, 255, 255);
		m_pSystemEffectColor[0] = m_pActor->GetSobProxy()->AddVisualSystemEffectColor(255, 255, 120);
		m_pSystemEffectColor[1] = m_pActor->GetSobProxy()->AddVisualSystemEffectColor(3, 20, 40, TRUE);
	}
	else
	{
		if (m_pItemTransform)
...
void CNtlSobCharProxyTransform::OnTransform_KaiOuKen(RwBool bTransform, BYTE byLevel, BYTE byRepeatCnt)
{
    if(bTransform)
    {
...
//		case 0:
//		{
			m_pActor->GetSobProxy()->SetInkColor(211, 170, 187); //Outline
			m_pKaiOuKenColor[0] = m_pActor->GetSobProxy()->AddVisualSystemEffectColor(242, 181, 140); //Accesory
			m_pKaiOuKenColor[1] = m_pActor->GetSobProxy()->AddVisualSystemEffectColor(160, 7, 12, TRUE); //Skin
/*		}
		case 1:
		{
			m_pActor->GetSobProxy()->SetInkColor(185, 121, 120);
			m_pKaiOuKenColor[0] = m_pActor->GetSobProxy()->AddVisualSystemEffectColor(192, 131, 90);
		...
		case 2:
		{
			...
			m_pKaiOuKenColor[1] = m_pActor->GetSobProxy()->AddVisualSystemEffectColor(255, 7, 12, TRUE);
		}
		default:
			break;
			// color*/
...
void CNtlSobCharProxyTransform::AttachEffectSuperSaiyan(CNtlPLCharacter* pPLCharacter)
{
    ...
    if(pPLCharacter->GetBoneByName(strBoneList2.c_str())) // 큰 이펙트
    {
...
void CNtlSobCharProxyTransform::AttachEffectSuperSaiyanBlue(CNtlPLCharacter* pPLCharacter)
{
	if (!m_vecEffect.empty())
		return;
	...
	for (int i = 0; i < 7; ++i)      // 작은 이펙트
	{
		if (pPLCharacter->GetBoneByName(strBoneList1[i].c_str()))
		{
			pEffect = (CNtlInstanceEffect*)GetSceneManager()->CreateEntity(PLENTITY_EFFECT, NTL_VID_SUPER_SAIYAN_BLUE_SMALL);
			if (pEffect)
			{
				pPLCharacter->AttachBone(pEffect, strBoneList1[i].c_str());
				m_vecEffect.push_back(pEffect);
	...
	if (pPLCharacter->GetBoneByName(strBoneList2.c_str())) // 큰 이펙트
	{
		pEffect = (CNtlInstanceEffect*)GetSceneManager()->CreateEntity(PLENTITY_EFFECT, NTL_VID_SUPER_SAIYAN_BLUE_SMALL); //Come Back to this
		if (pEffect)
		{
			pPLCharacter->AttachBone(pEffect, strBoneList2.c_str());
			m_vecEffect.push_back(pEffect);
...
CNtlPLItem* CNtlSobCharProxyTransform::GetTransformItem(eASPECTSTATE state) 
{
    SCharScheduleResInfo* pInfo = m_pPLCharacter->GetCharScheduleResInfo();    
	...
	std::string* pstrHeadKeyName;

	switch (state)
	{
		case ASPECTSTATE_SUPER_SAIYAN:
		{
			pstrHeadKeyName = CNtlDefaultItemTable::GetInstance().GetSuperSaiyanHeadKeyName(eClassGenderType, pInfo->uiHeadType);
			break;
		}

		case ASPECTSTATE_SUPER_SAIYAN_BLUE:
		{
			pstrHeadKeyName = CNtlDefaultItemTable::GetInstance().GetSuperSaiyanBlueHeadKeyName(eClassGenderType, pInfo->uiHeadType);
			break;
		}

		default:
		{
			NTL_ASSERTFAIL("Invalid Aspect State in GetTransformItem");
			return NULL;
		}
...
void CNtlSobCharProxyTransform::OnTransform_Mob( RwUInt32 tblIdx ) 
{
    ...
    pMonsterAttr->SetName(pTblData->wszNameText);
...
{
    // 변신상태에서 어른/어린이로 변경된 경우 변신을 해제하고 다시 변신한다.
    if(IsTransform())
    {
        m_bIsTransform = FALSE;
        EventSobTransform(&m_transFormData);
