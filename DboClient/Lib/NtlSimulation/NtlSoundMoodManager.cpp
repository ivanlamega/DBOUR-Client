...
VOID CNtlSoundMoodManager::Condition_AskPlaySFX_from_Server(sGU_PLAY_SFX* pPlayBGM)
{
	if (NULL == pPlayBGM)
	{
		DBO_FAIL("Invalid PLAY SFX pointer");
		return;
	}

	if (pPlayBGM->bPlay)
	{
		CNtlSoundEventGenerator::SendMusicRestItem(CHANNEL_GROUP_AVATAR_VOICE_SOUND, 1000.f * (RwReal)pPlayBGM->dwDelay, BGM_TYPE_SERVER_PLAYED);

		sNtlSoundPlayParameta tSoundParam;
		tSoundParam.iChannelGroup = CHANNEL_GROUP_AVATAR_VOICE_SOUND;
		tSoundParam.pcFileName = pPlayBGM->szName;
		tSoundParam.eBGM_Type = BGM_TYPE_SERVER_PLAYED;
		tSoundParam.bLoop = pPlayBGM->bLoop;
		GetSoundManager()->Play(&tSoundParam);
