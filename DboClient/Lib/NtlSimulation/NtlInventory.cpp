...
{
	NTL_ASSERT(bySlotIdx >= 0 && bySlotIdx < NTL_MAX_EQUIP_ITEM_SLOT, "CNtlInventoryBase::SetEquipTableId Invalid bySlotIdx =" + bySlotIdx);
...
CNtlSobItem* CNtlInventory::CreateItem(void *pParentSobItem, RwInt32 iItemPfIdx)
{
	...
	void *pSobObj = CNtlSLEventGenerator::SobSlotItemCreate(pParentSobItem, SLCLASS_SLOT_ITEM, pItemProfile->handle, 
						pItemProfile->tblidx, pItemProfile->aOptionTblidx[0], pItemProfile->aOptionTblidx[1],
						...
						FALSE, pItemProfile->byBattleAttribute, pItemProfile->byRestrictType, pItemProfile->awchMaker,
...
CNtlSobItem* CNtlInventory::CreateItem(void *pParentSobItem, SNtlEventSobItemAdd *pSobItemAdd)
{
	...
	void *pSobObj = CNtlSLEventGenerator::SobSlotItemCreate(pParentSobItem, SLCLASS_SLOT_ITEM, pSobItemAdd->hItemSerialId, 
															pSobItemAdd->uiItemTblId, pSobItemAdd->uiOptionTblId1, pSobItemAdd->uiOptionTblId2,
															...
															FALSE, pSobItemAdd->byBattleAttribute, pSobItemAdd->byRestrictType, pSobItemAdd->wszMaker,
...
void CNtlInventory::CreateEventHandler(RWS::CMsg &pMsg)
{
	...
	for(RwInt32 i = 0; i < pAvatarInfo->byItemCount; ++i)
	{
		...
		else if(byPlace == CONTAINER_TYPE_NETPYSTORE)
		{
			CreateItem(GetNtlSobManager()->GetSobObject(m_hBagItem[5]), i);
		}
		...
		else if(byPlace == CONTAINER_TYPE_EQUIP)
		{
			...
			SetEquipItem(pAvatarInfo->sItemPf[i].byPos, pAvatarInfo->sItemPf[i].handle);
		}

		else if(byPlace == CONTAINER_TYPE_SCOUT)
		{
			CNtlSob* pSobObj = GetNtlSobManager()->GetSobObject( GetScouterItem() );

			NTL_ASSERT( pSobObj, "Avatar does not have Scouter" );
			CreateItem( pSobObj, i);
		}
	}
...
void CNtlInventory::AddEventHandler(RWS::CMsg &pMsg)
{
	...
	{
		CreateItem(GetNtlSobManager()->GetSobObject(m_hBagItem[5]), pSobItemAdd);
	}
	...
	else if(byPlace == CONTAINER_TYPE_SCOUT)
	{
		...
		NTL_ASSERT( pSobObj, "Avatar does not have Scouter" );
		CreateItem( pSobObj, pSobItemAdd);
	}
...
void CNtlInventory::DeleteEventHandler(RWS::CMsg &pMsg)
{
	...
	else if( pSobItemDelete->byPlace == CONTAINER_TYPE_SCOUT )
	{
		CNtlSLEventGenerator::SobDelete( pSobItemDelete->hItemSerialId );
	}
}
...
void CNtlInventory::ItemMoveEventHandler(RWS::CMsg &pMsg)
{
	...
	if(pItemMove->bySrcPlace == CONTAINER_TYPE_BAGSLOT)
	{
		...
		else if(pItemMove->byDestPlace == CONTAINER_TYPE_SCOUT)
		{
			NTL_ASSERTFAIL("CNtlInventory::ItemMoveEventHandler : bag slot to scouter");
		}
	}
	...
	else if(pItemMove->bySrcPlace >= CONTAINER_TYPE_BAG_FIRST && pItemMove->bySrcPlace <= CONTAINER_TYPE_BAG_LAST)
	{
		...
		{
			// Source 이동
			pSrcSobItem->SetParentItemSerial( GetScouterItem() );
			pSrcParentBagSobItem->SetChildSerial( pItemMove->bySrcSlotIdx, pItemMove->hDestSerial );
			...
			SetScouterChildItem( pItemMove->byDestSlotIdx, pItemMove->hSrcSerial );
		}
	}
	...
	else if (pItemMove->bySrcPlace == CONTAINER_TYPE_EQUIP)
	{
		...
		{
			NTL_ASSERTFAIL("CNtlInventory::ItemMoveEventHandler : Item Move Equip to Scout");
		}
	}

	else if(pItemMove->bySrcPlace == CONTAINER_TYPE_SCOUT) 
	{
		if(pItemMove->byDestPlace == CONTAINER_TYPE_BAGSLOT)
		{
			NTL_ASSERTFAIL("CNtlInventory::ItemMoveEventHandler : Item Move Scout to BagSlot");
		}

		else if(pItemMove->bySrcPlace >= CONTAINER_TYPE_BAG_FIRST && pItemMove->byDestPlace <= CONTAINER_TYPE_BAG_LAST)
		{
			RwUInt8 byDestParentSlotIdx = (RwUInt8)(pItemMove->byDestPlace - CONTAINER_TYPE_BAG1);

			// source 이동
			pSrcSobItem->SetParentItemSerial( GetBagItem(byDestParentSlotIdx) );
			SetScouterChildItem( pItemMove->bySrcSlotIdx, pItemMove->hDestSerial );
			...
			if(pItemMove->hDestSerial != INVALID_SERIAL_ID)
			{
				CNtlSobItem *pDestSobItem = reinterpret_cast<CNtlSobItem*>( GetNtlSobManager()->GetSobObject( pItemMove->hDestSerial ) );
				NTL_ASSERT( pDestSobItem, "CNtlInventory::ItemMoveEventHandler : Dest item instance is null !!!" );
				pDestSobItem->SetParentItemSerial( GetScouterItem() );
			}
		...
		{
			NTL_ASSERTFAIL("CNtlInventory::ItemMoveEventHandler : source item type scout to equip");
		}
...
void CNtlInventory::ItemStackMoveEventHandler(RWS::CMsg &msg)
{
	...
	if( pDestBag->GetChildSerial( pItemStackMove->byDestSlotIdx ) == INVALID_SERIAL_ID )
	{
		...
		CNtlSLEventGenerator::SobItemAdd( GetNtlSLGlobal()->GetSobAvatar()->GetSerialID(),
										  pItemStackMove->hDestSerial, pItemData->tblidx, pSrcItemAttr->GetOptionIdx1(), pSrcItemAttr->GetOptionIdx2(),
										  ...
										  pSrcItemAttr->GetRestrictType(), pSrcItemAttr->GetMaker(), pSrcItemAttr->GetDurationType(), pSrcItemAttr->GetStartTime(), pSrcItemAttr->GetEndTime() );										  
		pSrcItem->HandleEvents( msg );		
	}

	//  합쳐지는 아이템은 같은 종류이더라도 사용시간, 소유자가 같을 때만 합쳐진다. 
...
void CNtlInventory::ItemUseActionEventHandler(RWS::CMsg &msg)
{
	...
	SetItemCooltimeInfo( pUseItemTblData->dwCool_Time_Bit_Flag, (RwReal)pUseItemTblData->wCool_Time );	
...
void CNtlInventory::ItemUpgradeEventHandler(RWS::CMsg &msg)
{
	...
	if( pItemUpgrade->byStonePlace >= CONTAINER_TYPE_BAG_FIRST && pItemUpgrade->byStonePlace <= CONTAINER_TYPE_BAG_LAST )
	{
		...
		NTL_ASSERTFAIL( "CNtlInventory::ItemUpgradeEventHandler : Upgrade place is Wrong" );
	...
	if( pItemUpgrade->byItemPlace >= CONTAINER_TYPE_BAG_FIRST && pItemUpgrade->byItemPlace <= CONTAINER_TYPE_BAG_LAST )
	{
		...
		else
		{
			...
			pItemAttr->SetBattleAttribute( pItemUpgrade->byBattleAttribute );		
		}		
	}
	else
		NTL_ASSERTFAIL( "CNtlInventory::ItemUpgradeEventHandler : Upgrade place is Wrong" );
