...
void CNtlSobAvatar::CreateEvent(void)
{
    ...
    LinkMsg(g_EventActionMapScouterUse, 0);
	...
	// 개인상점 흥정 아이템 구매
	LinkMsg(g_EventPrivateShopBusinessImmediateItemBuyingRes, 0 );
	LinkMsg(g_EventPrivateShopBusinessImmediateItemBuyingNfy, 0 );
	...
	LinkMsg(g_EventTMQBoard, 0);
	LinkMsg(g_EventTMQ, 0);
...
void CNtlSobAvatar::DestroyEvent(void)
{
    ...
    UnLinkMsg(g_EventActionMapScouterUse);
	...
	UnLinkMsg(g_EventPrivateShopBusinessImmediateItemBuyingRes);
	UnLinkMsg(g_EventPrivateShopBusinessImmediateItemBuyingNfy);
...
void CNtlSobAvatar::HandleEvents(RWS::CMsg &pMsg)
{
	...
	if(pMsg.Id == g_EventSobCreate)
	{
		...
		// skill setting
		// peessi: Skill은 이전로그인 상태의 쿨타임 정보등을 갱신하기 때문에 미리 초기화 되어야 한다. 
	...
	{
		SNtlEventMouseDashMove *pDashMove = reinterpret_cast<SNtlEventMouseDashMove*>(pMsg.pData);
		RwV3d vPos = GetPosition();
		RwV3d vSub;

		RwV3dSub(&vSub, &pDashMove->vPickPos, &vPos);

		RwReal fLen = RwV3dLength(&vSub);
		if(fLen < pDashMove->fLimitDist)
		{
			NTL_RETURNVOID();
		}
		else
		{
			...
			pMsg.Id == g_EventSobItemDurUpdate || pMsg.Id == g_EventSobItemUpgrade || pMsg.Id == g_EventSobEquippedItemUpgradeAll)
	{
		CNtlSobPlayer::HandleEvents(pMsg); 
		m_Inventory.HandleEvents(pMsg);		
	...
	else if (pMsg.Id == g_EventHelpHint || pMsg.Id == g_EventBind)
	{
		m_OtherParam.HandleEvents(pMsg);
			...
			pMsg.Id == g_EventSobFakeBuffRefreshAll )
	{
		m_Party.HandleEvents(pMsg);
			...
			pMsg.Id == g_EventPrivateShopBusinessImmediateItemBuyingRes ||
			pMsg.Id == g_EventPrivateShopBusinessImmediateItemBuyingNfy )
	{
	...
	else if( pMsg.Id == g_EventTMQBoard || pMsg.Id == g_EventTMQ || pMsg.Id == g_EventTMQCleintState )
	{
			 ...
			 pMsg.Id == g_EventScrambleDefaultInfo )
	{
		m_Dojo.HandleEvents(pMsg);
			...
			pMsg.Id == g_EventOtherNameplateChanged)
	{
		CNtlSobPlayer::HandleEvents(pMsg);
	}
	else
...
void CNtlSobAvatar::SetPosition(const RwV3d* pPos)
{
	...
	{
		std::string logInfo = "unavailable avatar position. x : " + std::to_string(pPos->x) + " y : " + std::to_string(pPos->y) +" z : " + std::to_string(pPos->z);
