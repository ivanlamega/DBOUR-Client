...
RwBool CNtlWorldConcept::Create(void)
{
	...
	LinkMsg(g_EventDojo);
	LinkMsg(g_EventTSOpenBattleStart);
	LinkMsg(g_EventTSOpenBattleEnd);
...
void CNtlWorldConcept::Destroy(void)
{
	...
	UnLinkMsg(g_EventTSOpenBattleStart);
	UnLinkMsg(g_EventTSOpenBattleEnd);
}
...
void CNtlWorldConcept::AddWorldPlayConcept( EWorldPlayConcept ePlayConcept ) 
{
	...
	case WORLD_PLAY_OPENPVP:
		pController = NTL_NEW CNtlWorldConceptOpenPVP();
		break;
	default:
...
void CNtlWorldConcept::HandleEvents(RWS::CMsg &pMsg)
{
	...
	else if (pMsg.Id == g_EventTSOpenBattleStart)
	{
		CNtlWorldConceptOpenPVP* pWorldConceptPVP = (CNtlWorldConceptOpenPVP*)GetWorldConceptController(WORLD_PLAY_OPENPVP);
		if (pWorldConceptPVP)
		{
			pWorldConceptPVP->EventHandler_BattleStart(pMsg.pData);
		}
	}
	else if (pMsg.Id == g_EventTSOpenBattleEnd)
	{
		CNtlWorldConceptOpenPVP* pWorldConceptPVP = (CNtlWorldConceptOpenPVP*)GetWorldConceptController(WORLD_PLAY_OPENPVP);
		if (pWorldConceptPVP)
		{
			pWorldConceptPVP->EventHandler_BattleEnd(pMsg.pData);
		}
	}
...
RwBool CNtlWorldConcept::IsEnemyTargetFromAvatarActor(CNtlSobActor *pActor, SERIAL_HANDLE hTargetSerial)
{
	...
	else if(uiTargetClassId == SLCLASS_PLAYER)				// 타겟이 Player라면 PVP상태의 타겟만이 적이된다.
	{
		...
		if (IsActivePlayConcept(WORLD_PLAY_OPENPVP))
		{
			CNtlWorldConceptOpenPVP* pWorldConceptPVP = (CNtlWorldConceptOpenPVP*)GetWorldConceptController(WORLD_PLAY_OPENPVP);
			...
			if (pWorldConceptPVP)
			{
				...
				if (pWorldConceptPVP->IsAttackableEnemy(hTargetSerial))
					return TRUE;
				else
					return FALSE;
			}
	...
	else if(uiTargetClassId == SLCLASS_PET)					// 타겟이 Pet이고, PVP상태의 적이 pet의 주인인경우에만 적이 된다.
	{
		...
		else if (IsActivePlayConcept(WORLD_PLAY_OPENPVP))
		{
			CNtlWorldConceptOpenPVP* pWorldConceptPVP = (CNtlWorldConceptOpenPVP*)GetWorldConceptController(WORLD_PLAY_OPENPVP);
...
{
	// PVP 상태인 경우엔 상대를 가장 가까운 적으로 판단한다.
	if ( IsActivePlayConcept( WORLD_PLAY_FREEPVP ) )
	{
		CNtlWorldConceptPVP* pWorldConceptPVP = (CNtlWorldConceptPVP*) GetWorldConceptController( WORLD_PLAY_FREEPVP );
	...
	{
		CNtlWorldConceptOpenPVP* pWorldConcept = reinterpret_cast<CNtlWorldConceptOpenPVP*>(GetWorldConceptController(WORLD_PLAY_OPENPVP));
		if (!pWorldConcept || pWorldConcept->GetState() != WORLD_STATE_IDLE)
		{
			return NULL;
		}
