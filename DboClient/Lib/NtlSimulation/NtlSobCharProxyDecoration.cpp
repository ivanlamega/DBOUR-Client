...
void CNtlSobCharDecorationProxy::SobStateTanslation( RWS::CMsg& pMsg ) 
{
    ...
    {
        // 죽었다가 다시 살아나면 천하제일 무도회 마크 다시 보임
...
//Come back to look at this to make it always show quest mark if quest mark - sangawku
...
void CNtlSobCharDecorationProxy::CreatePLShadowDecal(void)
{
	...
	param.pTexName = "shadow.png";
#ifdef MODSENABLED
	param.pTexPath = "\\Texture\\effect\\";
	#else
	param.pTexPath = ".\\Texture\\effect\\";
#endif
...
void CNtlSobCharDecorationProxy::CreatePLPlayerName(void)
{
	...
	if( pSobActor->GetClassID() == SLCLASS_AVATAR || pSobActor->GetClassID() == SLCLASS_PLAYER )
	{
		...
		m_pPlayerName = Helper_CreatePlayerName(NULL, color, pName, NTL_GUILD_NAME_COLOR, pSobPlayer->GetGuildName(), m_pEmblemFactor);
	}
	else
	{
...
void CNtlSobCharDecorationProxy::SetNameColor(const WCHAR* pwcName, COLORREF nameColor,
											  const WCHAR* pwcGuildName, COLORREF guildColor, BOOL isGamemaster)
{
	if(m_pPlayerName)
		m_pPlayerName->SetNameColor(pwcName, nameColor, pwcGuildName, guildColor, m_pEmblemFactor, isGamemaster);
}

void CNtlSobCharDecorationProxy::SetNickNameColor(const WCHAR* pwcNickName, COLORREF nickNameColor)
{
	if(m_pPlayerName)
		m_pPlayerName->SetTitleColor(pwcNickName, nickNameColor);
}
...
void CNtlSobCharDecorationProxy::UpdatePlayerNamePosition( RwV3d& vPos )
{
	if ( m_pPlayerName )
	{
		RwV3dAssign( &m_vNamePos, &vPos );
