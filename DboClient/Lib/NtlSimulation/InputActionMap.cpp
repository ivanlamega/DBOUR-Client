...
,m_bAcceptServer(TRUE)
{
	...
	m_sFrontDashMap.uiMoveFlags = NTL_MOVE_DASH_F;
	m_sLeftDashMap.byDown = 0;
	m_sLeftDashMap.fTime = 0.0f;
	m_sLeftDashMap.uiMoveFlags = NTL_MOVE_DASH_L;
	m_sRightDashMap.byDown = 0;
	m_sRightDashMap.fTime = 0.0f;
	m_sRightDashMap.uiMoveFlags = NTL_MOVE_DASH_R;
	m_sBackDashMap.byDown = 0;
	m_sBackDashMap.fTime = 0.0f;
	m_sBackDashMap.uiMoveFlags = NTL_MOVE_DASH_B;
...
/**
* \brief InputActionMap의 키 정보들을 초기화한다.
*/
...
int CInputActionMap::KeyDownHandler(unsigned int pKeyData)
{
	...
	{
		// TODO : 현재 탭 키는 하드코딩되어있는 ACTION_TARGET_AUTO 액션ID를 사용하는
		// 곳에 맵핑되어 있으므로 임시적인 로그인 포커스창의 탭 처리를 위해 추가한 코드입니다.
		// 작업을 해서 외부로 뺀다거나 탭 키만 예외로 처리해야 하는 방법을 만들어야 합니다.
		if( FALSE == m_bActive )
		{
...
int CInputActionMap::KeyUpHandler(unsigned int pKeyData)
{
	...
	// 만약에 키를 셋팅하게 되면 UP에서 해준다는 것을 매개변수로 넣는다.
...
RwUInt32 CInputActionMap::ConvertServerFlags(RwUInt32 uiFlags)
{
	...
	else if(uiFlags == NTL_BIT_FB_MOVE || uiFlags == NTL_BIT_LR_MOVE)
	{
...
* \brief 이동 액션에 따른 핸들러
*
* 키를 눌렀을 때 키의 비트를 현재 이동에 알맞게 셋팅해주고 서버에서 사용되는
* flag로 변환해준다.
* 
...
void CInputActionMap::ActionDownMoveHandler( RwUInt32 uiAction ) 
{
	...
	// 서버로 보내는 이동의 flag가 저장되어 있는 flag에서 변화가 생겼을 경우
	// flag를 보내준다.
...
void CInputActionMap::UnLinkAirUp(void)
{
	if (m_pCallAirUp)
		NTL_DELETE(m_pCallAirUp);
}
void CInputActionMap::UnLinkAirDown(void)
{
	if (m_pCallAirDown)
		NTL_DELETE(m_pCallAirDown);
}
void CInputActionMap::UnLinkAirFastDown(void)
{
	if (m_pCallAirFastDown)
		NTL_DELETE(m_pCallAirFastDown);
}
void CInputActionMap::UnLinkAirFreeDown(void)
{
	if (m_pCallAirFreeDown)
		NTL_DELETE(m_pCallAirFreeDown);
}
void CInputActionMap::UnLinkAirBehavior(void)
{
	if (m_pCallAirBehavior)
		NTL_DELETE(m_pCallAirBehavior);
}
/**
