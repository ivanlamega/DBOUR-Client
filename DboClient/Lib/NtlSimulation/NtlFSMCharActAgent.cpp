...
CNtlFSMCharActAgent::CNtlFSMCharActAgent()
{
	...
	m_byServerState			= 0;
	m_uiServerCondition		= 0;
	m_bActiveAdjustPosition	= FALSE;
	m_uActMode.uiActMode	= 0;
...
void CNtlFSMCharActAgent::ServerStateCreateAnalysis(sCHARSTATE *pCharState)
{
	...
	case CHARSTATE_MOVING:
		ServerStateCreateMoving(pCharState, FALSE);
		break;
    ...
    case CHARSTATE_TURNING:
        ...
        break;
	case CHARSTATE_AIR_DASH:
		ServerStateCreateMoving(pCharState, TRUE);
		break;
...
void CNtlFSMCharActAgent::ServerStateCreateMoving(sCHARSTATE *pServerState, RwBool isFlying)
{
	if(!isFlying)
	SetNextStateId(NTL_FSMSID_MOVE);
	else
		SetNextStateId(NTL_FSMID_AIR_MOVE);
...
void CNtlFSMCharActAgent::ServerStateCreateFollowing(sCHARSTATE *pServerState)
{
	...
	if(pFollow->byMovementReason == NTL_FOLLOW_SKILL)
	{
		SetNextStateId(NTL_FSMSID_SKILL_FOLLOW);
	}
	else if(pFollow->byMovementReason == NTL_FOLLOW_FIGHTING)
	{
...
void CNtlFSMCharActAgent::ServerConditionUpdateAnalysis(RwUInt8 byServerState, DWORD uiCondition)
{
	m_sCharState.sCharStateBase.dwConditionFlag = uiCondition;

	ConditionVisualColorFlickering(uiCondition);
	ConditionDiff(uiCondition);

	m_uiServerCondition = uiCondition;

	if(m_uiServerCondition & CHARCOND_FLAG_CLICK_DISABLE)
		SetClickDisable(TRUE);
	else
		SetClickDisable(FALSE);

	if(m_uiServerCondition & CHARCOND_FLAG_CLIENT_UI_DISABLE)
		SetClientUIDisable(TRUE);
	else
		SetClientUIDisable(FALSE);

	if(m_uiServerCondition & CHARCOND_FLAG_ATTACK_DISALLOW)
		SetAttackDisallow(TRUE);
	else 
		SetAttackDisallow(FALSE);

    // 혼란이나 공포일때는 이미 DirectMode가 적용되어 있다.
    if(!(uiCondition & CHARCOND_FLAG_CONFUSED || uiCondition & CHARCOND_FLAG_TERROR))
    {    
...
void CNtlFSMCharActAgent::ServerAspectUpdateAnalysis(sASPECTSTATE& sAspectState, RwBool bCreateUpdate)
{
	...
	case ASPECTSTATE_SUPER_SAIYAN_BLUE:
	...
	case ASPECTSTATE_INVALID:
		{
			// 회전 공격 해제
			if ( Logic_GetActorStateId(m_pActor) == NTL_FSMSID_SPIN_ATTACK )
			{
...
void CNtlFSMCharActAgent::ServerStateMoving(sCHARSTATE *pServerState)
{
	...
	{
		NTL_ASSERTFAIL("CNtlFSMCharActAgent::ServerStateMoving");
	}
	...
	// jump flag일 경우에 이와 같이 바꾸어 준다.
	RwUInt8 byMoveFlag = pMoving->byMoveDirection;
	...
	CNtlSLEventGenerator::SobMove(m_pActor->GetSerialID(), byMoveFlag, NTL_MOVE_FLAG_RUN, vLoc, vDir, pMoving->dwTimeStamp);
}
...
void CNtlFSMCharActAgent::ServerStateDestMove(sCHARSTATE *pServerState)
{
	...
	if ( !Logic_IsBus( m_pActor ) )
	{
		...
		CNtlSLEventGenerator::SobDestMove( m_pActor->GetSerialID(), pDestMove->byMoveFlag, vCurrLoc, pNodeList[0], pDestMove->dwTimeStamp, pDestMove->byDestLocCount, pNodeList );
	...
	else
	{
		...
		if ( pDestMove->bHaveSecondDestLoc )
		{
			...
			CNtlSLEventGenerator::SobSecondDestMove( m_pActor->GetSerialID(), pDestMove->byMoveFlag, vCurrLoc, vDest, vSecondDest, pDestMove->dwTimeStamp );
		}
		else
		{
			CNtlSLEventGenerator::SobSecondDestMove( m_pActor->GetSerialID(), pDestMove->byMoveFlag, vCurrLoc, vDest, vDest, pDestMove->dwTimeStamp );
		}
	}
}
...
void CNtlFSMCharActAgent::ServerStateFollowing(sCHARSTATE *pServerState)
{
	...
	CNtlSLEventGenerator::SobFollowMove(m_pActor->GetSerialID(), pFollow->hTarget, pFollow->byMoveFlag, vLoc, pFollow->fDistance, 
										pFollow->byMovementReason, pFollow->dwTimeStamp);
}
...
void CNtlFSMCharActAgent::ServerStateDash(sCHARSTATE *pServerState)
{
	...
	CNtlSLEventGenerator::SobDash(m_pActor->GetSerialID(), vDest, pDash->byMoveDirection, pDash->dwTimeStamp);
}
...
void CNtlFSMCharActAgent::ServerStateFainting(sCHARSTATE *pServerState)
{
	...
	// CNtlSLEventGenerator::SobFainting(m_pActor->GetSerialID());
}
...
{
    pServerState->sCharStateDetail.sCharStateKeepingEffect;
...
RwUInt32 CNtlFSMCharActAgent::HandleEvents(RWS::CMsg &pMsg)
{
	...
	else
	{
		...
		else if(pMsg.Id == g_EventSobHit)
		{
			...
			if(m_uiServerCondition & CHARCOND_FLAG_BARRIER)
				bBarrier = TRUE;

			if(m_uiServerCondition & CHARCOND_FLAG_DAMAGE_REFLECTION)
				bReflection = TRUE;
		...
		else if(pMsg.Id == g_EventSobVehicleStart)
		{
			...
			CNtlBeCharData* pBeData = reinterpret_cast< CNtlBeCharData* >( m_pActor->GetBehaviorData() );
			SCtrlStuff* pCtrlStuff = pBeData->GetCtrlStuff();
			pCtrlStuff->sVehicle.vSpawnPos = pData->vSpawnPos;
...
void CNtlFSMCharActAgent::ConditionVisualColorFlickering(RwUInt32 uiServerCondition)
{
	RwUInt32 uiDiffCondition = m_uiServerCondition ^ uiServerCondition;
	...
	// condition이 clear 되었는가?

	if( !(m_uiServerCondition & CHARCOND_FLAG_BLEEDING) && !(m_uiServerCondition & CHARCOND_FLAG_POISON) &&
		!(m_uiServerCondition & CHARCOND_FLAG_STOMACHACHE) )
	{
		if(m_pSystemEffectColor)
			pSobProxy->RemoveVisualSystemEffectColorFlickering(m_pSystemEffectColor);
...
void CNtlFSMCharActAgent::ConditionVisualColorFlickeringPop(void)
{
	...
	if(m_uiServerCondition & CHARCOND_FLAG_BLEEDING)
	{
		m_pSystemEffectColor = m_pActor->GetSobProxy()->AddVisualSystemEffectColorFlickering(255, 255, 0, 2.0f);
		return;
	}

	if(m_uiServerCondition & CHARCOND_FLAG_POISON)
	{
		m_pSystemEffectColor = m_pActor->GetSobProxy()->AddVisualSystemEffectColorFlickering(102, 0, 102, 2.0f);
		return;
	}

	if(m_uiServerCondition & CHARCOND_FLAG_STOMACHACHE)
	{
		m_pSystemEffectColor = m_pActor->GetSobProxy()->AddVisualSystemEffectColorFlickering(0, 0, 255, 2.0f);
		return;
	}
	//TODO Add Burning Flag here. 
}
...
void CNtlFSMCharActAgent::ConditionDiff(RwUInt32 uiServerCondition)
{
	RwUInt32 uiDiffCondition = m_uiServerCondition ^ uiServerCondition;
    m_uiServerCondition = uiServerCondition;

    if(m_uiServerCondition == 0 && uiDiffCondition == 0)
    {
...
void CNtlFSMCharActAgent::ConditionAfterEffect(RwBool bAffect)
{
    ...
    if(bAffect)
    {
        // 실제 휴유증 처리는 Idle State로 전이할때 처리하고, 여기서는 플래그만 체크해둔다.
        if(m_pActor->GetCurrentState() && m_pActor->GetCurrentState()->GetStateId() == NTL_FSMSID_IDLE)
        {
            // Idle인 경우에만 휴유증상태로 가구, 다른 상태에서는 Idle 상태가 되길 기다린다.
