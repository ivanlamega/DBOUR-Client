...
protected:
    ...
    void            OnTransform_KaiOuKen(RwBool bTransform, BYTE byLevel, BYTE byRepeatCnt);        ///< 계왕권 변신 처리
	...
	CNtlPLItem * GetTransformItem(eASPECTSTATE state);

	void			OnTransform_SuperSaiyanBlue(RwBool bTransform);  //Testing New Transforms

    // Super Saiyan
    void            AttachEffectSuperSaiyan(CNtlPLCharacter* pPLCharacter);                      ///< 초사이어인 이펙트 부착
	void            AttachEffectSuperSaiyanBlue(CNtlPLCharacter* pPLCharacter);
