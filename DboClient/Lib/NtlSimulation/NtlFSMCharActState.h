...
private:	
    ...
    SERIAL_HANDLE   m_uiSkillTblId;               ///< 스킬의 테이블 ID 
    RwBool          m_bRightFinish;               ///< 캔슬되지 않고 정상적으로 종료됐는지 플래그
...
// 스카우터 사용상태 FSM
// 2008.02.29 by agebreak
class CNtlFSMCharActScouterState : public CNtlFSMCharActStateBase
{
    DECLEAR_MEMORY_POOL(CNtlFSMCharActScouterState, NTL_SMALL_MEMORY_POOL)
    DECLEAR_FSMSTATE_FACTROY(CNtlFSMCharActScouterState)

public:
    CNtlFSMCharActScouterState();    
    ...
    void    SetAnim(RwUInt32 uiAnimKey);
    void    DestroyEffect();        ///< 종료전의 처리 작업들

protected:
    RwBool  m_bIsUIOpen;            /// UI를 열기위해 사용하는 플래그
    RwBool  m_bIsAlphaBlend;        ///< 캐릭터 알파 블렌드 적용 플래그
    RwReal  m_fFPSInterTime;        ///< 카메라 변경 보간 타임   
    RwBool  m_bExit;
    SOUND_HANDLE m_hLoopSound;      ///< 스카우터 사용중 루프되는 사운드
    CNtlSobProxySystemEffect* pSystemEffect;    
};
...
class CNtlFSMCharAirMoveState : public CNtlFSMCharActStateBase
{
	DECLEAR_MEMORY_POOL(CNtlFSMCharAirMoveState, NTL_DEFAULT_MEMORY_POOL)
		DECLEAR_FSMSTATE_FACTROY(CNtlFSMCharAirMoveState)
