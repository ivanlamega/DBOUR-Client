...
void CDboTSCQAgency::GU_Quest_Share_Nfy( WORD wResultCode, HOBJECT hActor, NTL_TS_T_ID tId )
{
	if ( RESULT_SUCCESS == wResultCode )
	{
		...
		// 3. 해당 퀘스트가 반복 퀘스트가 아니면서 진행 된 적이 있는지 검사.
		...
		// 4. 다른 퀘스트의 진행 상태에 따라 공유 할 수 없는 경우 검사.
		mapdef_TRIGGER_LIST::iterator it = m_defCurTList.begin();
		for ( ; it != m_defCurTList.end(); ++it )
		{
			...
			if ( pCtrl && pCont )
			{
				// 실패나 에러 상태의 퀘스트는 진행이 중지 되어 있는 상태이므로
				// 퀘스트 공유에 영향을 미치지 않는다
				...
				// 조건 컨테이너, 보상 컨테이너, 분기 컨테이너, 종료 컨테이너가 아닌 경우는 퀘스트 공유 불가
		...
		for ( it = m_defCurTList.begin(); it != m_defCurTList.end(); ++it )
		{
			...
			{
				CNtlTSLog::Log( "Wrong quest id. Info[%u]. [%s]", tCurId, TS_CODE_TRACE() );
			}
		}
	...
	{
		CNtlSLEventGenerator::SysMsg( INVALID_SERIAL_ID, wResultCode );
	}
}

void CDboTSCQAgency::GU_Avatar_TS_Completed_Info( const sCOMPLETE_QUEST_INFO& sInfo )
{
	OUT_QMSG_0( "[Quest] Processing quest completion information..." );
...
{
	OUT_QMSG_0( "[Quest] Quest progress information processing..." );
	...
	case 0:
		...
		break;
	default:
		CNtlTSLog::Log( "The progress quest info version mismatch. Info[%u]. [%s]", sInfo.byVer, TS_CODE_TRACE() );
		break;
	}
}
...
{
	OUT_QMSG_6( "[Quest] User -> GameServer : Confirm in progress [%u,%u,%u,%u,%u,%u]", tId, tcCurId, tcNextId, uiParam, byEventType, uiEventData );
...
{
	OUT_QMSG_3( "[Quest] GameServer -> User : Confirm in progress [%u,%u,%u]", tId, tcCurId, tcNextId );
...
{
	OUT_QMSG_1( "[Quest] User -> GameServer : Give up in progress [%u]", tId );
...
{
	OUT_QMSG_2( "[Quest] GameServer -> User : Give up in progress [%u, %u]", wResultCode, tId );
...
{
	OUT_QMSG_3( "[Quest] GameServer -> User : Server event start message in progress [%u, %u, %u]", tId, tcId, taId );
...
{
	OUT_QMSG_3( "[Quest] GameServer -> User : Server event end message in progress [%u, %u, %u]", tId, tcId, taId );
...
{
	OUT_QMSG_4( "[Quest] GameServer -> User : Server event update message in progress [%u, %u, %u, %u]", tId, tcId, taId, bySvrEvtType );
...
{
	OUT_QMSG_4( "[Quest] GameServer -> User : Update TS state message in progress [%u, %u, %u, %u]", tId, byType, wTSState, uiParam );
...
{
	OUT_QMSG_4( "[Quest] User -> GameServer : Update TS state message in progress [%u, %u, %u, %u]", tId, byType, wTSState, uiParam );
...
{
	OUT_QMSG_1( "[Quest] GameServer -> User : Update TS Event in progress [%u]", eID );
...
{
	OUT_QMSG_0( "[Quest] GameServer -> User : Remove TMQ Quest in progress" );
...
{
	OUT_QMSG_3( "[Quest] Trigger -> User : Regist quest info nfy message in progress [%u, %u, %u]", tId, tcId, taId );
...
{
	OUT_QMSG_1( "[Quest] Trigger -> User : Unregist quest info nfy message in progress [%u]", tId );
...
//	호출되는 시점.
//
//	1. g_EventShowQuestWindow 이벤트가 발생했을 경우 호출된다.
//
//////////////////////////////////////////////////////////////////////////
void CDboTSCQAgency::TU_ShowQuestWindowNfy( NTL_TS_T_ID tId )
{
	OUT_QMSG_1( "[Quest] Trigger -> User : Show quest window nfy message in progress [%u]", tId );
...
{
	OUT_QMSG_1( "[Quest] Trigger -> User : Show quest indicator nfy message in progress [%u]", tId );
...
{
	OUT_QMSG_3( "[Quest] Trigger -> User : Update quest state nfy message in progress [%u, %u, %u]", tId, uiUpdatedQState, uiQState );
...
{
	OUT_QMSG_3( "[Quest] Trigger -> User : Update quest progress info nfy message in progress [%u, %u, %u]", tId, eEvtInfoType, uiTimeLimit );
...
{
	OUT_QMSG_3( "[Quest] Trigger -> User : Accept proposal quest nfy message in progress [%u, %u, %u]", tId, tgId, tcId );
...
{
	OUT_QMSG_3( "[Quest] Trigger -> User : Accept reward nfy message in progress [%u, %u, %u]", tId, tgId, tcId );
...
{
	OUT_QMSG_2( "[Quest] Trigger -> User : Accept give up nfy message in progress [%u, %u]", tId, tgId );
...
{
	OUT_QMSG_3( "[Quest] User -> Trigger : Do quest message in progress [%u, %u, %u]", sQuestInfo.sKey.tID, eEvtGenType, uiOwnerId );
	...
	if ( sQuestInfo.bVisitOP )
	{
		...
		CNtlTSLog::Log( "Not execute the visit server event. Info[%u,%u,%u]. [%s]", sQuestInfo.sKey.tID, eEvtGenType, uiOwnerId, TS_CODE_TRACE() );
	...
	if ( eQUEST_PROGRESS_TYPE_NEW == sQuestInfo.eProgType )
	{
		...
		for ( ; itCurQuest != m_defCurTList.end(); ++itCurQuest )
		{
			...
			{
				CNtlTSLog::Log( "Wrong quest id. Info[%u]. [%s]", tID, TS_CODE_TRACE() );
			}
		}
...
void CDboTSCQAgency::UT_DoTutorialQuest( NTL_TS_T_ID tId )
{
	...
	for ( ; itCurQuest != m_defCurTList.end(); ++itCurQuest )
	{
		...
		{
			CNtlTSLog::Log( "Wrong quest id. Info[%u]. [%s]", tProgID, TS_CODE_TRACE() );
		}
	}
	...
	if ( NTL_TSRESULT_TYPE_SUCCESS == tsResult )
	{
		...
		// Tutorial의 경우는 카메라 제어권을 가져온것으로 간주하고 동작한다
...
{
	OUT_QMSG_1( "[Quest] Trigger -> User : Show proposal dialog message in progress [%u]", sKey.tID );

	const sREWARD_INFO* pDefRwd = pActRwd ? &pActRwd->GetDefRewardInfo( 0, API_GetTableContainer()->GetQuestRewardTable() ) : NULL;
	const sREWARD_INFO* pSelRwd = pActRwd ? &pActRwd->GetSelRewardInfo( 0, API_GetTableContainer()->GetQuestRewardTable() ) : NULL;
...
{
	OUT_QMSG_2( "[Quest] User -> Trigger : Show proposal dialog message in progress [%u, %u]", sKey.tID, bAccept );
...
{
	OUT_QMSG_1( "[Quest] Trigger -> User : Show reward dialog message in progress [%u]", sKey.tID );

	const sREWARD_INFO* pDefRwd = pContRwd ? &pContRwd->GetDefRewardInfo( 0, API_GetTableContainer()->GetQuestRewardTable() ) : NULL;
	const sREWARD_INFO* pSelRwd = pContRwd ? &pContRwd->GetSelRewardInfo( 0, API_GetTableContainer()->GetQuestRewardTable() ) : NULL;
...
{
	OUT_QMSG_3( "[Quest] User -> Trigger : Show reward dialog message in progress [%u, %u, %u]", sKey.tID, nSelIdx, bCancel );
...
{
	OUT_QMSG_1( "[Quest] Trigger -> User : Show user selection dialog message in progress [%u]", sKey.tID );
	...
	default:
		{
			CNtlTSLog::Log( "Quest user select window type mismatch. Info[%u,%u]. [%s]", sKey.tID, sKey.tcID, TS_CODE_TRACE() );
...
{
	OUT_QMSG_3( "[Quest] User -> Trigger : Show user selection dialog message in progress [%u, %u, %u]", sKey.tID, tcSelId, bCancel );
...
{
	OUT_QMSG_1( "[Quest] Trigger -> User : Show narration dialog message in progress [%u]", sKey.tID );
...
{
	OUT_QMSG_2( "[Quest] User -> Trigger : Show narration dialog message in progress [%u, %u]", sKey.tID, bCancel );
...
{
	OUT_QMSG_1( "[Quest] Trigger -> User : Show unified narration dialog message in progress [%u]", sKey.tID );
	...
	if ( m_pUnifiedQuestNarration )
	{
		CNtlTSLog::Log( "Overlapped unified quest narration. Info[%u, %u]. [%s]", sKey.tID, m_pUnifiedQuestNarration->GetTSKey().tID, TS_CODE_TRACE() );
	...
	{
		CNtlTSLog::Log( "Building Unified quest narration failed . Info[%u, %u]. [%s]", sKey.tID, m_pUnifiedQuestNarration->GetTSKey().tID, TS_CODE_TRACE() );
...
{
	OUT_QMSG_2( "[Quest] User -> Trigger : Show unified narration dialog message in progress [%u, %u]", sKey.tID, bCancel );
...
{
	OUT_QMSG_1( "[Quest] Trigger -> User : Show server event message in progress [%u]", sKey.tID );
...
{
	OUT_QMSG_1( "[Quest] User -> Trigger : Show server event message in progress [%u]", sKey.tID );
...
{
	OUT_QMSG_1( "[Quest] Trigger -> User : Show npc conversation message in progress [%u]", sKey.tID );
...
{
	OUT_QMSG_1( "[Quest] User -> Trigger : Show npc conversation message in progress [%u]", sKey.tID );
...
{
	OUT_QMSG_1( "[Quest] Trigger -> User : Register quest info message in progress [%u]", sKey.tID );
...
{
	OUT_QMSG_1( "[Quest] User -> Trigger : Register quest info message in progress [%u]", sKey.tID );
...
{
	OUT_QMSG_1( "[Quest] Trigger -> User : Camera message in progress [%u]", sKey.tID );
...
{
	OUT_QMSG_1( "[Quest] User -> Trigger : Camera message in progress [%u]", sKey.tID );
...
{
	OUT_QMSG_1( "[Quest] Trigger -> User : Show object conversation message in progress [%u]", sKey.tID );
...
{
	OUT_QMSG_1( "[Quest] User -> Trigger : Show object conversation message in progress [%u]", sKey.tID );
...
{
	OUT_QMSG_1( "[Quest] Trigger -> User : Hint message in progress [%u]", sKey.tID );
...
{
	OUT_QMSG_1( "[Quest] User -> Trigger : Hint message in progress [%u]", sKey.tID );
...
{
	OUT_QMSG_1( "[Quest] Trigger -> User : Open window message in progress [%u]", sKey.tID );
...
{
	OUT_QMSG_1( "[Quest] User -> Trigger : Open window message in progress [%u]", sKey.tID );
...
{
	OUT_QMSG_1( "[Quest] User -> Trigger : PC conversation message in progress [%u]", sKey.tID );
...
{
	OUT_QMSG_1( "[Quest] Trigger -> User : Teleport Mudosa message in progress [%u]", sKey.tID );
...
{
	OUT_QMSG_1( "[Quest] User -> Trigger : Teleport Mudosa message in progress [%u]", sKey.tID );
...
{
	OUT_QMSG_1( "[Quest] Trigger -> User : Register client timing message in progress [%u]", sKey.tID );
...
{
	OUT_QMSG_1( "[Quest] User -> Trigger : Register client timing message in progress [%u]", sKey.tID );
...
{
	OUT_QMSG_1( "[Quest] Trigger -> User : Execute client group message in progress [%u]", sKey.tID );
...
{
	OUT_QMSG_1( "[Quest] User -> Trigger : Execute client group message in progress [%u]", sKey.tID );
