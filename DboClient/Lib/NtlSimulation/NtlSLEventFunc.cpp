...
void CNtlSLEventGenerator::SysMsg(SERIAL_HANDLE hSerialId, RwUInt32 uiMsg, RwBool bServerMsg /* = TRUE */)
{
...
void CNtlSLEventGenerator::FormatSysMsg( SERIAL_HANDLE hSerialId, RwUInt32 uiMsg, RwBool bServerMsg, ... )
{
...
{
	CNtlSob *pObj = CNtlSobManager::GetInstance()->GetSobObject(hSerialId);   
...
void CNtlSLEventGenerator::CameraDash(SERIAL_HANDLE hSerialId, RwReal fDashSpeed, RwV3d vDest)
{
	...
	RwV3dAssign(&sCameraDash.vDest, &vDest);
...
{
	CNtlSobActor* pObj = (CNtlSobActor*)CNtlSobManager::GetInstance()->GetSobObject(hSerialID);
...
{
	CNtlSobActor* pObject = (CNtlSobActor*)CNtlSobManager::GetInstance()->GetSobObject( hSerialID );
...
{
	CNtlSobActor* pActor = (CNtlSobActor*)CNtlSobManager::GetInstance()->GetSobObject( hSerialID );
...
void CNtlSLEventGenerator::CameraDirectSplineNode(RwReal fDelayTime, RwV3d vStartPos, RwV3d vEndPos, RwV3d vLookAtPos, RwUInt8 byDirectLookAtType, const RwChar *pKey)
{
	...
	RwV3dAssign(&sCameraDTSplineNode.vStartPos, &vStartPos);
	RwV3dAssign(&sCameraDTSplineNode.vEndPos, &vEndPos);
...
void CNtlSLEventGenerator::ActionMapSkillUse(SERIAL_HANDLE hSerialId, RwBool bExistTarget, SERIAL_HANDLE hTargetSerialId, 
											 SERIAL_HANDLE hSkillSerialId, RwReal fFollowRange, RwUInt8 bySkillSlotIdx, RwUInt8 byRpBonusType)
{
...
void CNtlSLEventGenerator::ActionMapItemUse(SERIAL_HANDLE hSerialId, SERIAL_HANDLE hTargetSerialId, RwUInt8 byPlace, RwUInt8 byPos, RwUInt8 byCastingTime)
{
	...
	RWS::CMsg msg;
	msg.Id = g_EventActionMapItemUse;
...
void CNtlSLEventGenerator::ActionMapScouterUse()
{
	SEND_MSG(g_EventActionMapScouterUse, NULL);
}
...
void CNtlSLEventGenerator::TerrainClick(RwV3d vPickPos, RwBool bServerSend /*= TRUE*/)
{
	...
	RwV3dAssign(&sTerrainClick.vPickPos, &vPickPos);
...
RwBool CNtlSLEventGenerator::SobTargetSelect(SERIAL_HANDLE hSerialId)
{
	CNtlSob* pObj = CNtlSobManager::GetInstance()->GetSobObject( hSerialId );
...
void CNtlSLEventGenerator::MouseDashMove(RwV3d vPickPos, RwReal fLimitDist)
{
	...
	RwV3dAssign(&sMouseDashMove.vPickPos, &vPickPos);
	sMouseDashMove.fLimitDist = fLimitDist;

	RWS::CMsg msg;
	msg.Id = g_EventMouseDashMove;
	msg.pData = reinterpret_cast<void*>(&sMouseDashMove);
	_SendMsg(msg);
}
...
void CNtlSLEventGenerator::KeyboardDashMove(RwUInt32 uiMoveFlags)
{
	...
	RWS::CMsg msg;
	msg.Id = g_EventKeyboardDashMove;
	msg.pData = reinterpret_cast<void*>(&sKeyboardDashMove);
	_SendMsg(msg);
}

void CNtlSLEventGenerator::InputChagneHeading(RwBool bSmoothing, RwReal fDeltaAngle, RwBool& bSuccess)
{
	SNtlEventInputChangeHading sChangeHeading;
	sChangeHeading.bSuccess = TRUE;
	sChangeHeading.bSoothing = bSmoothing;
	sChangeHeading.fDeltaAngle = fDeltaAngle;
...
void* CNtlSLEventGenerator::SobPlayerCreate(RwUInt32 uiClassId, SERIAL_HANDLE hSerialId, RwV3d vLoc, RwV3d vDir, sPC_BRIEF *pPcBrief, sCHARSTATE *pCharState, RwBool bIsClientCreate /* = FALSE */)
{
	...
	sSobCreate.uiClassId = uiClassId;
	sSobCreate.hSerialId = hSerialId;
	RwV3dAssign(&sSobCreate.vLoc, &vLoc);
	RwV3dAssign(&sSobCreate.vDir, &vDir);
...
void* CNtlSLEventGenerator::SobWorldItemCreate(RwUInt32 uiClassId, SERIAL_HANDLE hSerialId, RwV3d vLoc, RwInt32 eObjType, void* pBrief, void* pState)
{
	SNtlEventSobWorldItemCreate sWorldItemCreate;
	sWorldItemCreate.uiClassId = uiClassId;
	sWorldItemCreate.hSerialId = hSerialId;
	RwV3dAssign( &sWorldItemCreate.vLoc, &vLoc );
...
void* CNtlSLEventGenerator::SobSlotItemCreate(void *pParentSobItem, RwUInt32 uiClassId, SERIAL_HANDLE hSerialId, 
											  RwUInt32 uiItemTblId, RwUInt32 uiOptionTblId1, RwUInt32 uiOptionTblId2, RwUInt8 byPlace, RwUInt8 bySlotIdx, RwUInt8 byStackCount,
											  ...
											  RwUInt8 byRestrictType, const WCHAR* wszMaker, RwUInt8 byDurationType, DBOTIME StartTime, DBOTIME EndTime)
{
	...
	sSobCreate.uiOptionTblId1 = uiOptionTblId1;
	sSobCreate.uiOptionTblId2 = uiOptionTblId2;
	...
	sSobCreate.byRestrictType = byRestrictType;
...
void* CNtlSLEventGenerator::SobBuffCreate(RwUInt32 uiClassId, SERIAL_HANDLE hSerialId, RwUInt8 byBuffType, RwUInt32 uiTblId, RwUInt32 uiRemainTime, RwUInt32 uiKeepTime, RwReal* apEffectValue)
{
	...
	for( RwInt32 i = 0 ; i < NTL_MAX_EFFECT_IN_SKILL ; ++i )
		sSobCreate.afEffectValue[i] = apEffectValue[i];
...
void CNtlSLEventGenerator::SobDirectionFloat(SERIAL_HANDLE hSerialId, RwUInt8 byMoveFlags, RwV3d vDir)
{
...
void CNtlSLEventGenerator::SobItemAdd(SERIAL_HANDLE hSerialId, SERIAL_HANDLE hItemSerialId, RwUInt32 uiItemTblId, 
									  RwUInt32 uiOptionTblId1, RwUInt32 uiOptionTblId2, RwUInt8 byPlace, RwUInt8 bySlotIdx, RwUInt8 byStackCount, 
									  ...
									  RwUInt8 byRestrictType, const WCHAR* wszMaker, RwUInt8 byDurationType, DBOTIME StartTime, DBOTIME EndTime)
{
	...
	sSobItemAdd.uiOptionTblId1 = uiOptionTblId1;
	sSobItemAdd.uiOptionTblId2 = uiOptionTblId2;
	...
	sSobItemAdd.byRestrictType = byRestrictType;
										 ...
										 RwUInt8 byRestrictType, const WCHAR* wszMaker, RwUInt8 byDurationType, DBOTIME StartTime, DBOTIME EndTime)
{
	...
	sSobItemUpdate.uiOptionTblId1 = uiOptionTblId1;
	sSobItemUpdate.uiOptionTblId2 = uiOptionTblId2;
	...
	sSobItemUpdate.byRestrictType = byRestrictType;
...
void CNtlSLEventGenerator::SobEquipModelChange(SERIAL_HANDLE hSerialId, RwUInt8 bySlotIdx, RwUInt32 uiTblId, RwUInt8 byGrade, RwUInt8 byBattleAttribute)
{
	...
	RWS::CMsg msg;
	msg.Id = g_EventSobEquipChange;
	msg.pData = reinterpret_cast<void*>(&sSobEquipChange);
	_SendMsg(msg);
}
...
void CNtlSLEventGenerator::SobItemUseAction(SERIAL_HANDLE hSerialId, RwUInt32 uiTableIdx, RwUInt8 ucSkillResultCount, sSKILL_RESULT* pSkillResult)
{
	...
	RWS::CMsg msg;
	msg.Id = g_EventSobItemUseAction;
	msg.pData = reinterpret_cast<void*>( &sSobItemUseAction );
	_SendMsg( msg );
}

void CNtlSLEventGenerator::SobItemUpgrade(RwUInt8 byBattleAttribute, RwUInt8 byStoneStack, RwUInt8 byItemGrade, RwUInt8 byItemPlace, RwUInt8 byItemPos, RwUInt8 byStonePlace, RwUInt8 byStonePos, RwBool bItemDestroy)
{
	SNtlEventSobItemUpgrade sSobItemUpgrade;
	sSobItemUpgrade.byBattleAttribute	= byBattleAttribute;
...
{
	SNtlEventSobDeleteQuickSlotIcon sSobDeleteQuickSlotIcon;
	sSobDeleteQuickSlotIcon.hSerialId		= hSerialId;

	RWS::CMsg msg;
	msg.Id = g_EventSobDeleteQuickSlotIcon;
	msg.pData = reinterpret_cast<void*>( &sSobDeleteQuickSlotIcon );
	_SendMsg( msg );
}
...
void CNtlSLEventGenerator::SobStanding(SERIAL_HANDLE hSerialId, RwV3d vLoc, RwV3d vDir)
{
	...
	RwV3dAssign(&sSobStanding.vLoc, &vLoc);
	RwV3dAssign(&sSobStanding.vDir, &vDir);

	RWS::CMsg msg;
	msg.Id = g_EventSobStanding;
	msg.pData = reinterpret_cast<void*>(&sSobStanding);
	_SendMsg(msg);
}
...
void CNtlSLEventGenerator::SobAttack(SERIAL_HANDLE hSerialId, SERIAL_HANDLE hAttackerSerialId, SERIAL_HANDLE hDefenderSpawnId, 
									 ...
									 RwV3d vPushDir, RwUInt32 uiSyncEventId)
{
	...
	sSobAttack.vPushDir				= vPushDir;
	sSobAttack.uiSyncEventId		= uiSyncEventId;

	RWS::CMsg msg;
	msg.Id = g_EventSobAttack;
	msg.pData = reinterpret_cast<void*>(&sSobAttack);
	_SendMsg(msg);
}
...
void CNtlSLEventGenerator::SobJump(SERIAL_HANDLE hSerialId, RwUInt8 byMoveFlags, RwV3d vDir, RwV3d vJumpDir)
{
	SNtlEventSobJump sSobJump;

	sSobJump.hSerialId = hSerialId;
	sSobJump.byMoveFlags = byMoveFlags;
	sSobJump.vDir = vDir;
...
void CNtlSLEventGenerator::SobSkillAction(SERIAL_HANDLE hSerialId, RwUInt16 wResultCode, RwUInt32 uiTargetSerialI, RwUInt32 uiSkillTblId, 
										  RwUInt8 byRpBonusType, RwUInt8 byResultCount,  sSKILL_RESULT *pSkillAct, RwUInt32 uiSyncEventId)
{
...
void CNtlSLEventGenerator::SobBuffAdd(SERIAL_HANDLE hSerialId, RwUInt8 byBuffType, RwUInt32 uiTblId, RwUInt32 uiRemainTime, RwUInt32 uiKeepTime, RwReal* afEffectValue)
{
	...
	for( RwInt32 i = 0 ; i < NTL_MAX_EFFECT_IN_SKILL ; ++i )
		sSobBuffAdd.afEffectValue[i] = afEffectValue[i];
...
void CNtlSLEventGenerator::SobBuffDrop(SERIAL_HANDLE hSerialId, RwUInt8 byBuffType, RwUInt32 uiTblId)
{
	SNtlEventSobBuffDrop sSobBuffDrop;

	sSobBuffDrop.hSerialId = hSerialId;
	sSobBuffDrop.byBuffType = byBuffType;
	sSobBuffDrop.uiTblId = uiTblId;
...
void CNtlSLEventGenerator::SobBuffRefreshAll(SERIAL_HANDLE hSerial, RwUInt8 byCount, sBUFF_INFO* pBuffInfo)
{
	...
	RWS::CMsg msg;
	msg.Id = g_EventSobBuffRefreshAll;
	msg.pData = reinterpret_cast<void*>(&sSobBuffRefreshAll);
	_SendMsg(msg);
}

void CNtlSLEventGenerator::SobFakeBuffAdd(SERIAL_HANDLE hSerialId, RwUInt8 byBuffType, RwUInt32 uiTblId, RwUInt32 uiRemainTime, RwUInt32 uiKeepTime, RwReal* afEffectValue)
{
	...
	for( RwInt32 i = 0 ; i < NTL_MAX_EFFECT_IN_SKILL ; ++i )
		sSobFakeBuffAdd.afEffectValue[i] = afEffectValue[i];
...
void CNtlSLEventGenerator::SobFakeBuffDrop(SERIAL_HANDLE hSerialId, RwUInt8 byBuffType, RwUInt32 uiTblId)
{
	SNtlEventFakeBuffDrop sSobFakeBuffDrop;

	sSobFakeBuffDrop.hSerialId = hSerialId;
	sSobFakeBuffDrop.byBuffType = byBuffType;
	sSobFakeBuffDrop.uiTblId = uiTblId;
...
void CNtlSLEventGenerator::SobServerUpdateCondition(SERIAL_HANDLE hSerialId, DWORD dwCondition)
{
	SNtlEventSobServerUpdateCondition sSobServerCondition;
	sSobServerCondition.hSerialId		= hSerialId;
	sSobServerCondition.dwCondition		= dwCondition;		
...
void CNtlSLEventGenerator::SobTargetUpdate(SERIAL_HANDLE hSerialId, RwBool bShow)
{
	...
	RWS::CMsg msg;
	msg.Id = g_EventSobTargetInfoUpdate;
	msg.pData = reinterpret_cast<void*>(&sSobTargetInfoUpdate);
	_SendMsg(msg);
}
...
void CNtlSLEventGenerator::PartyMemberAdd(SERIAL_HANDLE hSerial, WCHAR* pwcName, RwUInt8 byRace /* = 0 */, RwUInt8 byClass /* = 0 */,
										  RwUInt8 byLevel /* = 0 */, RwUInt32 wCurLP /* = 0 */, RwUInt32 wMaxLP /* = 0 */,
...
{	
	...
	sPartyAdd.wCurLP = (RwUInt16)wCurLP;
	sPartyAdd.wMaxLP = (RwUInt16)wMaxLP;
...
void CNtlSLEventGenerator::PartyCharmActivity(RwUInt32 uiMessage, RwUInt8 byIndex,
											  RwUInt32 uiTableIndex /* = INVALID_SERIAL_ID */, RwReal fRemainTime /* = 0.f */)
{
	sPartyCharmActivity sPacket;

	sPacket.uiMessage = uiMessage;
	sPacket.byIndex = byIndex;
	sPacket.uiTableIndex = uiTableIndex;
	sPacket.fRemainTime = fRemainTime;

	RWS::CMsg msg;
	msg.Id = g_EventPartyCharmActivity;
	msg.pData = reinterpret_cast<void*>(&sPacket);
	_SendMsg(msg);
}
...
void CNtlSLEventGenerator::SobGetState(SERIAL_HANDLE hSerialId, RwUInt8& byServerState, RwUInt8& byClientState)
{
...
void CNtlSLEventGenerator::SobGetFightingMode(SERIAL_HANDLE hSerialId, RwBool& bFightingMode)
{
	...
	bFightingMode = sSobGetFightingMode.bFightingMode;
}
...
void CNtlSLEventGenerator::TSOpenBattleStart(SERIAL_HANDLE hSource)
{
	SNtlEventTSOpenBattleStart sData;
	sData.handle = hSource;

	RWS::CMsg msg;
	msg.Id = g_EventTSOpenBattleStart;
	msg.pData = reinterpret_cast<void*>(&sData);
	_SendMsg(msg);
}

void CNtlSLEventGenerator::TSOpenBattleEnd(SERIAL_HANDLE hSource)
{
	SNtlEventTSOpenBattleEnd sData;
	sData.handle = hSource;
	RWS::CMsg msg;
	msg.Id = g_EventTSOpenBattleEnd;
	msg.pData = reinterpret_cast<void*>(&sData);
	_SendMsg(msg);
}
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
void CNtlSLEventGenerator::TrigObjScouterDir_CreateSTObj( SERIAL_HANDLE hSerial )
{
	SNtlEventTrigObjScouterDir_CreateSTObj Data;

	Data.hSerial = hSerial;

	RWS::CMsg msg;
	msg.Id = g_EventTrigObjScouterDir_CreateSTObj;
	msg.pData = reinterpret_cast<void*>(&Data);
	_SendMsg(msg);
}

void CNtlSLEventGenerator::TrigObjScouterDir_DeleteSTObj( SERIAL_HANDLE hSerial )
{
	SNtlEventTrigObjScouterDir_DeleteSTObj Data;

	Data.hSerial = hSerial;

	RWS::CMsg msg;
	msg.Id = g_EventTrigObjScouterDir_DeleteSTObj;
	msg.pData = reinterpret_cast<void*>(&Data);
	_SendMsg(msg);
}
...
void CNtlSLEventGenerator::PrivateShopNameTag(SERIAL_HANDLE hOwnerSerial, RwBool bIsOwnerEmpty, RwUInt32 uiPrivateShopState, WCHAR* pwcPrivateShopName)
{
	...
	/*
	if (pwcPrivateShopName)
	{
	wcscpy_s(sPrivateShopNameTag.awcPrivateShopName, NTL_MAX_PRIVATESHOP_NAME + 1, pwcPrivateShopName);
	}
	else
	{
	memset(sPrivateShopNameTag.awcPrivateShopName, NULL, sizeof(WCHAR) * (NTL_MAX_PRIVATESHOP_NAME + 1));
	}
	*/
...
void CNtlSLEventGenerator::PrivateShopBusinessRequestRes( RwUInt16 wResultCode, RwUInt8 byPrivateShopInventorySlotPos, RwUInt8 byItemState ) 
{
	SNtlEventPrivateShopBusinessRequestRes sPrivateShopBusinessRequestRes;
	sPrivateShopBusinessRequestRes.wResultCode = wResultCode;
	sPrivateShopBusinessRequestRes.byPrivateShopInventorySlotPos = byPrivateShopInventorySlotPos;
	sPrivateShopBusinessRequestRes.byItemState = byItemState;

	RWS::CMsg msg;
	msg.Id = g_EventPrivateShopBusinessRequestRes;
	msg.pData = reinterpret_cast<void*>(&sPrivateShopBusinessRequestRes);
	_SendMsg(msg);
}

void CNtlSLEventGenerator::PrivateShopBusinessRequestNfy( RwUInt8 byPrivateShopInventorySlotPos, RwUInt8 byItemState ) 
{
	SNtlEventPrivateShopBusinessRequestNfy sPrivateShopBusinessRequestNfy;
	sPrivateShopBusinessRequestNfy.byPrivateShopInventorySlotPos = byPrivateShopInventorySlotPos;
	sPrivateShopBusinessRequestNfy.byItemState = byItemState;

	RWS::CMsg msg;
	msg.Id = g_EventPrivateShopBusinessRequestNfy;
	msg.pData = reinterpret_cast<void*>(&sPrivateShopBusinessRequestNfy);
	_SendMsg(msg);
}

void CNtlSLEventGenerator::PrivateShopBusinessConsentReq( SERIAL_HANDLE hVisitorID, RwUInt8 byPrivateShopInventorySlotPos ) 
{
	SNtlEventPrivateShopBusinessConsentReq sPrivateShopBusinessConsentReq;
	sPrivateShopBusinessConsentReq.hVisitorID = hVisitorID;
	sPrivateShopBusinessConsentReq.byPrivateShopInventorySlotPos = byPrivateShopInventorySlotPos;

	RWS::CMsg msg;
	msg.Id = g_EventPrivateShopBusinessConsentReq;
	msg.pData = reinterpret_cast<void*>(&sPrivateShopBusinessConsentReq);
	_SendMsg(msg);
}

void CNtlSLEventGenerator::PrivateShopBusinessConsentNfy( SERIAL_HANDLE hOwner, RwUInt8 byPrivateShopInventorySlotPos ) 
{
	SNtlEventPrivateShopBusinessConsentNfy sPrivateShopBusinessConsentNfy;
	sPrivateShopBusinessConsentNfy.hOwner = hOwner;
	sPrivateShopBusinessConsentNfy.byPrivateShopInventorySlotPos = byPrivateShopInventorySlotPos;

	RWS::CMsg msg;
	msg.Id = g_EventPrivateShopBusinessConsentNfy;
	msg.pData = reinterpret_cast<void*>(&sPrivateShopBusinessConsentNfy);
	_SendMsg(msg);
}

void CNtlSLEventGenerator::PrivateShopBusinessWaittimeOutNfy( SERIAL_HANDLE hOwner, RwUInt8 byPrivateShopInventorySlotPos ) 
{
	SNtlEventPrivateShopBusinessWaittimeOutNfy sPrivateShopBusinessWaittimeOutNfy;
	sPrivateShopBusinessWaittimeOutNfy.hOwner = hOwner;
	sPrivateShopBusinessWaittimeOutNfy.byPrivateShopInventorySlotPos = byPrivateShopInventorySlotPos;

	RWS::CMsg msg;
	msg.Id = g_EventPrivateShopBusinessWaittimeOutNfy;
	msg.pData = reinterpret_cast<void*>(&sPrivateShopBusinessWaittimeOutNfy);
	_SendMsg(msg);

}

void CNtlSLEventGenerator::PrivateShopBusinessPricefluctuationsRes( RwUInt16 wResultCode, RwUInt8 byPrivateShopInventorySlotPos, RwUInt32 dwBeforeZenny, RwUInt32 dwAfterZenny ) 
{
	SNtlEventPrivateShopBusinessPricefluctuationsRes sPrivateShopBusinessPricefluctuationsRes; 
	sPrivateShopBusinessPricefluctuationsRes.wResultCode = wResultCode;
	sPrivateShopBusinessPricefluctuationsRes.byPrivateShopInventorySlotPos = byPrivateShopInventorySlotPos;
	sPrivateShopBusinessPricefluctuationsRes.dwBeforeZenny = dwBeforeZenny;
	sPrivateShopBusinessPricefluctuationsRes.dwAfterZenny = dwAfterZenny;

	RWS::CMsg msg;
	msg.Id = g_EventPrivateShopBusinessPricefluctuationsRes;
	msg.pData = reinterpret_cast<void*>(&sPrivateShopBusinessPricefluctuationsRes);
	_SendMsg(msg);
}

void CNtlSLEventGenerator::PrivateShopBusinessPricefluctuationsNfy( SERIAL_HANDLE hOwner, RwUInt8 byPrivateShopInventorySlotPos, RwUInt32 dwBeforeZenny, RwUInt32 dwAfterZenny ) 
{
	SNtlEventPrivateShopBusinessPricefluctuationsNfy sPrivateShopBusinessPricefluctuationsNfy;
	sPrivateShopBusinessPricefluctuationsNfy.hOwner = hOwner;
	sPrivateShopBusinessPricefluctuationsNfy.byPrivateShopInventorySlotPos = byPrivateShopInventorySlotPos;
	sPrivateShopBusinessPricefluctuationsNfy.dwBeforeZenny = dwBeforeZenny;
	sPrivateShopBusinessPricefluctuationsNfy.dwAfterZenny = dwAfterZenny;

	RWS::CMsg msg;
	msg.Id = g_EventPrivateShopBusinessPricefluctuationsNfy;
	msg.pData = reinterpret_cast<void*>(&sPrivateShopBusinessPricefluctuationsNfy);
	_SendMsg(msg);
}

void CNtlSLEventGenerator::PrivateShopBusinessImmediateItemBuyingRes( RwUInt16 wResultCode, sPRIVATESHOP_ITEM_DATA* pPrivateShopItemData ) 
{
	SNtlEventPrivateShopBusinessImmediateItemBuyingRes sPrivateShopBusinessImmediateItemBuyingRes;
	sPrivateShopBusinessImmediateItemBuyingRes.wResultCode = wResultCode;
	sPrivateShopBusinessImmediateItemBuyingRes.sPrivateShopItemData = *pPrivateShopItemData;

	RWS::CMsg msg;
	msg.Id = g_EventPrivateShopBusinessImmediateItemBuyingRes;
	msg.pData = reinterpret_cast<void*>(&sPrivateShopBusinessImmediateItemBuyingRes);
	_SendMsg(msg);
}

void CNtlSLEventGenerator::PrivateShopBusinessImmediateItemBuyingNfy( SERIAL_HANDLE hPrivateShopOwner, sPRIVATESHOP_ITEM_DATA* pPrivateShopItemData ) 
{
	SNtlEventPrivateShopBusinessImmediateItemBuyingNfy sPrivateShopBusinessImmediateItemBuyingNfy;
	sPrivateShopBusinessImmediateItemBuyingNfy.hPrivateShopOwner = hPrivateShopOwner;
	sPrivateShopBusinessImmediateItemBuyingNfy.sPrivateShopItemData = *pPrivateShopItemData;

	RWS::CMsg msg;
	msg.Id = g_EventPrivateShopBusinessImmediateItemBuyingNfy;
	msg.pData = reinterpret_cast<void*>(&sPrivateShopBusinessImmediateItemBuyingNfy);
	_SendMsg(msg);
}

void CNtlSLEventGenerator::PrivateShopBusinessCancleRes( RwUInt16 wResultCode, RwUInt8 byCancleType, RwUInt8 byPrivateShopInventorySlotPos ) 
{
	SNtlEventPrivateShopBusinessCancleRes sPrivateShopBusinessCancleRes;
	sPrivateShopBusinessCancleRes.wResultCode = wResultCode;
	sPrivateShopBusinessCancleRes.byCancelType = byCancleType;
	sPrivateShopBusinessCancleRes.byPrivateShopInventorySlotPos = byPrivateShopInventorySlotPos;

	RWS::CMsg msg;
	msg.Id = g_EventPrivateShopBusinessCancleRes;
	msg.pData = reinterpret_cast<void*>(&sPrivateShopBusinessCancleRes);
	_SendMsg(msg);
}

void CNtlSLEventGenerator::PrivateShopBusinessCancleNfy( SERIAL_HANDLE hOWner, RwUInt8 byCancleType, RwUInt8 byPrivateShopInventorySlotPos ) 
{
	SNtlEventPrivateShopBusinessCancleNfy sPrivateShopBusinessCancleNfy;
	sPrivateShopBusinessCancleNfy.hOwner = hOWner;
	sPrivateShopBusinessCancleNfy.byCancelType = byCancleType;
	sPrivateShopBusinessCancleNfy.byPrivateShopInventorySlotPos = byPrivateShopInventorySlotPos;

	RWS::CMsg msg;
	msg.Id = g_EventPrivateShopBusinessCancleNfy;
	msg.pData = reinterpret_cast<void*>(&sPrivateShopBusinessCancleNfy);
	_SendMsg(msg);

}

void CNtlSLEventGenerator::ChatMessagePrivateShopBusiness( WCHAR* pwchSenderCharName, RwUInt16 wMessageLengthInUnicode, WCHAR* pwcMessage ) 
{
	SNtlEventChatMessagePrivateShopBusiness sChatMessagePrivateShopBusiness; 
	sChatMessagePrivateShopBusiness.pwchSenderCharName = pwchSenderCharName;
	sChatMessagePrivateShopBusiness.wMessageLengthInUnicode = wMessageLengthInUnicode;
	memset( sChatMessagePrivateShopBusiness.awcMessage, 0, NTL_MAX_LENGTH_OF_CHAT_MESSAGE_UNICODE*sizeof(WCHAR) + 1);
	memcpy( sChatMessagePrivateShopBusiness.awcMessage, pwcMessage, wMessageLengthInUnicode*sizeof(WCHAR) );
	sChatMessagePrivateShopBusiness.awcMessage[wMessageLengthInUnicode] = L'\0';

	RWS::CMsg msg;
	msg.Id = g_EventChatMessagePrivateShopBusiness;
	msg.pData = reinterpret_cast<void*>(&sChatMessagePrivateShopBusiness);
	_SendMsg(msg);
}

void CNtlSLEventGenerator::SLTMQBoard(RwUInt32 uiEventType, RwUInt32 uiTargetHandle /* = 0xffffffff */,
									  RwUInt32 uiValue /* = 0xffffffff */, void* pExData /* = NULL */)
{
	SNtlEventTMQBoard sTMQEvent;
	...
	RWS::CMsg msg;
	msg.Id		= g_EventTMQBoard;
	msg.pData	= reinterpret_cast<void*>(&sTMQEvent);
	_SendMsg(msg);
}

void CNtlSLEventGenerator::SLTMQBoardNotify(RwUInt32 uiEventType, RwUInt32 uiTargetHandle /* = 0xffffffff */,
											RwUInt32 uiValue /* = 0xffffffff */, void* pExData /* = NULL */)
{
	SNtlEventTMQBoardNotify sTMQEvent;
	sTMQEvent.uiEventType	= uiEventType;
	sTMQEvent.hTargetHandle = uiTargetHandle;
	sTMQEvent.uiValue = uiValue;
	sTMQEvent.pExData = pExData;
...
void CNtlSLEventGenerator::PortalStartRes(WORD wResultCode, RwUInt32 hNPCSerialID)
{
	// 포탈 NPC를 클릭했을때의 응답 이벤트
	SNtlEventPortalStartRes data;
	data.wResultCode = wResultCode;    
	data.hNPCSerailID = hNPCSerialID;
	SEND_MSG(g_EventPortalStartRes, &data);    
}

void CNtlSLEventGenerator::PortalInfoRes( BYTE byCount, BYTE* paPortalID ) 
{
...
void CNtlSLEventGenerator::PortalAddRes( WORD wResultCode, BYTE portalID ) 
{
...
void CNtlSLEventGenerator::PortalRes( WORD wResultCode, BYTE byPoint ) 
{
...
void CNtlSLEventGenerator::FriendAddRes(WORD wResultCode, WCHAR* wchName, RwUInt32 uiTargetID)
{
	...
	swprintf_s(data.wchName, L"%s", wchName);
...
void CNtlSLEventGenerator::FriendMoveRes(WORD wReslutCode, RwUInt32 uiTargetID)
{
	...
	// 캐릭터 이름 (블랙 리스트에 추가하기 위해서 필요하다)
	swprintf_s(data.wchName, L"%s", GetNtlSLGlobal()->GetSobAvatar()->GetFriendList()->GetMemberbyKey(uiTargetID)->wszMemberName);
...
void CNtlSLEventGenerator::FriendInfo(sFRIEND_INFO* pInfo, RwBool bIsFirst)
{
	SNtlEventFriendInfo data;
	memcpy(&data.sInfo, pInfo, sizeof(sFRIEND_INFO));
    data.bIsFirst = bIsFirst ? true : false;
...
void CNtlSLEventGenerator::ScouterEvent(RwInt32 iType, RwUInt32 uiSerailID /* = 0 */, RwInt32 iUserData2 /* = 0 */, RwInt32 iUserData3 /* = 0 */)
{
...
void CNtlSLEventGenerator::SobVehicleStart( SERIAL_HANDLE hDriver, const RwV3d& vSpawnPos,
...
void CNtlSLEventGenerator::HoipoiMixItemMachineDel( void )
{
	SEND_MSG( g_EventHoipoiMixItemMachineDel, NULL );
}
...
void CNtlSLEventGenerator::HoipoiMixItemRecipeInfo( RwUInt8 byCount, void* pRecipeData )
{
...
void CNtlSLEventGenerator::HoipoiMixItemMakeExp( RwUInt32 dwCurExp, RwUInt8 byCurLevel )
{
...
void CNtlSLEventGenerator::HoipoiMixJobSetRes( SERIAL_HANDLE hNpcHandle, RwUInt8 byRecipeType )
{
	SNtlEventHoipoiMixJobSetRes sData;
	sData.hNpcHandle = hNpcHandle;
	sData.byRecipeType = byRecipeType;

	SEND_MSG( g_EventHoipoiMixJobSetRes, &sData );
}

void CNtlSLEventGenerator::HoipoiMixJobResetRes( SERIAL_HANDLE hNpcHandle, RwUInt8 byRecipeType )
{
	SNtlEventHoipoiMixJobResetRes sData;
	sData.hNpcHandle = hNpcHandle;
	sData.byRecipeType = byRecipeType;

	SEND_MSG( g_EventHoipoiMixJobResetRes, &sData );
}
...
void CNtlSLEventGenerator::MyNameplateMode(SERIAL_HANDLE hSerialID)
{
	SNtlEventMyNameplateMode data;
	data.hSerialId = hSerialID;

	SEND_MSG(g_EventMyNameplateChanged, &data);
}

void CNtlSLEventGenerator::OtherNameplateMode(SERIAL_HANDLE hSerialID)
{
	SNtlEventOtherNameplateMode data;
	data.hSerialId = hSerialID;

	SEND_MSG(g_EventOtherNameplateChanged, &data);
}
