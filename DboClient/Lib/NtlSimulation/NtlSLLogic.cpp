...
RwBool Logic_SetDevUser(void)
{
	SDevMasterInfo *pMasterInfo = API_GetSLDevMasterInformation();
	if (pMasterInfo != NULL && !pMasterInfo->bDevUser)
	{
		pMasterInfo->bDevUser = 1;
		return TRUE;
	}
	return FALSE;
}
...
RwBool Logic_IsCostumeItem(sITEM_TBLDAT *pItemTblData)
{
	...
	if (pItemTblData->byItem_Type == ITEM_TYPE_COSTUME_SET ||
		pItemTblData->byItem_Type == ITEM_TYPE_COSTUME_HAIR_STYLE || pItemTblData->byItem_Type == ITEM_TYPE_COSTUME_MASK ||
		pItemTblData->byItem_Type == ITEM_TYPE_HAIR_ACCESSORY || pItemTblData->byItem_Type == ITEM_TYPE_BACK_ACCESSORY ||
		pItemTblData->byItem_Type == ITEM_TYPE_DOGI || pItemTblData->byItem_Type == ITEM_TYPE_COSTUME_QUEST)
	...
	case SLCLASS_AVATAR:
		{
				...
				//if player is a gamemaster
				if (pAvatarInfo->sCharPf.bIsGameMaster)
				{
					pPlayerProxy->SetNameColor(pAvatarAttr->GetName(), avatarNameColor,
						pGuild->GetGuildName(), NTL_GAME_MASTER_NAME_COLOR, TRUE);
				}
				if (GetNtlStorageManager()->GetBoolData(dSTORAGE_GAMEINFO_MY_NAMEPLATE) == TRUE)
					pPlayerProxy->EnablePlayerNameVisible(true);
				else
					pPlayerProxy->EnablePlayerNameVisible(false);
	...
	case SLCLASS_PLAYER:	
		{
			...
			//if player is a gamemaster
			if (pPlayerAttr->GetIsGameMaster()) {
				pPlayerProxy->SetNameColor(pPlayerAttr->GetName(), playerNameColor,
					pPlayer->GetGuildName(), NTL_GAME_MASTER_NAME_COLOR, TRUE);
			}
			if (GetNtlStorageManager()->GetBoolData(dSTORAGE_GAMEINFO_OTHER_NAMEPLATE))
				pPlayerProxy->EnablePlayerNameVisible(TRUE);
			else
				pPlayerProxy->EnablePlayerNameVisible(FALSE);
			break;
	...
	case SLCLASS_MONSTER:
		{
			...
			if( pGroup->IsUseMonsterScouter(pMOB_TBLDAT->dwMobGroup))// ||	pMOB_TBLDAT->bShow_Name )
			{
				...
				if( pAvatar )
				{
					...
					if( pAvatarAttr->GetLevel() + 5 < pMonsterAttr->GetLevel() )
						monsterNameColor = NTL_MONSTER_NAME_COLOR_STRONG;
					else if( pAvatarAttr->GetLevel() - 5 > pMonsterAttr->GetLevel() )
...
//			pEffectData->effectCode == ACTIVE_SUPER_SAIYAN_BLUE   ||
...
RwUInt8 Logic_GetDogiItemColor(CNtlSobPlayer* pSobPlayer, RwBool bUICharacter /* = FALSE */)
{
	...
	if ( bUICharacter )
	{
		...
		pEquipItem = pUICharacter->pUIEquipProxy->GetEquipItem(EQUIP_SLOT_TYPE_DOGI); //Change this back to EQUIP_SLOT_TYPE_COSTUME to see where to add new items - Sangawku
	}
	else
	{
		pEquipItem = pCharProxy->GetEquipItem( EQUIP_SLOT_TYPE_DOGI );
	}
							  ...
							  RwBool bUICharacter /* = FALSE */, RwBool bForce /*= FALSE */)
{
	...
	else
	{
		...
		pEquipItem = pCharEquipProxy->GetEquipItem( EQUIP_SLOT_TYPE_DOGI ); //Change this back to EQUIP_SLOT_TYPE_COSTUME to see where to add new items - Sangawku
	}
