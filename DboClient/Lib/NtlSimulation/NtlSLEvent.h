...
extern RWS::CEventId g_EventActionMapScouterUse;           // 스카우터 사용
...
//////////////////////////////////////////////////////////////////////////
extern RWS::CEventId g_EventHoipoiMixItemMachineDel;
...
extern RWS::CEventId g_EventHoipoiMixJobSetRes;			///< 잡 셋팅 ( 호이포이 믹스 )
extern RWS::CEventId g_EventHoipoiMixJobResetRes;			///< 잡 리셋 ( 호이포이 믹스 )
...
extern RWS::CEventId g_EventPartyCharmActivity;
...
// WhoList Event
//_____________________________
extern RWS::CEventId g_EventWhoInfo;
extern RWS::CEventId g_EventWhoInfoChange;
extern RWS::CEventId g_EventWhoInfoNotify;           ///< UI를 업데이트 하기 위한 이벤트 (데이터->UI)
//-----------------------------
// OtherParam Event
//_____________________________
...
extern RWS::CEventId g_EventPrivateShopBusinessRequestRes;				
extern RWS::CEventId g_EventPrivateShopBusinessRequestNfy;				
extern RWS::CEventId g_EventPrivateShopBusinessConsentReq;				
extern RWS::CEventId g_EventPrivateShopBusinessConsentNfy;
extern RWS::CEventId g_EventPrivateShopBusinessWaittimeOutNfy;			
extern RWS::CEventId g_EventPrivateShopBusinessPricefluctuationsRes;	
extern RWS::CEventId g_EventPrivateShopBusinessPricefluctuationsNfy;
extern RWS::CEventId g_EventPrivateShopBusinessImmediateItemBuyingRes;	
extern RWS::CEventId g_EventPrivateShopBusinessImmediateItemBuyingNfy;
extern RWS::CEventId g_EventPrivateShopBusinessCancleRes;				
extern RWS::CEventId g_EventPrivateShopBusinessCancleNfy;
extern RWS::CEventId g_EventChatMessagePrivateShopBusiness;

//-----------------------------
// TMQ
//_____________________________

extern RWS::CEventId g_EventTMQBoard;
extern RWS::CEventId g_EventTMQBoardNotify;
...
// Portal System
//_____________________________
extern RWS::CEventId g_EventPortalStartRes;         ///< 포탈 NPC를 클릭했을때의 응답
...
extern RWS::CEventId g_EventTSOpenBattleStart;		// 전투
extern RWS::CEventId g_EventTSOpenBattleEnd;		// 전투
...
extern RWS::CEventId g_EventTrigObjScouterDir_CreateSTObj;			// 스카우터 트리거 오브젝트 생성
extern RWS::CEventId g_EventTrigObjScouterDir_DeleteSTObj;			// 스카우터 트리거 오브젝트 제거
...
//////////////////////////////////////////////////////////////////////////
//Changed Nameplates
extern RWS::CEventId g_EventMyNameplateChanged;
extern RWS::CEventId g_EventOtherNameplateChanged;
//////////////////////////////////////////////////////////////////////////
// Mob 관련 Event
...
struct SNtlEventSysMsg						// system message event data structer.
{
	...
	RwUInt32		uiMsg;					// shared result code id.	
	RwBool			bServerMsg;				
};
...
struct SNtlEventSobItemCreate : public SNtlEventSobBaseCreate
{
	...
	RwUInt32		uiOptionTblId1;
	RwUInt32		uiOptionTblId2;
	RwUInt8			byPlace;
	RwUInt8			bySlotIdx;
...
struct CNtlEventSobBuffCreate : public SNtlEventSobBaseCreate
{
	...
	RwUInt32		uiRemainTime;
	RwUInt32		uiKeepTime;
	RwReal			afEffectValue[NTL_MAX_EFFECT_IN_SKILL];
};
...
{
	RwUInt8			byBattleAttribute;
	RwUInt8			byStoneStack;
	RwUInt8			byItemGrade;
	RwUInt8			byItemPlace;
	RwUInt8			byItemPos;
	RwUInt8			byStonePlace;
	RwUInt8			byStonePos;
	RwBool			bItemDestroy;
};
...
enum ePartyMesssageType			///< 파티 메세지(SNtlEventPartyUpdate, SNtlEventPartyUpdateValue이 같은 것을 쓴다)
{
	...
	PMT_LEADER_CHARM_OPEN,		///< 파티장이 파티부적창을 열었다
	PMT_LEADER_CHARM_CLOSE,		///< 파티장이 파티부적창을 닫았다

	PMT_PARTY_DUNGEON_STATE,	///< 파티 던전 상태
	PMT_PARTY_DUNGEON_INITIALIZE,///< 파티 초기화
};

enum ePartyCharmActivityType
{
	PCAT_ACTIVE,				///< 파티 참 부적 발동
	PCAT_RELEASE,				///< 파티 참 부적 해제
};

struct sPartyCharmActivity
{
	RwUInt32			uiMessage;
	RwUInt8				byIndex;
...
enum eDojoEventType
{
	...
	DOJO_EVENT_BANK_HISTORY,						///< 유파 창고 도장 자금 사용내역
...
enum eTMQBoardType
{
	TBT_ROOM_LIST,					///< 방 목록 정보 갱신
	TBT_ROOM_INFO,					///< 방 하나의 상세정보

	TBT_ROOM_RESERVATION,			///< TMQ를 예약하였다	
	TBT_ROOM_RESERVATION_NFY,		///< 기존 일행 및 새로운 일행에게 룸정보를 알려준다
	TBT_ROOM_LEAVE,					///< TMQ 예약을 취소하였다	

	TBT_ROOM_SUCCESS_SELECTION,		///< TMQ 추첨이 성공하였다
	TBT_ROOM_ASK_RETRY,				///< 다음 TMQ에 재신청할지를 물어본다

	TBT_ROOM_TELEPORT,				///< TMQ에서 제공하는 1회용 텔레포트 기능을 사용하였다

	TBT_ROOM_LAUNCHED_TM,			///< 타임머신이 발사되었다

	TBT_ROOM_SERVER_EXCEPTION,		///< 뭔가의 예외상황으로 출발할 수 없다.
};

struct SNtlEventTMQBoard
{
...
struct SNtlEventTMQBoardNotify
{
	RwUInt32		uiEventType;
	SERIAL_HANDLE	hTargetHandle;
	RwUInt32		uiValue;
...
struct SNtlEventRBBattleTeamInfo
{
	...
	RwUInt8			byCount;
	void*			pMemberList;
	WCHAR			wcOwnerPartyName[NTL_MAX_SIZE_PARTY_NAME_IN_UNICODE + 1];
	WCHAR			wcChallengerPartyName[NTL_MAX_SIZE_PARTY_NAME_IN_UNICODE + 1];
};
...
struct SNtlEventShowQuestWindow_Nfy
{
	...
	sREWARD_INFO		sDefaultReward[4];		// 퀘스트 보상 정보 - 기본 보상
	sREWARD_INFO		sSelectReward[4];		// 퀘스트 보상 정보 - 선택 보상
...
struct SNtlEventTSOpenBattleStart
{
	HOBJECT handle;
};
struct SNtlEventTSOpenBattleEnd
{
	HOBJECT handle;
};
...
struct SNtlEventTrigObjScouterDir_CreateSTObj
{
	SERIAL_HANDLE	hSerial;
};

struct SNtlEventTrigObjScouterDir_DeleteSTObj
{
	SERIAL_HANDLE	hSerial;
};
...
* 요청 손님에게만 보내는 이벤트
*/
struct SNtlEventPrivateShopBusinessRequestRes
{
	RwUInt16	wResultCode;					///< 결과 코드
	RwUInt8		byPrivateShopInventorySlotPos;	///< 개인상점의 아이템 슬롯 위치
	RwUInt8		byItemState;					///< 아이템 상태
};

/**
* \brief 개인상점 흥정 요청 알림
* 요청 손님 외의 개인상점에 방문한 모든 손님에게 보내는 이벤트
*/
struct SNtlEventPrivateShopBusinessRequestNfy
{
	RwUInt8		byPrivateShopInventorySlotPos;	///< 개인상점의 아이템 슬롯 위치
	RwUInt8		byItemState;					///< 아이템 상태
};
...
/**
* \brief 개인상점 흥정 신청
* Game Server가 User(상점 주인)에게 보내는 Request 
*/
struct SNtlEventPrivateShopBusinessConsentReq
{
	SERIAL_HANDLE		hVisitorID;							///< 방문자의 ID(Serial Handle)
	RwUInt8				byPrivateShopInventorySlotPos;		///< 개인상점의 아이템 슬롯 위치
};

/**
* \brief 개인상점 흥정 수락 여부 알림
* 개인상점 주인이 흥정을 수락하게 되면 흥정손님에게 Notify를 보냄
*/
struct SNtlEventPrivateShopBusinessConsentNfy
{
	SERIAL_HANDLE		hOwner;								///< 상점 주인의 Serial_Handle
	RwUInt8 			byPrivateShopInventorySlotPos;		///< 개인상점의 아이템 슬롯 위치
};

/**
* \brief 개인상점 흥정 대기 타임아웃
* 개인상점을 수락 하기까지의 대기 시간이 초과하였을 경우 흥정 손님을 포함한
* 개인상점에 방문한 모든 손님에게 Notify
*/
struct SNtlEventPrivateShopBusinessWaittimeOutNfy
{
	SERIAL_HANDLE		hOwner;								///< 상점 주인의 Serial_Handle
	RwUInt8				byPrivateShopInventorySlotPos;		///< 개인상점의 아이템 슬롯 위치
};
...
/**
* \brief 개인상점 흥정 아이템의 가격 조정 결과
* 게임 서버는 가격이 변동된 아이템의 결과를 상점 주인에게 보내준다.(Result)
*/
struct SNtlEventPrivateShopBusinessPricefluctuationsRes
{
	RwUInt16			wResultCode;						///< 결과 코드
	RwUInt8				byPrivateShopInventorySlotPos;		///< 개인상점의 아이템 슬롯 위치
	RwUInt32 			dwBeforeZenny;						///< 이전의 아이템 가격
	RwUInt32			dwAfterZenny;						///< 변동된 아이템 가격
};

/**
* \brief 개인상점 흥정 가격 변동의 알림
* 게임 서버는 흥정하고 있는 손님에게 가격이 변동되었다는 것을 보내준다.(Notify)
*/
struct SNtlEventPrivateShopBusinessPricefluctuationsNfy
{
	SERIAL_HANDLE		hOwner;								///< 상점 주인의 Serial_handle
	RwUInt8				byPrivateShopInventorySlotPos;		///< 개인상점의 아이템 슬롯 위치
	RwUInt32			dwBeforeZenny;						///< 이전의 아이템 가격
...
/**
* \brief 개인상점 흥정 즉시 구매의 결과
* 게임 서버는 손님에게 즉시 구매의 결과를 알려준다. (Result)
*/
struct SNtlEventPrivateShopBusinessImmediateItemBuyingRes
{
	RwUInt16					wResultCode;				///< 결과 코드
	sPRIVATESHOP_ITEM_DATA		sPrivateShopItemData;		///< 개인상점 아이템 데이타 구조체
};

/**
* \brief 개인상점 흥정 즉시 구매의 알림
* 게임 서버는 상점 주인, 흥정 손님을 뺀 손님들에게 즉시 구매의 결과를 알려준다. (Notify)
*/
struct SNtlEventPrivateShopBusinessImmediateItemBuyingNfy
{
	SERIAL_HANDLE				hPrivateShopOwner;			///< 상점 주인의 Serial_Handle
	sPRIVATESHOP_ITEM_DATA		sPrivateShopItemData;		///< 개인상점 아이템 데이타 구조체
};

/**
* \brief 개인상점 흥정 취소의 결과
* 취소 요청자(개인상점 주인, 흥정 요청 손님)에게 Result
*/
struct SNtlEventPrivateShopBusinessCancleRes
{
	RwUInt16			wResultCode;						///< 결과 코드
	RwUInt8				byCancelType;						///< 취소 타입( 1: 흥정대기때 취소, 2: 흥정때 취소)
	RwUInt8 			byPrivateShopInventorySlotPos;		///< 개인상점 아이템 슬롯의 위치
};

/**
* \brief 개인상점 흥정 취소의 알림
* 취소요청자에 따라서 그 외의 대상자( 흥정손님 또는 주인 )
*/
struct SNtlEventPrivateShopBusinessCancleNfy
{
	SERIAL_HANDLE		hOwner;							///< 취소한 사람의 Serial_Handle
	RwUInt8				byCancelType;					///< 1: 흥정대기때 취소, 2: 흥정때 취소
	RwUInt8				byPrivateShopInventorySlotPos;	///< 개인상점 아이템 슬롯의 위치
};

/**
* \brief 개인상점 흥정 채팅
* 개인상점 흥정 시스템에서 Community Server에서 User에게 보내는 채팅 메시지
*/
struct SNtlEventChatMessagePrivateShopBusiness
{
	WCHAR*			pwchSenderCharName;
	RwUInt16		wMessageLengthInUnicode;
	WCHAR			awcMessage[NTL_MAX_LENGTH_OF_CHAT_MESSAGE_UNICODE+1];
};
...
// Portal System
struct SNtlEventPortalStartRes
{
	WORD           wResultCode;
...
struct SNtlEventFriendAddRes
{
	...
	WCHAR           wchName[NTL_MAX_SIZE_CHAR_NAME_UNICODE + 1];    ///< 캐릭터 네임
...
{
	RwBool        bIsRequester;          ///< 1: 도움을 요청한 몹, 2: 도움을 요청받은 몹
};

#define FPS_INTERPOLATION_TIME  0.2f                   ///< 일반 카메라->FPS 카메라 보간 타임
struct SNtlEventCameraFPS
{
	RwV3d   vCurrLookAt;                    ///< 현재 바라보고 있는 방향
	RwV3d   vTargetLookAt;                  ///< 1인칭 카메라로 변경되었을때 바라볼 방향
};

enum eScouterEventType
{
	SCOUTER_EVENT_SET_SCOUTER,				///< 스카우터 바디를 설정한다
	SCOUTER_EVENT_UNSET_SCOUTER,			///< 스카우터 바디를 해제한다

	SCOUTER_EVENT_CURRENT_POWER,			///< 현재의 전력사용량을 알린다

	SCOUTER_EVENT_MEASURE_POWER_START,		///< 전투력 측정을 시작한다
	SCOUTER_EVENT_MEASURE_MEASURE_RESULT,	///< 전투력 측정값을 받아왔다.
	SCOUTER_EVENT_MEASURE_ERROR,			///< 전투력 측정 에러

	SCOUTER_EVENT_ENTER,		            ///< 스카우터 상태로 들어갈때 (사용)
	SCOUTER_EVENT_OUT_OF_ORDER,				///< 스카우터 고장
	SCOUTER_EVENT_EXIT_AFTER_OUT_OF_ORDER,	///< 스카우터 고장 GUI 연출후 종료
	SCOUTER_EVENT_EXIT,					    ///< 스카우터 상태에서 나올때 (사용 끝)    

	SCOUTER_EVENT_PC_INFO_VIEW,				///< PC 정보 보기
	SCOUTER_EVENT_FIND_OBJECT,				///< 숨겨진 오브젝트 찾기
	SCOUTER_EVENT_QUEST_SEARCH,			    ///< 주변 퀘스트 찾기

	// 서버로 받은 다른 캐릭터 정보
	SCOUTER_EVENT_NFY_ENTER,                
	SCOUTER_EVENT_NFY_EXIT,
	SCOUTER_EVENT_NFY_BROKEN,


    // FSM단에서 UI로 보내는 이벤트
    SCOUTER_EVENT_FSM_ENTER,
    SCOUTER_EVENT_FSM_EXIT,
};

struct SDboEventScouter
{
	RwInt32			iType;			///< Scouter에 전달되는 데이터 타입
...
struct SNtlEventHoipoiMixJobSetRes
{
	SERIAL_HANDLE	hNpcHandle;
	RwUInt8			byRecipeType;
};

struct SNtlEventHoipoiMixJobResetRes
{
	SERIAL_HANDLE	hNpcHandle;
	RwUInt8			byRecipeType;
};
...
// Nameplate Gui Mode
//////////////////////////////////////////////////////////////////////////
struct SNtlEventMyNameplateMode : public SNtlEventSob
{
};

struct SNtlEventOtherNameplateMode : public SNtlEventSob
{
};
//////////////////////////////////////////////////////////////////////////
// Mob 관련 Event
