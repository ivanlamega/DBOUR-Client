...
void CNtlAvatarSkillContainer::CreateEventHandler(RWS::CMsg &pMsg)
{
	...
	for(RwInt32 i = 0; i < pAvatarInfo->bySkillCount; i++)
	{
		pSobSkill = reinterpret_cast<CNtlSobSkill*>( CNtlSLEventGenerator::SobSkillCreate( pSobAvatar->GetSerialID(),
																						...
																						pAvatarInfo->sSkillPf[i].bySlotId,
		...
		SetSkillSerial(pAvatarInfo->sSkillPf[i].bySlotId, pSobSkill->GetSerialID());
...
void CNtlAvatarSkillContainer::SkillInitHandler( RWS::CMsg& msg )
{
	...
	// 스킬 정보 초기화. NeedSp 가 0인 녀석은 초기화 안함.
	for( i = 0 ; i < NTL_MAX_PC_HAVE_SKILL ; ++i )
	{
		if( m_hSerial[i] == INVALID_SERIAL_ID )
			continue;
...
{
	CSkillTable* pSkillTbl = nullptr;
	pSkillTbl = API_GetTableContainer()->GetSkillTable();
	CSystemEffectTable* pEffectTbl = nullptr;	
	pEffectTbl = API_GetTableContainer()->GetSystemEffectTable();

	sSKILL_TBLDAT* pSkillTblData = nullptr;
		
	pSkillTblData = reinterpret_cast<sSKILL_TBLDAT*>( pSkillTbl->FindData(uiTblId) );
	...
	for(RwInt32 i = 0; i < NTL_MAX_EFFECT_IN_SKILL; ++i)
	{
		...
		sSYSTEM_EFFECT_TBLDAT* pEffectTblData = nullptr;
		pEffectTblData = reinterpret_cast<sSYSTEM_EFFECT_TBLDAT*>( pEffectTbl->FindData(pSkillTblData->skill_Effect[i]) );
		if (pEffectTbl == nullptr) //Make sure table exists. -sangawku 
		{
			NTL_ASSERTFAIL("pEffectTbl is NULL");
			return;
		}
		if (pEffectTblData == nullptr) //Make sure table exists. -sangawku 
		{
			NTL_ASSERTFAIL("pEffectTblData is NULL");
			return;
		}
		if (pEffectTblData->effectCode == PASSIVE_DASH)
		{
			m_wDashReqLP = pSkillTblData->wRequire_LP;
			m_wDashReqEP = pSkillTblData->wRequire_EP;
			m_wDashReqRP = pSkillTblData->byRequire_RP_Ball;
	...
	if(pMsg.Id == g_EventSobCreate)
	{
		...
		for(RwInt32 i = 0; i< pSummonPetSkillInfo->bySkillCount; ++i )
		{
			pSobSkill = reinterpret_cast<CNtlSobSkill*>( CNtlSLEventGenerator::SobSkillCreate(
				...
				pSummonPetSkillInfo->aSkillInfo[i].bySlotIndex,	// Slot Index
				DBO_RP_BONUS_TYPE_INVALID, 
				FALSE, 0) );

			SetSkillSerial(pSummonPetSkillInfo->aSkillInfo[i].bySlotIndex, pSobSkill->GetSerialID());
			...
			for(RwInt32 j = 0; j < NTL_MAX_NPC_HAVE_SKILL; ++j )
			{
				...
				{
					// AI 세팅
					SetSkillUseBasis( pSummonPetSkillInfo->aSkillInfo[i].bySlotIndex, pNpcTblData->byUse_Skill_Basis[j] );
					SetSkillUseLp( pSummonPetSkillInfo->aSkillInfo[i].bySlotIndex, pNpcTblData->wUse_Skill_LP[j] );
					SetSkillUseTime( pSummonPetSkillInfo->aSkillInfo[i].bySlotIndex, pNpcTblData->wUse_Skill_Time[j] );
				}
			}
		}
	}
