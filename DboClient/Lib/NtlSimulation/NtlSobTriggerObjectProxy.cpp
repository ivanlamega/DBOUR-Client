...
void CNtlSobTriggerObjectProxy::InitState(RwUInt8 byMainState, RwUInt8 bySubState)
{
	...
	else
	{
		if ( m_byMainState != byMainState )
		{
			...
			{
				NTL_ASSERTFAIL( "Trigger object animation chain empty." << m_pPLObject->GetName() << m_byMainState << "\n" );
	...
	// Scouter object인 경우 Visible에 대한 컨트롤은 CS 연동 상태와는 무관하게 안보이는 상태로
	// 시작하게 된다
	if ( IsScouterObject() )
	{
		EnableVisible( FALSE );
	}
	// TLQ object인 경우 Visible에 대한 컨트롤은 CS 연동 상태와는 무관하게 현재 아바타의
	// 퀘스트 진행 여부에 의해 visible이 결정된다
	else if ( IsTLQObject() )
	{
...
void CNtlSobTriggerObjectProxy::PlayAnimation(RwReal fPlayTime)
{
	if(!m_AnimChain.empty() && m_pPLObject)
	{
		...
		// Animation chain에서 마지막 Animation만 LOOP로 동작시킨다
		if ( !m_pPLObject->SetTriggerAnimation( iAniID, fPlayTime, m_AnimChain.empty() ? TRUE : FALSE ) )
		{
			std::cout << "Trigger object animation play failed" << m_pPLObject->GetName() << iAniID << fPlayTime << "\n";
		}
	}
}
