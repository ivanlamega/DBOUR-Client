...
{          
    ...
    {
		m_wndStyle = WS_POPUP | WS_THICKFRAME | WS_CAPTION | WS_SYSMENU | WS_MAXIMIZEBOX | WS_MINIMIZEBOX;
...
void CNtlApplication::ChangeWindowStyle(RwBool bFullScreen)
{
	...
	if(m_bFullScreen)
	{
		m_wndStyle = WS_POPUP | WS_MINIMIZEBOX | WS_SYSMENU | WS_VISIBLE;
		m_wndFlags = SWP_SHOWWINDOW;
		m_hWndInsertAfter = HWND_TOPMOST;
	}
	else
	{
		m_wndStyle = WS_OVERLAPPEDWINDOW | WS_MINIMIZEBOX | WS_SYSMENU | WS_VISIBLE;
		m_wndFlags = SWP_SHOWWINDOW;
	...
	SetWindowPos(m_hWnd, m_hWndInsertAfter, 0, 0, rect.right - rect.left + 1, rect.bottom - rect.top, m_wndFlags);
	...
	{
		Resize( rectAfter.right - rectAfter.left + 1 , rectAfter.bottom - rectAfter.top, TRUE, FALSE );
	}
	...
	if( !m_bFullScreen )
	{
		...
		RwInt32 nScreenWidth = GetSystemMetrics( SM_CXSCREEN ) +1;
...
std::vector<std::string> split_string(std::string_view content, std::string_view delimeter)
{
	...
	{
		result.emplace_back(std::move(prev_pos), content.end());
	}
	return result;
}
...
RwTexture * DDSTextureRead(const RwChar * szName, const RwChar * szMaskName)
{
	...
	std::string file;
	file = std::string(szName);
	std::transform(file.begin(), file.end(), file.begin(), ::tolower);
	std::string lowerFile = file;

	std::string strPath(RwImageGetPath());

	bool hasInserted = false;


	//Split path into multiple paths.
	auto tokens = split_string(strPath, ";");

	
	std::string fullDDS = strPath + file;
	std::string fullPng = strPath + file;
	std::string filedds = file;
	std::string filepng = file;
	filedds.insert(filedds.size(), ".dds");
	filepng.insert(filepng.size(), ".png");


	fullDDS.insert(fullDDS.size(), ".dds");
	fullPng.insert(fullPng.size(), ".png");
	
	/*if (lowerFile.compare("sun_001") == 0)
	{
		bool hitCheckpoint = true;
		if (hitCheckpoint)
		{
			NtlLogFilePrint((RwChar*)lowerFile.c_str());
		}
	}*/
	for (auto& token : tokens)
	{
		std::transform(token.begin(), token.end(), token.begin(), ::tolower);
		std::size_t foundTok = token.find(".\\");
		std::size_t foundTok2 = token.find("\\");
		std::size_t foundTok3 = token.find(";");
		std::size_t foundTok4 = token.find("tex");
		if (foundTok != std::string::npos)
		{
			while (foundTok != std::string::npos)
			{
				token.insert(foundTok + 2, "mods\\");
				foundTok = token.find_first_of(";", foundTok + 6);
			}
		}
		else if (foundTok2 == 0 || foundTok4 == 0)
		{
			if (foundTok4 == 0 && foundTok2 == 0)
			{
				token.insert(2, "mods\\");
				foundTok3 = token.find(";");
			}
			else if(foundTok4 == 0 && foundTok2 != 0)
			{
				token.insert(0, "mods\\");
				foundTok3 = token.find(";");
			}
			...
			while (foundTok3 != std::string::npos)
			{
				token.insert(foundTok3 + 1, "mods\\");
				foundTok3 = token.find_first_of(";", foundTok3 + 6);
			}
		}

		std::string newFile = token + filedds;
		if (std::experimental::filesystem::exists(newFile))
		{
			std::string modsLoaded = "Mod Loaded: " + newFile;
			NtlLogFilePrint((RwChar*)modsLoaded.c_str());
			printf(modsLoaded.c_str());
			RwImageSetPath(token.c_str());
		...
		std::string newPng = token + filepng;
		if (std::experimental::filesystem::exists(newPng))
		{
			std::string modsLoaded = "Mod Loaded: " + newPng;
			NtlLogFilePrint((RwChar*)modsLoaded.c_str());
			printf(modsLoaded.c_str());
			RwImageSetPath(token.c_str());
			pTexture = RwD3D9DDSTextureRead(szName, szMaskName);
	...
	//png중에 확장자가 붙지 않고 들어오는 경우도 있기 때문에 다시 한번
	//처리를 한다.(예: skydummy) by HoDong (2006. 8. 7)
	if(!pTexture)
	{
		pTexture = RwD3D9DDSTextureRead(szName, szMaskName);
		if(pTexture == NULL)
