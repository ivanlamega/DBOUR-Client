...
FMOD_RESULT API_Create_Stream(CNtlSound* pSound, sNtlSoundPlayParameta* pParameta, FMOD_MODE mode, std::string& strFullName)
{
	...
	// File에 직접 접근
	std::string modFullName = "mods\\sound\\" + pSound->m_strName;
	std::transform(modFullName.begin(), modFullName.end(), modFullName.begin(), ::tolower);
	if (std::experimental::filesystem::exists(modFullName))
	{
		return CNtlSoundGlobal::m_pFMODSystem->createStream(modFullName.c_str(), mode, 0, &(pSound->m_pFMODSound));
	}
