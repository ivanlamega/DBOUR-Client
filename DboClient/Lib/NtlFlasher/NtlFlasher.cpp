...
GFile* FlasherFileOpener::OpenFile( const char* pFilename )
{
...
{
	if( pMovie && m_pFSCallBack )
	{
		m_pFSCallBack->Call( (VOID*)command, (VOID*)args );
	}
}
...
BOOL CNtlFlasher::Create( CONST CHAR* szFileName )
{
	GFxLoader loader;
	
	loader.SetFileOpener( m_pFileOpener );

	loader.SetFSCommandHandler( m_pCommandHandler );
	...
	strGfxtFileName = strPath + "\\" + strName + ".gfx";
	
	if( !( m_pMovieDef = *loader.CreateMovie( strGfxtFileName.c_str(), GFxLoader::LoadWaitCompletion ) ) )
	{
		if( !( m_pMovieDef = *loader.CreateMovie( strSwfFileName.c_str(), GFxLoader::LoadWaitCompletion ) ) )
		{
			Destroy();

			// lleo52 성복아 수정 바람
//			NTL_ASSERTFAIL( szFileName << "Flash Load Fail" );

			return FALSE;
		}		
	}
	
	if( !( m_pMovie = *m_pMovieDef->CreateInstance() ) )
	{
		Destroy();
...
VOID CNtlFlasher::Destroy(VOID)
{
	if( m_pCommandHandler )
		m_pCommandHandler->UnLinkFSCallBack();
...
FLOAT CNtlFlasher::GetFrameRate()
{
	if( m_pMovie )
