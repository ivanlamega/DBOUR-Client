	...
	if(GetNtlResourceManager()->IsLoadScheduling())
	{
		...
		for(RwUInt32 i = 0; i < m_pSchedulingLMProp->_vecSchedulingLMAtomProp.size(); ++i)
		{
			...
			if(pCurAtomic)
			{
				...
				strcat_s(szFullPathName, 256, dGET_WORLD_PARAM()->WorldBSP);
...
int CNtlPLObject::CallBackBaseAnim( RwUInt32 uiEventData ) 
{
	...
	case EVENT_ANIM_ALPHA:
		...
		break;
	case EVENT_ANIM_TMQ:
		OnEventTMQ((SEventTMQ*)pEventAnim);
		break;
	case EVENT_ANIM_EXPLOSION:
		OnEventExplosion((SEventExplosion*)pEventAnim);
		break;
	}
...
void CNtlPLObject::OnEventTMQ( SEventTMQ* pEventTMQ ) 
{
	RwBool bIn;
	...
	if (m_uiRenderFrame != uiRenderFrame)
	{
		if (!CNtlPLEntity::CullingTest(pRwCamera, uiRenderFrame))
		{
			...
			{	
				...
				{							
					fVisibleDistance = 1512.0f; //RwCameraGetFarClipPlane(pRwCamera);
				}
#endif				
...
RwBool CNtlPLObject::LoadFromFile(FILE* pFile, EActiveWorldType eActiveWorldType)
{
...
#ifdef TW_VER
	unknownShit uUnknownShit;
	fread(&uUnknownShit.fVisibilityDistance, sizeof(RwReal), 1, pFile);
	SetUnknownUnion(uUnknownShit);
	...
	if (uUnknownShit.dwUnknownShit == 0xFFFFFFFF) {
		...
		SetVisibleCullingDistance(uUnknownShit.fVisibilityDistance);
	}
#else
	// visibility distance
	RwReal fVisibilityDistance;
	...
	if (uUnknownShit.dwUnknownShit == 4294967295) {
		fread(&dwUnknownShit2, sizeof(RwInt32), 1, pFile);
		SetUnknownInt32(dwUnknownShit2);
	}
	else if (dwNameIdx != 0xFFFFFFFF) {
		fread(&dwUnknownShit2, sizeof(RwInt32), 1, pFile);
		SetUnknownInt32(dwUnknownShit2);
	...
	if (dwUnknownShit2 != 0) {
		LoadUnknownStructFromFile(pFile);
	}
#endif
...
#ifdef TW_VER
	unknownShit uUnknownShit = GetUnknownUnion();
	//fwrite(&uUnknownShit.fVisibilityDistance, sizeof(RwReal), 1, pFile);

	if (uUnknownShit.dwUnknownShit == 0xFFFFFFFF) {
		fwrite(&uUnknownShit.dwUnknownShit, sizeof(uUnknownShit.dwUnknownShit), 1, pFile);
	}
	else if (dwNameIdx != 0xFFFFFFFF) {
		fwrite(&uUnknownShit.dwUnknownShit, sizeof(uUnknownShit.dwUnknownShit), 1, pFile);
	}
...
#ifdef TW_VER
	RwInt32 dwUnknownShit2 = GetUnknownInt32();
	if (uUnknownShit.dwUnknownShit == 4294967295) {
		fwrite(&dwUnknownShit2, sizeof(RwInt32), 1, pFile);
	}
	else if (dwNameIdx != 0xFFFFFFFF) {
...
RwInt32	CNtlPLObject::SkipToFile(FILE* pFile, EActiveWorldType eActiveWorldType, RwUInt32 uiObjectType)
{
	...
	::fseek(pFile, (sizeof(RwReal)), SEEK_CUR); // UnknownShit
