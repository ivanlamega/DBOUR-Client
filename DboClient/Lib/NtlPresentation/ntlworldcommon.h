...
// 클라이언트를 사용하고자 하실때는 반드시 주석처리하시고 리빌드 하세여 - 우디
...
#define dNTL_WORLD_VERSION_OLD					("1.0") //1.0
#ifdef TW_VER
#define dNTL_WORLD_VERSION						("1.4")
#else
#define dNTL_WORLD_VERSION						("1.1")
#endif
...
#ifdef TW_VER
		dwUnknown		= Prop.dwUnknown;
#endif

		_RGBSkyColor[0] = Prop._RGBSkyColor[0];
		_RGBSkyColor[1] = Prop._RGBSkyColor[1];
		_RGBSkyColor[2] = Prop._RGBSkyColor[2];
		::strcpy_s(_BaseSkyTexName, 10, Prop._BaseSkyTexName);
		_BaseSkySpeed	= Prop._BaseSkySpeed;
		::strcpy_s(_BlendedTexName[0], 10, Prop._BlendedTexName[0]);
		::strcpy_s(_BlendedTexName[1], 10, Prop._BlendedTexName[1]);
