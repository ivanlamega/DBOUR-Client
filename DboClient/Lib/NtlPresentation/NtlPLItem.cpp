...
RwBool	CNtlPLItem::SetThreadSafeProperty(const CNtlPLProperty *pData)
{
	...
	//현재 Visual Manager에 등록이 될 때 Update 등록이 동적으로 등록이
	//안된다. 여기에서 등록을 해야 제대로 된다.
...
{
	// Body인 경우는 Clone Atomic List를 보관을 하고 있는다.
...
void CNtlPLItem::SetApplyEquipData()
{
	...
	if( m_sScheduleResInfo.bEquip )
	{
		if( m_sScheduleResInfo.bAttach )
		{
			...
			//Body 인 경우 Owner의 Clump 에서 World에 Add 됨
			if( g_CheckItemBody(GetResType()) )
			{
...
void CNtlPLItem::SetApplyEquipDataOwner(RpAtomic *pAtomic, CNtlPLCharacter *pOwner)
{
	...
	////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	// void	_rpToonAtomicChainAtomicRenderCallback(RpAtomic *atomic) 이쪽에서 CB이 꼬이게 된다.
	// 복수 atom을 드는 dff에서 각각의 atom이 서로 다르게 들어올때가 있다.
	// 예를 들어 defaultrender or ToonAtomRender등이다. 그런데 m_fnDefRenderCallBack이 변수를
	// 쓰는 바람에 현재 PLItem에서 atom이 3개일때 첫번째의 CB가 m_fnDefRenderCallBack에 저장이 되어 버리고
	// 다음에 오는 애들은 이 m_fnDefRenderCallBack이 강제로 담아져버리기 때문에 _rpToonAtomicChainAtomicRenderCallback
	// 여기에서 cb이 꼬이게 되버린다. item중 의자에 않은 나메크장로, 의자에 않은 아이, 의자에 올린 알등 의자와 의자위에
	// 풀(블렌딩)이 이에 해당한다. dff의 복수 atom이 memory에 적재됬을때 그 순서는 항상 일정치 않다고 RW에서
	// 본것 같다. 또한 현재 의자 mesh는 무언가 export가 잘못되었다. 안에 기본 cb들의 정보가 전부 NULL로 되어있다.
	// 문제가 될것 같다. 성봉씨가 꼭 도움을 주어야 하는데. 걱정이 된다.
	//  dead call로 인하여... 반복 호출을 막는다.
	////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
RwBool CNtlPLItem::SetUpgradeEffect(ENtlPLItemGrade eGrade)
{
    ...
    // 두번째 Bone이 설정되어 있으면 Effect를 추가한다.
