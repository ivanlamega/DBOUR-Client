...
RwBool CNtlPLPlayerName::Create( const SPLEntityCreateParam * pParam )
{
	...
	// 카메라 컬링 거리
	m_fCullDist = PLAYERNAME_DIST; //Test for player name distance - Sangawku //pPlayerNameParam->fCullDist;
	...
	else if( (RwInt32)wcslen( pPlayerNameParam->pGuildName ) > 0 )
	{
		...
		if (CreateGMNameTexture())
			m_byFlag |= dNTL_PL_FLAG_GAMEMASTER;
...
RwBool CNtlPLPlayerName::CreateGMNameTexture(const WCHAR* pName)
{
	// 길드 이름의 폰트를 만든다.
	m_pGamemasterFont = g_FontMgr.CreateGuiFont(DEFAULT_FONT, dNTL_PLAYERNAME_FONTSIZE_GUILD, DEFAULT_FONT_ATTR);
	if (m_pGamemasterFont == NULL)
	{
		NTL_RETURN(FALSE);
	}
	...
	// 글자의 사이즈를 저장하고 사이즈에 알맞은 텍스쳐를 만든다.
	CPos size = m_pGamemasterFont->GetTextWSize(pName, (int)wcslen(pName));
	m_posGamemasterNameSize.SetPos(size.x, size.y);

	RwInt32 nMaxWidth = m_posNameSize.x > m_posGamemasterNameSize.x ? m_posNameSize.x : m_posGamemasterNameSize.x;

	m_pTexture = g_TextureMgr.CreateTexture(nMaxWidth + dPLAYER_NAME_CREATE_TEXTURE_OFFSET, m_posNameSize.y + m_posGamemasterNameSize.y + PLAYERNAME_LINE_INTERVAL_Y);
	if (m_pTexture == NULL)
		NTL_RETURN(FALSE);
...
void CNtlPLPlayerName::SetNameColor( const WCHAR* pwcName, COLORREF color, const WCHAR* pwcGuildName, COLORREF guildcolor
									,sEmblemFactor* pEmblemFactor, BOOL isGamemaster)
{
	if( m_pTexture )
		m_pTexture->FillClear();
	...
	if( pwcGuildName == NULL )
	{
			if (CreateNameTexture(pwcName))
				m_byFlag = dNTL_PL_FLAG_NAME;
			if (isGamemaster)
				m_byFlag = dNTL_PL_FLAG_NAME | dNTL_PL_FLAG_GAMEMASTER;
	}
	else if( (RwInt32)wcslen( pwcGuildName ) > 0 )
	{
		...
		if( CreateGuildNameTexture( pwcGuildName ) )
		{
			...
			if (isGamemaster)
				m_byFlag = dNTL_PL_FLAG_NAME | dNTL_PL_FLAG_GUILD | dNTL_PL_FLAG_GAMEMASTER;

			if (CreateGuildEmblemTexture(pEmblemFactor))
			{
				if(isGamemaster)
					m_byFlag = dNTL_PL_FLAG_NAME | dNTL_PL_FLAG_GUILD | dNTL_PL_FLAG_EMBLEM | dNTL_PL_FLAG_GAMEMASTER;
				else
	...
	if( m_pTexture )
	{
		...
		else if (m_byFlag & dNTL_PL_FLAG_GAMEMASTER)
		{
			CRectangle rect;
			CPos size = m_pGamemasterFont->GetTextWSize(L"[GM]", (int)wcslen(L"[GM]"));
			...
			m_pGamemasterFont->TextOutW(L"                                ");
			m_pGamemasterFont->TextOutW(L"[GM]");

			// texture에 string copy
			g_FontMgr.BitmapToTexture(m_pTexture, rect, guildcolor, TE_SHADOW, 0, 2);

			CPos size2 = m_pFont->GetTextWSize(pwcName, (int)wcslen(pwcName));
			m_pFont->TextOutW(L"                ");
			m_pFont->TextOutW(pwcName);
			rect.left = (m_pTexture->GetWidth() - size2.x) / 2;
			rect.top = size.y + PLAYERNAME_LINE_INTERVAL_Y;
