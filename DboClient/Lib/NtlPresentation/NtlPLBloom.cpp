...
CNtlPLBloom::CNtlPLBloom(void)
{
	...
	m_fTotalMonoPower	= 0.1f;
	m_MonoPower[0]		= 0.1f;
	m_MonoPower[1]		= 0.1f;
	m_MonoPower[2]		= 0.1f;
	m_RestTime4Change	= -400.0f;
...
void CNtlPLBloom::HandleEvents(RWS::CMsg &pMsg)
{
	if(pMsg.Id == NPEI_IS_ANOTHER_FIELD_CHANGED || pMsg.Id == NPEI_IS_ANOTHER_BLOCK_CHANGED)
	{
		...
		RwInt32 SrcCmp = static_cast<RwInt32>(m_MonoPower[0] * 1.0f);
		RwInt32 DstCmp = static_cast<RwInt32>(pNtlFieldProp->_MonoPower * 1.0f);
