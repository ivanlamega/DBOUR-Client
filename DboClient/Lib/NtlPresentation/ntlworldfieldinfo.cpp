...
#ifdef TW_VER
	// For TW here is 1x DWORD ~Nady
	fread(&pNtlFieldProp->dwUnknown, sizeof(RwInt32), 1, pFile);
#endif
...
#ifdef TW_VER
	// For TW here is 1x DWORD ~Nady
	fwrite(&pNtlFieldProp->dwUnknown, sizeof(RwInt32), 1, pFile);
#endif
...
#ifdef TW_VER
	SizMemBlock += sizeof(RwInt32); // dwUnknown
#endif
...
RwInt32 CNtlWorldFieldInfo::FieldPlanetSkipToFile(FILE* pFile)
{
	for(RwInt32 i = 0; i < dNTLPL_PLANET_MAX_CNT; ++i)
	{
		fseek(pFile, sizeof(sNTL_PLANET), SEEK_CUR);
	}
