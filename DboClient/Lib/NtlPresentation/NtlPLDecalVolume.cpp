...
#ifdef MODSENABLED
		m_pTexture = CNtlPLResourceManager::GetInstance()->LoadTexture(m_acTextureName, "\\Texture\\ntlwe\\decal\\");
#else
		m_pTexture = CNtlPLResourceManager::GetInstance()->LoadTexture(m_acTextureName, ".\\Texture\\ntlwe\\decal\\");
#endif
...
RpCollisionTriangle* NtlRwIm3DVertexGetDecalBsp(RpIntersection *pIntersection, RpWorldSector* pRpWorldSector, RpCollisionTriangle *pRpCollisionTriangle, RwReal fRatio, void* pData)
{
	...
	RwV3d v0, v1;
	RwV3dSub(&v0, pRpCollisionTriangle->vertices[1], pRpCollisionTriangle->vertices[0]);
	RwV3dSub(&v1, pRpCollisionTriangle->vertices[2], pRpCollisionTriangle->vertices[0]);
...
RwBool CNtlPLDecalVolume::Pick(RwReal& fDist)
{
	...
	RwV3dSub(&vDirRay, &rayCamera.end, &rayCamera.start);
	RwV3dSub(&vRayDeltha, &dGET_COLLISION_INFO()->RayIntersectionPt4Terrain, &rayCamera.start);
