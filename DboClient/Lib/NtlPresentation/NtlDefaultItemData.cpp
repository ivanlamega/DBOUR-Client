...
std::string * CNtlDefaultItemTable::GetSuperSaiyanBlueHeadKeyName(EClassGenderType eClassGenderType, RwUInt32 uiHeadType)
{
	RwUInt32 uiKey = CREATE_DEF_ITEM_HEAD(eClassGenderType, uiHeadType);

	if (m_mapSuperSaiyanBlueHeadKeyName.find(uiKey) == m_mapSuperSaiyanBlueHeadKeyName.end())
		return NULL;

	return &m_mapSuperSaiyanBlueHeadKeyName[uiKey];
}

RwBool CNtlDefaultItemTable::AddSuperSaiyanBlueHeadKeyName(EClassGenderType eClassGenderType, RwUInt32 uiHeadType, const std::string &strHeadKeyName)
{
	RwUInt32 uiKey = CREATE_DEF_ITEM_HEAD(eClassGenderType, uiHeadType);
	...
	m_mapSuperSaiyanBlueHeadKeyName[uiKey] = strHeadKeyName;
...
void CNtlDefaultItemTable::Destroy()
{
	...
	m_mapSuperSaiyanBlueHeadKeyName.clear();
			...
			if(pCharTypeNodeList)
			{
				...
				for(long m = 0; m < charTypeCount; ++m)
				{
					...
					if(pCharTypeNode)
					{
						...
						pCharTypeNode->selectNodes(L"HEAD_MESH_SUPER_SAIYAN_BLUE", &pType_NodeList);
						if (pType_NodeList)
						{
							pType_NodeList->get_length(&typeCount);
							for (long n = 0; n < typeCount; ++n)
							{
								...
								if (pData_NodeList)
								{
									...
									for (long t = 0; t < dataCount; t++)
									{
										...
										AddSuperSaiyanBlueHeadKeyName(eClassGenderType, (RwUInt32)atoi(szType), szKeyName);
									}
								}
							}
						}
