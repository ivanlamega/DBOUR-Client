...
void CNtlPLEquipItem::AddCItem(CNtlPLItem* _pItem, RwBool _bAttach, RwUInt32 _CBitSet)
{
	...
	if (_pItem->GetEquipSlotType() == EQUIP_SLOT_TYPE_QUEST || _pItem->GetEquipSlotType() == EQUIP_SLOT_TYPE_DOGI || _pItem->GetEquipSlotType() == EQUIP_SLOT_TYPE_HAIR
		|| _pItem->GetEquipSlotType() == EQUIP_SLOT_TYPE_ACCESSORY_1 || _pItem->GetEquipSlotType() == EQUIP_SLOT_TYPE_ACCESSORY_2 || _pItem->GetEquipSlotType() == EQUIP_SLOT_TYPE_ACCESSORY_3)
		bCostume = true;
	
	if(_CBitSet & eCBSTS_JACKET && !bCostume)
		RemoveItem(ITEM_EQUIP_JACKET);
	if(_CBitSet & eCBSTS_PANTS && !bCostume)
		RemoveItem(ITEM_EQUIP_PANTS);
	if(_CBitSet & eCBSTS_BOOTS && !bCostume)
		RemoveItem(ITEM_EQUIP_BOOTS);
	if(_CBitSet & eCBSTS_TWO_HAND)	
		RemoveItem(ITEM_EQUIP_TWO_HAND);
	if(_CBitSet & eCBSTS_HEAD && !bCostume)
		RemoveItem(ITEM_EQUIP_HEAD);
	if(_CBitSet & eCBSTS_FACE && !bCostume)
...
void CNtlPLEquipItem::RemoveItem(CNtlPLItem *pItem)
{
	...
	//Body Item 이면 Default Item로 교체 한다.
...
RwBool CNtlPLEquipItem::SetChangeDefFaceItem(EClassGenderType eClassGenderType, RwUInt32 uiFaceType, RwUInt32 uiColorType)
{
	...
	if( eClassGenderType == NAMEK_ADULT ||
