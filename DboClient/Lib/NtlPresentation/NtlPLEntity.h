...
protected:
	...
	struct unknownNewStructIfUnknownShit2IsTrue {
		DWORD dwMoreUnknownShit[2];

		DWORD dwMoreUnknownShitNum;
		struct amountOftheseDependOnShitNum {
			DWORD dwUnknownShitBool; //??

			struct onlyHereIf_ShitBool_IsTrue {
				unsigned int uiLength;
				char acName[128];
				unsigned int uiLength2;
				char acName2[128];

				unsigned int idkSize;
				BYTE *idkArray = nullptr; // [idkSize*8]

				~onlyHereIf_ShitBool_IsTrue() {
					if (idkSize > 0 && idkArray != nullptr)
						delete[] idkArray;
				}
			};
			onlyHereIf_ShitBool_IsTrue *pOnlyHereIf_ShitBool_IsTrue = nullptr;

			~amountOftheseDependOnShitNum() {
				if (dwUnknownShitBool > 0 && pOnlyHereIf_ShitBool_IsTrue != nullptr) {
					delete[] pOnlyHereIf_ShitBool_IsTrue;
					pOnlyHereIf_ShitBool_IsTrue = nullptr;
				}
			}
		};
		amountOftheseDependOnShitNum *pamountOftheseDependOnShitNum = nullptr;

		unknownNewStructIfUnknownShit2IsTrue() {};
		~unknownNewStructIfUnknownShit2IsTrue() {
			if (dwMoreUnknownShitNum > 0 && pamountOftheseDependOnShitNum != nullptr) {
				delete[] pamountOftheseDependOnShitNum;
			}
		};
	};

	unknownShit uUnknownUnionShit;
	RwInt32 iUnknownShit2; // Probably bool, maybe iNum?
	unknownNewStructIfUnknownShit2IsTrue *pUnknownNewStructIfUnknownShit2IsTrue = nullptr;

public:
	void LoadUnknownStructFromFile(FILE* pFile);
	void SaveUnknownStructToFile(FILE* pFile);

	static void SkipUnknownStructToFile(BYTE *&pFileMemTmp, RwInt32 dwUnknownShit2);
protected:
#endif
...
public:
	...
	*/
    ...
    /************************************************************************/
    /* Entity를 바로 삭제하지 않고, 현재 업데이트중인것을 다 끝내고 삭제할때*/
    /* 호출하는 인터페이스                                                  */
    /************************************************************************/
    virtual void   Finish() {}
	...
	*/
	...
	/**
	*  renderware resource를 world에 add 한다.
	*  \see RemoveWorld
	*/
	...
	*/
	...
	DWORD		GetObjNameIdx();
#ifdef TW_VER
	VOID		SetUnknownUnion(unknownShit unkUnion);
	unknownShit GetUnknownUnion();

	VOID		SetUnknownInt32(RwInt32 iRwInt32);
	RwInt32		GetUnknownInt32();
#endif
...
#ifdef TW_VER
inline void CNtlPLEntity::SetUnknownUnion(unknownShit unkUnion) {
	uUnknownUnionShit = unkUnion;
}


inline CNtlPLEntity::unknownShit CNtlPLEntity::GetUnknownUnion() {
	return uUnknownUnionShit;
}

inline void CNtlPLEntity::SetUnknownInt32(RwInt32 iRwInt32) {
	iUnknownShit2 = iRwInt32;
}


inline RwInt32 CNtlPLEntity::GetUnknownInt32() {
	return iUnknownShit2;
}
...
#ifdef dNTL_WORLD_CULLING_NEW
	// m_uiRenderFrame이 초기값(-1)과 동일하다면 CullingTest를 하지 않는 Entity다.
	if (uiFlags)
	{
		uiFlags &= m_uiCullFlags;
		// PVS Flags는 모든 Entitiy가 가지고 있다. CullingTest를 하지 않아도 PVS Flags는 존재 한다.
		if (((uiFlags & NTL_PLEFLAG_CULLED_PVS) && CNtlPLGlobal::m_UseTerrainPVSMode))
		{
			return FALSE;
		}		
		// PVS를 제외한 나머지 Flag는 RenderFrame이 동일해야지만 최신 정보라는 의미이며 다르다며 보이지 않는 것인다.
		// 단 RenderFrame이 -1인 경우는 FrameTest를 사용하지 않는 경우 이기 때문이 Flag 정보의 가치가 없다.
