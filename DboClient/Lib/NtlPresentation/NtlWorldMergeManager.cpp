...
#ifdef TW_VER
		sprintf_s(sTmpPrjParam.WorldChar64Buf, 64, "propinfo_od_%04d.dat", i);
#else
		sprintf_s(sTmpPrjParam.WorldChar64Buf, 64, "wfif%d", i);
#endif
...
#ifdef TW_VER
		BYTE byHeader[4] = { 'N', 'V', 'P', 'I' };
		fwrite(&byHeader, sizeof(byHeader), 1, pFile);
		DWORD idk = 0;
		fwrite(&idk, sizeof(DWORD), 1, pFile);
#endif 
...
#else
	iTileSize = 4; // code above is fk stupid, tile is always 4
#endif
				...
				for (int iGX = iX; iGX < iX + _iGroupFieldNum; ++iGX)
				{
					...
					for(RwInt32 iSectorCnt = 0; iSectorCnt < sWorldParam.WorldFieldSectorNum * sWorldParam.WorldFieldSectorNum; ++iSectorCnt)
					{
						...
						for (int iDoodadCnt = 0; iDoodadCnt < iNumDoodads; ++iDoodadCnt)
						{
							...
							DWORD dwNameIdx;
							memcpy(&dwNameIdx, pFileMemTmp, sizeof(dwNameIdx));
							pFileMemTmp += (sizeof(DWORD)); // Obj. name index

#ifdef TW_VER
							CNtlPLEntity::unknownShit uUnknownShit;
							memcpy(&uUnknownShit.fVisibilityDistance, pFileMemTmp, sizeof(RwReal));
							pFileMemTmp += (sizeof(uUnknownShit.fVisibilityDistance));

							if (uUnknownShit.dwUnknownShit == 0xFFFFFFFF) {
								pFileMemTmp += sizeof(RwReal); // fVisibilityDistance
							}
							else if (dwNameIdx != 0xFFFFFFFF) {
								pFileMemTmp += sizeof(RwReal); // fVisibilityDistance
							}
...
#ifdef TW_VER
							RwInt32 dwUnknownShit2 = 0;
							if (uUnknownShit.dwUnknownShit == 4294967295) {
								memcpy(&dwUnknownShit2, pFileMemTmp, sizeof(dwUnknownShit2));
								pFileMemTmp += sizeof(dwUnknownShit2);
							}
							else if (dwNameIdx != 0xFFFFFFFF) {
								memcpy(&dwUnknownShit2, pFileMemTmp, sizeof(dwUnknownShit2));
								pFileMemTmp += sizeof(dwUnknownShit2);
							...
							if (dwUnknownShit2 != 0) {
								CNtlPLEntity::SkipUnknownStructToFile(pFileMemTmp, dwUnknownShit2);
							}
#endif
							...
							sprintf_s(acNaviFind, NTL_MAX_DIR_PATH, "%s\\Tool\\ToolShareData\\NaviModel\\%s_PE_Nav_*.dff",
										(&g_sNtlWorldParam)->CurWorkingFolderName, acDoodadName);
					...
					// Open wfif ~Nady
					FILE *pFileWFIF = NULL;
					fopen_s(&pFileWFIF, sWorldParam.WorldChar64Buf, "rb");
					_chdir(sWorldParam.WorldProjectFolderName);
					...
					DWORD dwHeightCount = (sWorldParam.WorldSectorSize / iTileSize + 1) * (sWorldParam.WorldSectorSize / iTileSize + 1);
...
#ifdef TW_VER
							if (pFileWFIF) {
								RwReal* pfHeight = new RwReal[dwHeightCount]; // TODO: Corner height or center?


								CNtlWorldSector *pNtlWorldSector = new CNtlWorldSector;
								GetNtlWorldSectorInfo()->SectorMaterialSkipToFile(pFileWFIF);

								RwV3d* pvVect = new RwV3d[dwHeightCount];
								GetNtlWorldSectorInfo()->SectorHeightfieldLoadFromFile(pFileWFIF, pvVect); //?? get Y only?
								for (int i = 0; i < dwHeightCount; ++i) {
									pfHeight[i] = pvVect[i].y;
								}
								delete[] pvVect;

								GetNtlWorldSectorInfo()->SectorPrelightsSkipToFile(pFileWFIF);

								GetNtlWorldSectorInfo()->SectorObjectLoadFromFile(pFileWFIF, pNtlWorldSector, AW_HEGITHFIELD);
								//delete pNtlWorldSector; // Because skip doesnt ready for TW

								GetNtlWorldSectorInfo()->SectorWaterSkipToFile(pFileWFIF);
								GetNtlWorldSectorInfo()->SectorShadowSkipToFile(pFileWFIF);
								GetNtlWorldSectorInfo()->SectorEffectSkipToFile(pFileWFIF);
								GetNtlWorldSectorInfo()->SectorSoundEffectSkipToFile(pFileWFIF);
								GetNtlWorldSectorInfo()->SectorCullSkipToFile(pFileWFIF);
								GetNtlWorldSectorInfo()->SectorTileTransparencySkipToFile(pFileWFIF);
								GetNtlWorldSectorInfo()->SectorShoreLineSkipToFile(pFileWFIF);
								GetNtlWorldSectorInfo()->SectorDecalSkipToFile(pFileWFIF);
								GetNtlWorldSectorInfo()->SectorPlantSkipToFile(pFileWFIF);
								GetNtlWorldSectorInfo()->SectorWorldLightSkipToFile(pFileWFIF);
								GetNtlWorldSectorInfo()->SectorOccluderSkipToFile(pFileWFIF);
								GetNtlWorldSectorInfo()->SectorHeatHazeObjectSkipToFile(pFileWFIF);
								GetNtlWorldSectorInfo()->SectorLightObjectSkipToFile(pFileWFIF);
								GetNtlWorldSectorInfo()->SectorDojoSkipToFile(pFileWFIF);
