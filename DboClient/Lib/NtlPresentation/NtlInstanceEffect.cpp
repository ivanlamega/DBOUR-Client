...
void CNtlInstanceEffect::CreateSystem(CNtlResourceEffect* pResourceEffect, const RwMatrix& matRender)
{
	...
	if (!pResourceEffect->m_svResourceSystem.empty())
	{
		...
		for ( it = pResourceEffect->m_svResourceSystem.begin(); 
			  it != pResourceEffect->m_svResourceSystem.end(); 
			  ++it)
		{
			...
			{
				CNtlInstanceComponentSystem* pInstanceComponentSystem = 
					CNtlEffectSystemFreeList::CreateSystem(pResourceComponentSystem->GetSystemType());
...
/**
 * Effect를 Update를 한다.
 * \param fElapsedTime 전 Frame에서 현재까지 흐른 시간
 */
RwBool CNtlInstanceEffect::Update(RwReal fElapsedTime)
{
    ...
    {
        // NOTE: 안보이거나 컬링되는 경우에도 주기적으로 업데이트는 해줘야 한다.
	...
	for (it = m_svSystem.begin(); it != m_svSystem.end(); ++ it)
	{
        // ComponentSystem이 모두 False를 반환해야지만,Effect가 False를 반환한다.
...
/**
 * Effect를 Rendering을 한다.
 */
RwBool CNtlInstanceEffect::Render()
{
	if (m_pResourceEffect && m_pResourceEffect->m_bShow == FALSE) return TRUE;

	if (!m_bShow || m_bIsCulling) return TRUE;    
	...
	if (!CNtlWorldObjVisionAnalyzer::GetInstance()->GetCheck(eNWOVA_VISIBILTITY_DIST))
	{
		...
		fVisibleDistance = 1512.0f;
#else
		fVisibleDistance = 2048.0f;
	...
	for (it = m_svSystem.begin(); it != m_svSystem.end(); ++ it)
	{
		...
		if ((m_pResourceEffect && (*it)->m_pResourceComponentSystem->m_bShow) ||
            !m_pResourceEffect)
		{
			(*it)->Render();
		}
...
void  CNtlInstanceEffect::SetMatrix(RwMatrix& matWorld)
{
    ...
    // NOTE : Attach된 Mesh System들은 매 업데이트시마다 강제로 스케일된 Bone의 Matrix를 받아오기 때문에, 
    // False인 경우 원래 스케일로 변환하는 처리가 필요하다.
    if(!m_bApplyScale)
    {
 ...
 void CNtlInstanceEffect::SetPosition(const RwV3d *pPos)
{
	...
	RwV3d vSubDist;
	RwV3dSub(&vSubDist, pPos, pvOldPos);
...
void CNtlInstanceEffect::SetScale( RwReal fScale)
{
    ...
    if(m_bApplyScale)
    {
	    ...
	    for (it = m_svSystem.begin(); it != m_svSystem.end(); ++ it)
	    {
		    // 각 이펙트 System의 Scale을 설정하는데, 파티클 System에서만 오버라이딩되어있다.
		    // 다른 Effect System들은 SetTransform()의 Matrix 설정만으로 Scale이 가능하다. (2007.7.19 by agebreak)
		    (*it)->SetScale(fScale);
	    }
    }
}
