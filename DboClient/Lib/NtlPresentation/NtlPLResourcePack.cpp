...
const char* CNtlPLResourcePack::encrypt(std::string data)
{
		char key[5] = { 'D', 'B', 'O', 'N', 'U' }; //Any chars will work
...
RwInt32 CNtlPLResourcePack::LoadPack(const RwChar *pFileName)
{
	...
	bool bSuccess = s.LoadFile((char*)pFileName, true, NTL_PACK_CRYPT_KEY); //FFCF4403F048FFC683C30883FB1072E1SDFDF4D DANNY-BOI
	if(!bSuccess)
...
RwInt32 CNtlPLResourcePack::SavePack(const RwChar *pFileName)
{
	...
	{
		NTL_ASSERTFAIL("CNtlPLResourcePack::SavePack is header name empty !!!");
		return NTL_FILEPACK_ERROR;
	}
	...
	strExt = szExt;
	const char* crypted = encrypt(NTL_PACK_CRYPT_KEY);
...
{
	for(RwInt32 i = 0; i < (RwInt32)MAX_NTL_PACK_TYPE; i++)
	{
		m_pResPack[i] = NTL_NEW CNtlPLResourcePack;
	}
    ...
    // ts
    pResPack = m_pResPack[NTL_PACK_TYPE_TS];
    pResPack->RegistHeaderName(NTL_PACKNAME_TS_HEADER);
    pResPack->RegistDataName(NTL_PACKNAME_TS_DATA);
...
RwChar* CNtlPLResoucePackManager::DumpFile(const char *pFileName, RwUInt32& uiSize)
{
	FILE *fp = NULL;
...
RwInt32 CNtlPLResoucePackManager::SaveTsPack( void ) 
{
    std::list<std::string> listFolder;
    listFolder.push_back(NTL_FOLDER_TS);

    return SavePack(NTL_PACK_TYPE_TS, listFolder, NTL_PACKNAME_TS_HEADER, NTL_PACKNAME_TS_DATA);
}
...
void CNtlPLResoucePackManager::LoadTs( const RwChar* pFileName, void** pData, RwInt32* iSize ) 
{
    *pData	= NULL;
    *iSize = 0;
    ...
    pPathRoot = rwstrstr(m_chFileBuffer, "ts");
    ...
    CNtlPLResourcePack *pResPack = m_pResPack[NTL_PACK_TYPE_TS];
    pElement = pResPack->FindHeader(m_chFullBuffer);
    if(pElement == NULL)
        return;
