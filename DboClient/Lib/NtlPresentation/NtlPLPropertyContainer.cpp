...
RwBool CNtlPLPropertyContainer::LoadObject(const RwChar *pFileName)
{
	...
	CNtlPLObjectProperty::m_strVer = chBuffer;
...
RwBool CNtlPLPropertyContainer::LoadItemForDataEditor(CNtlFileSerializer& sl)
{
	RwBool result = LoadItemSerialize(sl);
	if (result)
		return TRUE;
	else
		return FALSE;
...
RwBool CNtlPLPropertyContainer::LoadCharacter(const RwChar *pFileName)
{
	...
	else
	{
		...
		CNtlPLCharacterProperty::m_strVer = chBuffer;
...
RwBool CNtlPLPropertyContainer::LoadEffect(const RwChar *pFileName)
{
	...
	if (nSplineCount > 0)
	{
		...
		for (RwInt32 i = 0; i < nSplineCount; ++ i)
		{
			...
			// merge 할 경우 이름이 같은 Spline은 같은 내용이다. (전제조건)
			// 때문에 중복해서 추가하지 않는다.
...
RwBool CNtlPLPropertyContainer::LoadSerialize(RwBool bEncrypt /* = FALSE */, RwChar* szCryptPassword)
{
	...
	else
	{
		...
		if(bEncrypt)
		{
			bResult = nsl.LoadFile(".\\property\\characterproperty.edf", TRUE, szCryptPassword);
		}
		else
		{
			bResult = nsl.LoadFile(".\\property\\characterproperty.rdf");
		}	
		...
		if(bEncrypt)
		{
			bResult = nsl.LoadFile(".\\property\\objectproperty.edf", TRUE, szCryptPassword);
		}
		else
		{
			bResult = nsl.LoadFile(".\\property\\ObjectProperty.rdf");
		}	
		...
		if(bEncrypt)
		{
			bResult = nsl.LoadFile(".\\property\\itemproperty.edf", TRUE, szCryptPassword);
		}
		else
		{
			bResult = nsl.LoadFile(".\\property\\itemproperty.rdf");
		}
...
RwBool CNtlPLPropertyContainer::LoadObjectSerialize( CNtlFileSerializer& sl ) 
{
	...
	for(UINT i = 0; i < uiPropertySize; ++i)
	{
		CNtlPLObjectProperty* pProperty = NTL_NEW CNtlPLObjectProperty();
		pProperty->LoadSerialize(sl);
		if (pProperty->m_strDffFile == "10_N_MUSHROOM_001.DFF")
			printf("Mushroom Found");
