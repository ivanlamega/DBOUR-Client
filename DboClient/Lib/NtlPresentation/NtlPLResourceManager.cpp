...
std::vector<std::string> split_clump(std::string s, std::string delimiter) {
...
CNtlPLResource* CNtlPLResourceManager::LoadDefault(const char *pStrName, const char * pResourcePath, unsigned int uiType)
{
	...
	*/
	...
	if(pResource == NULL)
	{
		...
		//Mod Loading for object files? 
		//std::string file, completeFile;
		//file = std::string(pFileName);
		//std::transform(file.begin(), file.end(), file.begin(), ::tolower);
		//std::string lowerFile = file;
		//std::string strPath;
		//
		//string directory;
		//const size_t last_slash_idx = completeFile.rfind('\\');
		//if (std::string::npos != last_slash_idx)
		//{
		//	directory = completeFile.substr(0, last_slash_idx);
		//}

		//auto tokens = split_clump(directory, ";");

		////file.insert(file.size(), ".dff");
		//std::string fullFile = directory + file;

		//if (lowerFile.compare("a_hch_head08_h_m.dff") == 0)
		//{
		...
		//	std::size_t foundTok4 = token.find("item");
		...
		//		//Close it because its going to load here first. 
		//		RwStreamClose(pStream, NULL);
		//		//now open our new mod. 
		//		pStream = RwStreamOpen(rwSTREAMFILENAME, rwSTREAMREAD, (RwChar*)newFile.c_str());
		...
		else    // 해당되는 Clump를 Load하지 못하면 NULL을 반환한다. (by agebreak 2006-11-29)
		{
			if(fp)
				RwFclose(fp);
	...
	{
		// 같은 이름의 resource가 존재할 때.
		// clone 데이터가 아닌 animation, world.
...
RwTexture* CNtlPLResourceManager::LoadTexture(const char *pStrName, const char * pResourcePath, EResDictType eType /*= RES_TEX_DEFAULT_DICT*/ , const char *pStrMaskName /*= NULL*/)
{
	...
	if (pTexture == NULL)
	{
		...
		DBO_ASSERT(pTexture, "the texture find not(" << pStrName << ")" << "-" << acCurrentDirectory << "-" << pResourcePath);
	}
	
	//RwTextureSetFilterMode(pTexture, eFilterMode);
	//by HongHoDong(2006.6.15) MIP 옵션이 있는 경우
	//Save를 하고 읽을 경우 Mip이 Check되어 있으면 Mipmap을 찾는다.
	//dds의 경우는 문제가 없으나 png파일의 경우 문제가 생긴다.
...
//	nHeight = GetSortSize(nHeight);
	...
	//RwTextureSetFilterMode(pRwTexture, eFilterMode); 
