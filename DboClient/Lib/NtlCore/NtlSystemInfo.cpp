...
const char* GetSystemMacAddress()
{
	...
	uint8_t uidSize = sizeof(acMACAddress);

	char macAddress[15];
	memset(macAddress, 0, sizeof(macAddress));
	if ((sizeof(macAddress) - 1) / 2 < uidSize)
	{
		DBO_FAIL("Character buffer too small for mac address. \n");
	}
	for (int cnt = 0; cnt < uidSize; cnt++)
	{
		// convert byte to its ascii representation
		sprintf(&macAddress[cnt * 2], "%02X", acMACAddress[cnt]);
	}
	OutputDebugString(macAddress);
	return macAddress;
}